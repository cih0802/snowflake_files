...
TABLE_KOR_MAP = {
    ...
    "DIM_MEMBER": "회원 차원(1행/회원 · 현재 상태)",
    "DIM_MEMBER_ACQUISITION": "회원 획득 귀속 차원",
    # 🔴 [2026-09-16 O166] `DIM_MEMBER_CURRENT` 항목을 지웠다 — **개명이 아니라 객체 소멸**이다.
    #   개편 후 `DIM_MEMBER` 자체가 1행/회원(실측 1,785,299 = 회원 유일수)이고
    #   상태버전 이력은 `DIM_MEMBER_STATUS_HISTORY` 다. 🔴 `DIM_MEMBER` 로 치환하면 이 dict 의
    #   기존 `"DIM_MEMBER"` 키(위 줄)와 **겹쳐 조용히 덮어쓴다** ⇒ 치환이 아니라 삭제가 맞다.
    ...
    "FACT_MEMBER_DEV_ACHIEVEMENT": "회원개발 목표 대비 실적 팩트",
    "FACT_EVENT_ATTENDANCE": "행사 참여 팩트",
    ...
    "FACT_MEMBER_SPONSORSHIP_SPAN": "회원×후원약정 팩트",
    "FACT_MESSAGE_DISPATCH": "서비스/발송 이벤트 팩트",
    "FACT_TARGET_PROJECT": "사업 목표 팩트",
    "FACT_TARGET_MEMBER_DEV": "회원개발 목표 팩트",
    # GOLD WIDE
...
TABLE_GRAIN_MAP = {
    ...
    # 🆕 🔴 [2026-09-16 O166 실측] grain 이 바뀌었다 — 종전 기재 「회원 × 상태버전 (SCD2)」는 stale 이다.
    #   실측 = 1,785,299행 · `MEMBER_DK` 유일 1,785,299 · `MEMBER_SK` 유일 1,785,299 ⇒ **1행/회원**.
    #   상태버전 이력은 `DIM_MEMBER_STATUS_HISTORY`(8,069,279 = 회원당 4.52행)로 분리됐다.
    "DIM_MEMBER": "1회원 현재상태 (MEMBER_DK · MEMBER_SK 유일)",
    "DIM_MEMBER_ACQUISITION": "1회원 획득시점 (MEMBER_DK)",
    # 🔴 [O166] `DIM_MEMBER_CURRENT` 삭제 — 객체 소멸(위 `DIM_MEMBER` 가 그 역할을 흡수) · 키 중복 회피.
    ...
    "FACT_MEMBER_DEV_ACHIEVEMENT": "MONTH_KEY × ORG_SK × DEV_TYPE",
    "FACT_EVENT_ATTENDANCE": "DATE_SK × MEMBER_DK × EVENT_SK",
    ...
    "FACT_MEMBER_SPONSORSHIP_SPAN": "MEMBER_DK × SPNSR_BSNS_NO",
    "FACT_MESSAGE_DISPATCH": "DATE_SK × MEMBER_DK × SERVICE_SK × CAMPAIGN_SK",
    "FACT_TARGET_PROJECT": "MONTH_KEY × ORG_SK × SPONSORSHIP_SK",
    "FACT_TARGET_MEMBER_DEV": "MONTH_KEY × ORG_SK × DEV_TYPE",
    # GOLD WIDE
