...
DOCS = [
    ...
    '99_NEXT_SESSION.md',
    #   🆕 🔴🔴 [2026-09-16 O165 편입] Inspection 절차서 — **이 분모에 없어서 실사고를 놓쳤다.**
    #     O165 가 `edit` 앵커로 `## ▣ 5` 제목을 삼켰고 그 편집에서 **파일 꼬리 36줄(§6·§7 전체)이
    #     함께 소실**됐는데, 이 게이트는 그 문서를 **보지 않으므로 🟢 를 유지했다**.
    #     🔴 이것이 `R3-9 ㉢`(분모 등재와 기준선 발행은 별개의 두 동작)의 실물이고,
    #       `R1-6-18 ④`(폴더 밖 문서는 손으로 편입하라)를 이 문서가 이행하지 않은 결과다.
    #     🟢 이 문서는 **판정식 카탈로그·실패 사례의 정본**이라 제목 유실이 곧 자산 유실이다.
...
def chunk_files(p):
    ...
    🆕 🔴🔴 [2026-09-17 O172 · 의도적 미편입 — 결정을 여기 적는다]
      **인수인계 라벨 파일**(`99_NEXT_SESSION-O0172-A.md`)은 이 게이트 분모에 **넣지 않는다.**
      🔎 왜 = 이 게이트의 축은 「**기존 파일을 편집하다가 제목을 삼켰는가**」다(실사고 = 앵커로
        제목 파괴 5회). 라벨 파일은 **매 세션 새로 생기고 그 뒤 고치지 않는** append 단위이므로
        편입하면 **세션마다 「제목 신설」로 골든 재발행이 필요**해진다 ⇒ 그것은 O172 가 없애려던
        비용 그 자체다(종전 취소선 → `--update-golden` 경로와 같은 형태로 되돌아간다).
      🔴 **미편입은 「검사 안 함」이므로 대체 축을 명시한다** = ㉠ 줄길이는
        `doc_line_length_gate.CANON_GLOB` 이 본다(`*-O[0-9][0-9][0-9][0-9]-*.md`)
        ㉡ 상한은 `doc_type_gate` 가 조각 폴더 전건을 재서 본다
        ㉢ 좌표는 `doc_coord_gate` 가 그 폴더를 walk 하며 본다
        ㉣ 규격 위생은 `split_doc --verify` 의 「라벨 파일 규격 위반」 경고가 본다.
      🟢 판정식 = **「분모에서 뺀다」는 결정은 뺀 이유와 대체 축을 함께 적어야 결정이 된다.**
      🔎 이 파일은 `-%03d` 로 **구성해** 열거하므로 라벨 파일은 **원리적으로** 섞이지 않는다
        (사이드카·라벨을 `listdir` 로 쓸어담던 `doc_census` 와 달랐고, 그 차이가 조각 수
        **35 ↔ 36** 불일치의 원인이었다 — O172 가 `doc_census` 쪽을 이 방식으로 맞췄다).
