...
CANON_GLOB = [
    ...
    # ── [2026-08-18 O82-C → 2026-08-28 O108 교체] 조각 분할 산출물 ─────────
    ...
    # 🔴 [2026-08-28 O108] 7문서 전부 폴더로 이전 완료 ⇒ 직속 형제 패턴 `20_issue/*-NNN.md` 는
    #   **0건**이 되어 죽은 패턴이 됐다. `20_issue/*/*-NNN.md` 가 이미 전건을 커버한다.
    '20_issue/*/*-[0-9][0-9][0-9].md',            # 20_issue 폴더 조각 (모든 분할 문서)
    # 🔴 [2026-08-28 O107] 99_NEXT 조각을 **폴더로 이전**했다(`--to-outdir`).
    '99_NEXT_SESSION_조각/*-[0-9][0-9][0-9].md',   # 인수인계 폴더 조각
    # 🆕 🔴 [2026-08-29 O112] `00_guides/01_문서분할_규약.md` 를 여유 부족으로 분할했다
    #   ⇒ 그 조각도 사람이 읽는 정본이므로 편입한다(`R1-6-18 ④` 분모 4곳 중 이 게이트분).
    #   🟢 `00_guides/*_조각/` 로 두면 이후 다른 가이드를 분할해도 자동 추종한다.
    '00_guides/*_조각/*-[0-9][0-9][0-9].md',      # 00_guides 폴더 조각
    # 🆕 🔴🔴 [2026-09-17 O170] **조각 폴더 사이드카**(`00_선택표.md`) 편입.
    #   🔴 왜 = 위 패턴은 `-NNN.md` 만 보므로 사이드카가 **분모 밖**이었고
    #     `doc_census.split_denominator_check()` 가 **「조각 1개가 줄길이 분모 밖이다」로 11건** 잡았다
    #     (그 함수가 `R1-6-25` 의 집행부다 — 조문이 아니라 그 함수가 「분할 후 점검」의 정본이다).
    #   🟢 사이드카는 자동 생성물이지만 **줄길이는 검사한다** — `read` 절단은 생성물에서도 손실이다.
    #   🔴 파일명이 `-NNN.md` 와 겹치지 않으므로 **조각 열거에는 섞이지 않는다**(별 축이다).
    '20_issue/*_조각/00_선택표.md',
    '99_NEXT_SESSION_조각/00_선택표.md',
    '00_guides/*_조각/00_선택표.md',
    # 🆕 🔴🔴 [2026-09-17 O172 신설 · 사용자 지시] **인수인계 라벨 파일 + 그 색인** 편입.
    #   🔴 왜 = 라벨 파일(`…-O0172-A.md`)은 **조각이 아니므로** 위 `-[0-9][0-9][0-9].md` 패턴에
    #     걸리지 않는다 ⇒ 편입하지 않으면 **매 세션 새로 생기는 정본이 검사 밖**이 된다.
    #     🔎 이 게이트가 막는 손실은 「`read` 가 1줄 2,000자에서 절단한다」이고(`R1-5-2`),
    #     인수인계는 **다음 세션이 반드시 읽는 문서**라 절단 손실이 가장 비싸다.
    #   🟢 `00_인수인계_색인.md` 도 자동 생성물이지만 사이드카와 같은 근거로 검사한다.
    #   🔴 이 두 패턴은 **조각 열거와 별 축이다** — 조각 수를 세지 않는다(`doc_census.chunk_rx` 소관).
    '99_NEXT_SESSION_조각/*-O[0-9][0-9][0-9][0-9]-*.md',
    '99_NEXT_SESSION_조각/00_인수인계_색인.md',
]
