...
NUM_EXEMPT = [
    ...
    #   🆕 🔴🔴 [2026-09-10 O155] **`re.I` 를 붙인다** — `DEC-52` 4블록 표준이 `[Grain: …]` 를
    #      **대문자 G** 로 쓰는데 이 두 패턴은 소문자 `grain` 만 봤다 ⇒ **표준을 따르면 규칙7 위반**이 났다.
    #      🔎 실물(O155) = `SV_MEMBER_EVENT` COMMENT 를 4블록으로 표준화하며 `[Grain: 회원 × 일 × 사건 1행]`
    #        으로 쓰자 `소규모` 축(`N행`)에 걸려 **라이브 도달 위반 1건**이 됐다. 종전 문안은
    #        소문자 `grain: … 1행` 이어서 우연히 면제되고 있었다.
    #      🔴 나머지 16종 SV 는 `(1행=1회원)` 처럼 `=` 를 써서 **다른 면제**에 걸렸기 때문에 이 충돌이
    #        드러나지 않았다 ⇒ **규약과 게이트가 어긋난 채 잠재해 있었다**(`R3-9 ㉡` 의 실물).
    #      🟢 `\d{1,2}` 자릿수 제한은 그대로다 ⇒ 「grain 기준 적재 2,170,572행」은 여전히 잡힌다.
    (re.compile(r'grain[^.。]{0,30}?(?<![\d,])\d{1,2}\s*행', re.I),
     'grain 정의(grain 이 앞 · 계열당 1행 포함 · 대소문자 무관) — 구조이고 적재량과 무관'),
    (re.compile(r'(?<![\d,])\d{1,2}\s*행[^.。]{0,40}?grain', re.I),
     'grain 대비 서술(행이 앞 · 쉼표 대비 포함 · 대소문자 무관) — 구조이고 적재량과 무관'),
