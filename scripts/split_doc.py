...
# 🆕 🔴🔴 [2026-09-10 O154-B] **항목 캡을 30/80 → 14/40 으로 낮춘다.**
#   🔴 왜 = `99_NEXT_SESSION` 허브가 **40,782 / 40,960 B**(여유 **178 B**)까지 차서 다음 세션이
#     인수인계 절을 추가하면 **상한을 넘는다**. 허브 구성 실측(O154-B) =
#     머리말 1,781 · 조각목차 4,843 · **선택표 31,545(77.3%)** · 대응표 2,614.
#     ⇒ 지배 항목이 **선택표**이고 그 크기는 `조각수 × (절 캡 + ID 캡)` 이다.
#   🟢 왜 안전한가 = 이 표는 **색인**이고 이미 `… 외 N건` 오버플로 표기를 쓴다(전량이 아님을 선언).
