...
EXTRA_DOCS = [
    ...
    #   🆕 [2026-09-21 O173] `R3` 무변경 이관 신설(처방 = O172-C-4 ㉠안 · 지연 로드 문서).
...
#   🔴 왜 별 유형인가 = 처리가 셋과 **다르다**:
#     ㉠ **손으로 고치지 않는다**(고치면 다음 재발행에서 조용히 사라진다)
#     ㉡ **분할하지 않는다** — 색인을 쪼개면 「어느 조각을 읽을지」를 못 고른다
#     ㉢ 상한에 닿으면 재균형·은퇴가 아니라 **밀도(항목 캡)를 내린다**
#     ㉣ **인용 정본이 아니다**(절단·캡이 있다).
#   ⇒ 그래서 여유 판정(`MIN_FREE`)은 **경고로만** 본다 — 갱신형처럼 「다음 편집에서 확정 초과」가
#     아니고, 사람이 그 파일에 행을 더하지 않기 때문이다. 정본 = `R1-6-26`.
AUTOGEN = '자동생성'
...
def actual_docs():
    ...
    for rel in EXTRA_DOCS:
        ...
        if os.path.isdir(chunk_dir):
            # 🆕 🔴 [2026-09-17 O172 · 의도적 **편입** — 여기만 전건을 본다]
            #   이 게이트는 「조각 수」가 아니라 **상한(300줄 AND 40KB)** 을 재므로 폴더의
            #   **사이드카·인수인계 라벨 파일까지 전건 검사하는 것이 옳다** — 상한 초과는 파일 종류와
            #   무관하게 `read` 전량 확보를 깨뜨린다.
            #   🔴 그러므로 `doc_census.chunk_rx`(조각만) 와 **분모가 다른 것은 결함이 아니라 설계**다.
            #     🟢 판정식 = **「같은 것을 다르게 재지 마라」는 「다른 것을 같게 재라」가 아니다** —
            #       세는 대상이 다르면(조각 수 ↔ 상한 준수) 분모도 달라야 하고, 그 차이를 **적어야** 한다
            #       (적지 않은 차이가 조각 수 35 ↔ 36 불일치였다).
...
def main():
    ...
    if fails:
        ...
        return 1
    if warns:
        # 🆕 🔴 [2026-08-30 O123-D 시정] **두 축을 함께 낸다.**
        #   종전 = `경고 %d건` 이 `len(warns)`(= **메시지 수**)였는데 바로 위 상세는
        #   `여유 부족 2건`(= **항목 수**)을 냈다 ⇒ 한 도구의 출력 안에서
        #   **「경고 1건」과 「2건」이 동시에 보였다**(실측 · `R3-9 ㉡` 「같은 것을 다르게 재는 지점」).
        #   🔴 사람은 이것을 「경고가 1개」로 읽고 **한 문서만 손본다.**
        #   ⇒ 🟢 판정식 = 집계 단위가 둘이면 **둘 다 이름 붙여 출력**하라(하나를 고르면 반드시 오독된다).
        print('🟡 통과(경고 %d건 · 대상 항목 %d건)' % (len(warns), len(tight)))
        for w in warns:
            print(' -', w)
        return 0
