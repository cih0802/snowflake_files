...
""")
    ...
    axis('🔴🔴 중첩 취소선 + 승계 문구 + 말단·0건 가드 (O165 신설 · 오염 기반)')
    #   🔴 **이 축이 없어서 실사고를 놓쳤다.** 위 세 픽스처는 `~~` 가 **겹치지 않는다**.
    #     정본에는 겹치는 형태가 있다 = `## ~~0-DDDD. … ~~여기서 시작한다~~ … ~~`
    #     ⇒ `STRIKE_SPAN` 의 비탐욕 쌍 맞추기가 **문구를 구간 밖으로 밀어내** 「살아 있다」로 봤고,
    #       O163 이 `§0-LLLL` 을 예고만 하고 쓰지 않아 **후보가 그 절 하나뿐**이었으므로
    #       브리핑이 **2026-08-31 절을 현행 시작점으로 발행**했다(O143 과 같은 사고의 3차 형태).
    #   🔴 픽스처는 `99_NEXT_SESSION_조각/99_NEXT_SESSION-003.md:80` ·
    #     `-022.md:192·201` 원문 형태를 그대로 옮겼다(창작 0).
    #   🆕 🔴🔴 **[O165 자기시정] 초판 `NESTED` 는 이 결함을 잡지 못했다.**
    #     초판은 픽스처 말미에 **살아 있는 `§0-LLLL` 을 넣었다** ⇒ 시정 전 코드에서도 그 절이
    #     날짜로 중첩 절을 이겨 **전건 통과**했다(실증 = `tmp/_o165_prefix_fail_probe.py` 뒤집힘 **0건**).
    #     🔴 실사고 조건은 **후속 절 부재**였다 — 중첩 절이 **유일 후보**여야 오선택이 난다.
    #     ⇒ 🟢 판정식 = **음성 픽스처는 「그 결함이 발현한 상태」를 재현해야 한다.**
    #       고친 코드로 통과하는 것을 확인하는 것만으로는 **그 축이 무엇을 보는지 알 수 없다**
    #       (절차서 `11_O누적작업_점검_재현_절차.md` §0 「성공 판정식」).
    NESTED_ONLY = """
...
"""
    #   🔴 이것이 실사고 그대로다 = 전 절이 승계됐고 후속 절이 **없다**.
    #     시정 전 코드는 중첩 절(§0-DDDD · 2026-08-31)을 **현행으로 발행**했다.
    co2, so2 = sb.current_handoff(L(NESTED_ONLY))
    check('중첩 절이 유일 후보여도 미선택', co2, None)
    check('그때 하위 항목도 0', so2, [])
    check('중첩 취소선은 승계로 판정', sb.superseded(
        '## ~~0-DDDD. 🔴🔴 [2026-08-31 O126 — ~~여기서 시작한다~~ · §0-EEEE 로 승계됐다]~~'), True)
    tail2 = sb.tail_handoff(L(NESTED_ONLY))
    check('말단 절 = §0-KKKK', '0-KKKK' in tail2['title'], True)
    #   🟢 후속 절이 생기면 그것이 현행이 된다(오탐 역방향).
    NESTED = NESTED_ONLY + (
        '## 0-LLLL. 🔴🔴 [2026-09-16 O165 필독 — **여기서 시작한다.** §0-KKKK 는 승계됐다]\n'
        '### ▣ LLLL1 현행 항목\n')
    cn, sn = sb.current_handoff(L(NESTED))
    check('중첩 취소선 절 미선택', cn is not None and 'O126' in cn['title'], False)
    check('현행 절 = O165(§0-LLLL)', '0-LLLL' in cn['title'], True)
    check('현행 절이 문서 말단', cn['is_last'], True)
    check('현행 절 항목 1건', len(sn), 1)
    #   🔴 「는 승계됐다」는 **현행 절이 스스로 적는 문구**다 ⇒ 승계 신호로 쓰면 살아 있는 절을 죽인다.
    check('「는 승계됐다」는 배제 사유 아님',
          sb.superseded('## 0-LLLL. 🔴 [O165 — **여기서 시작한다.** §0-KKKK 는 승계됐다]'), False)
    check('「로 승계됨」은 배제 사유', sb.superseded('## 0-KKKK. [O163 — 여기서 시작한다 · §0-LLLL 로 승계됨]'), True)
    check('「시작점은」도 배제 사유', sb.superseded('## 0-RRR. [O111-B — 여기서 시작한다 ⇒ 시작점은 위 §0-SSS 다]'), True)
    #   🔴 후보 0 = O163 이 실제로 만든 상태(예고만 하고 §0-LLLL 미작성) ⇒ **침묵하면 안 된다.**
    ORPHAN = """
## 0-JJJJ. 🟢 [2026-09-16 O162 — ~~여기서 시작한다~~ · **§0-KKKK 로 승계됨**]
### ▣ JJJJ1 항목
## 0-KKKK. 🟢 [2026-09-17 O163 — ~~여기서 시작한다~~ · **§0-LLLL 로 승계됨**]
### ▣ KKKK1 항목
"""
    co, so = sb.current_handoff(L(ORPHAN))
    check('예고만 하고 미작성 시 현행 0건', co, None)
    #   🔴 `subs` 에 말단 절을 끼워 넣지 않는다(축 혼용 금지 · 「항목이 있다」는 거짓 신호가 된다).
    check('후보 0 이면 하위 항목도 0', so, [])
    tail = sb.tail_handoff(L(ORPHAN))
    check('말단 절은 진단용으로 제공', '0-KKKK' in tail['title'], True)
    check('말단 절 좌표 운반', tail['where'].startswith('SYNTH.md:'), True)
    check('빈 입력의 말단은 None', sb.tail_handoff(L('')), None)
    #   🔴 말단 아님 가드 = 현행 뒤에 **승계 표기 없는 절**이 오면 경고 축이 켜져야 한다.
    NOT_LAST = """
## 0-AAA. 🔴 [2026-09-10 O200 필독 — **여기서 시작한다.**]
### ▣ 현행 항목
## 0-BBB. 🔴 [2026-09-01 O199 필독 — **여기서 시작한다.**] 날짜가 옛것인데 아래에 있다
### ▣ 옛 항목
"""
    cl, _sl = sb.current_handoff(L(NOT_LAST))
    check('날짜 최신 절을 현행으로', 'O200' in cl['title'], True)
