...
WHITE = re.compile(r'#[0-9]+|(?:CM|MM|MS|PM|CONF|DEC|O|P|E|G|Q|AD|SVL|R)-?[0-9]+|'
                   ...
                   # 🔴🔴 [2026-08-29 O120 신설] **서수형 라벨 접미** — `유형1`·`타입2` 의 숫자는
                   #   실측치가 아니라 **컬럼명의 일부**다. 실측 오탐 2건:
                   #   `CMPGN_TYPE1_NM` COMMENT 「캠페인 유형1명 (MM295 라벨)」 → 소규모 `1명`
                   #   `CMPGN_TYPE2_NM` COMMENT 「캠페인 유형2명 (MM296 라벨)」 → 소규모 `2명`
                   #   ⇒ 「유형1」+「명(칭)」이 「1명(사람 1명)」으로 읽혔다. 원천이 늘어도 변하지 않으므로
                   #     위 주석의 「타입은 수치가 아니다」 원칙에 그대로 해당한다.
                   #   🔴 좁게 잡았다 — 접미가 숫자에 **바로 붙은 경우만** 제거한다.
                   #     「유형 3건」처럼 공백이 끼면 여전히 잡힌다(무력화 아님 · test 축2).
                   r'유형[0-9]+|타입[0-9]+|'
                   ...
                   # 🔴🔴 [2026-08-29 O120 신설] **월↔연 환산 상수 12 는 달력 상수다.**
                   #   실측 오탐 1건: `FACT_BUDGET_YEARLY` 테이블 COMMENT
                   #   「FACT_BUDGET 은 월 grain 이라 연 총액을 담으면 SUM 이 12배가 된다」.
                   #   이것은 grain 혼입 경고이고 **적재량과 무관**하다(1년은 항상 12개월) ⇒ 위 O53 원칙 그대로.
                   #   🔴 좁게 잡았다 — `SUM 이` 선행 문맥이 있을 때만 면제한다.
                   #     **맨 「12배」는 여전히 잡힌다**(진짜 팬아웃 실측치일 수 있다 · 무력화 아님 · test 축2-C).
                   r'SUM\s*이\s*\*{0,2}12배|'
