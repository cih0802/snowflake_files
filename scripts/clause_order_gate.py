...
DOCS = [
    ...
    #   🆕 🔴 [2026-09-21 O173] `R3` 이관 신설 — **이것이 5번째 분모다.**
    #     🔴 `R1-6-18 ④` 는 분모 「4곳」을 요구하는데 **조문 문서는 여기까지 5곳**이다
    #       (선례 `01_문서분할_규약`·`02_파일쓰기_안전규약` 도 전부 여기 등재돼 있다).
    #     🔎 실사고(O173 자기적발) = 분모 4곳만 넣고 **「편입 완료」로 신고**했고,
    #       그 사이 `R3` 정본의 조문 순서·중복은 **검사되지 않았다**.
