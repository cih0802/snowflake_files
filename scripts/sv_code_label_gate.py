...
LABEL_PAIRS = {
    ...
    'SUBTYPE':         None,
    # 🆕 🔴🔴 [2026-09-17 O170] `CPR_DIV_CD`(법인구분) — **이 base 뷰에는 라벨 컬럼이 없다.**
    #   라이브 실측 = `SERVING.ML_MEMBER_RISK_V` 74,949행 · 코드 **2종**(`I` 8,727 · `S` 66,222).
    #   🟢 라벨 원천은 실재한다 = `GOLD.DIM_CAMPAIGN.CPR_DIV_NM`(`A`=통합 · `I`=사단 · `S`=사복).
    #   🔴 그런데 그 뷰에 **`CPR_DIV_NM` 컬럼이 없으므로** 여기 라벨명을 적으면 거짓이 된다
    #      (`if lab in base_cols` 로 걸러지긴 하나 등재부는 **주장 발행**이다 · `R2-7-4` 축).
    #   🔴 `A`(통합)는 이 SV 에 **부재**한다 ⇒ 「3종」으로 쓰면 코드값 실재 축에 걸리고
    #      「2종」으로 쓰면 원천이 3종이라는 사실이 감춰진다 ⇒ **코드값 열거는 근거철에 둔다**
    #      (`_o170_evidence.md` §E5-3). COMMENT 에는 코드값만(`R2-6`).
    #   🟢 그래서 `None` = **「라벨 컬럼 부재를 확인했다」**로 등재한다 — 이러면 「미등재」 정보가
...
RETIRED_LITERALS = [
    ...
    # 🆕 [2026-09-17 O170-E] `SV_AD` 열거에서 뺀 값 2종 — AGENCY 원천 재적재로 실제값에서 사라졌다.
    #   🔴 부정문이라도 라이브 COMMENT·DDL 텍스트에 다시 쓰면 새 부채다(O167 판정식) ⇒ 여기만 둔다.
    ('nf1834a', "SV_AD.AD_GROUP_NM 의 폐기 열거값 — 2026-09-17 원천 재적재 후 실제값 부재(O170-E)"),
    ('하단DA', "SV_AD.AD_TYPE_NM 의 폐기 열거값 — 2026-09-17 원천 재적재 후 실제값 부재(O170-E)"),
]
