...
| `15_dbt파이프라인_작업지시 프롬프트_20260714.md` | 순서 7(SILVER dbt)·8(GOLD) 작업지시 프롬프트 (재현용, 2026-07-20 완료) |
...
| `_archive/` | 구 설계·검토 결과·구버전(20260630 작업계획, `silver_stepbystep_ddl.sql`, `09_SILVER_DDL_20260702.sql`, `10_SILVER_이슈해결 핸드오버.md`, `11_SILVER_블로커_triage` 등) — 현 문서로 대체됨. **참조하지 말 것** |
...
## 읽는 순서 (신규 진입자)
1. 본 문서 → 2. `02_BRONZE-GOLD연결`(정본 매핑 인덱스) → 3. 원천별 실행문서 `03_CRM전용`(완료)·`04_GA4전용`·`05_ERP전용`·`06_AGENCY전용` → 4. `08_테이블DDL`·`09_적재쿼리`(구조·적재 SQL) → (적재 이력) `10_SILVER_RUN_이력_비교` → (GA4 트랙) `07_샤드통합 설계결정` → `22_dbt`(권장)/`23_SP` → (dbt·GOLD 재현) `15_dbt파이프라인`
