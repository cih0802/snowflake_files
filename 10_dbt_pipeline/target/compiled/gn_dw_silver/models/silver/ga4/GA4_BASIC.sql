...
--   ⇒ `003001 … must have MODIFY granted on TABLE GN_DW.SILVER.GA4_BASIC` 로 이 모델이 죽었다(실측).
...
--   🔴 `04_silver_design/08_SILVER_테이블DDL_20260714.sql` 의 GA4_BASIC 선언을 고치면 **여기 CAST 도 같이 고쳐라**(2곳).
--      교차검증 = 이 SELECT 를 임시뷰로 만들어 INFORMATION_SCHEMA 로 GA4_BASIC 과 비교 → **차이 0 이어야 한다**.
