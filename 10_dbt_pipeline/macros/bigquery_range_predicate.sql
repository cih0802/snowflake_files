-- bigquery_range_predicate: GA4 적재 범위 술어의 단일 정의 지점 (2026-08-19 O88 신설)
...
{#
  ...
  왜 바꿨나
    종전에는 `bigquery_dt_ranges` 에 리터럴 구간을 박아 두고 **매 run 그 범위 전량을 재적재**했다.
  ...
  3가지 모드 (우선순위 순)
    ⓐ `bigquery_dt_ranges` var 가 **주어지면** 그 구간 목록을 쓴다 — **백필 전용 수동 오버라이드**.
    ...
    ⓒ 그 밖(평시 일일 run) = **롤링 윈도우** `[오늘 - bigquery_lookback_days, 열린 상한]`.
  ...
  ⚠️ lookback 은 「지연 도착 방어」가 아니다 — 원천은 **지연 도착 종료 후 동결된 데이터**가
     ...
     🔴 run 을 lookback 일수보다 오래 건너뛰면 **창 아래에 구멍이 남는다.** 그 구멍을
        막는 것은 lookback 을 키우는 것이 아니라 `tests/warn_bigquery_load_gap.sql` 이다
  ...
  3가지 렌더러 — **컬럼의 물리 타입이 3종이라서** 필요하다. 창 계산은 `bigquery_load_window()`
  하나이고 렌더러는 그것을 각 타입의 리터럴로만 옮긴다(여기에 창 로직을 다시 쓰지 마라).
    · `bigquery_range_predicate(col)`      DATE            예: `EVENT_DT`  · `b.EVENT_DT`
    · `bigquery_range_predicate_text(col)` VARCHAR(8)      예: `EVENT_DATE` (원천 TEXT YYYYMMDD)
    · `bigquery_range_predicate_sk(col)`   NUMBER(8)       예: `DATE_SK`    (GOLD 팩트 YYYYMMDD)
...
{% macro bigquery_load_window() %}
  {%- set override = var('bigquery_dt_ranges', []) -%}
    ...
    {%- for r in override -%}
      {%- if r | length != 2 -%}
        {{ exceptions.raise_compiler_error(
             "bigquery_dt_ranges 의 각 원소는 [시작일, 종료일] 2개여야 한다. 받은 값: " ~ r) }}
      {%- endif -%}
    {%- endfor -%}
    {{ return(override) }}
  ...
  {#- ⓒ 평시 = 롤링 윈도우 -#}
  {%- else -%}
    {%- set lookback = var('bigquery_lookback_days', 3) | int -%}
    {%- if lookback < 0 -%}
      {{ exceptions.raise_compiler_error(
           "bigquery_lookback_days 는 0 이상이어야 한다. 받은 값: " ~ lookback) }}
    {%- endif -%}
...
{#- DATE 컬럼용 -#}
{% macro bigquery_range_predicate(col='EVENT_DT') %}
  {%- set w = bigquery_load_window() -%}
  {%- if w is none -%}
    TRUE /* bigquery_range_predicate: 최초 run ⇒ 전량 적재 */
  {%- else -%}
  (
    {%- for r in w %}
...
{% macro bigquery_range_predicate_text(col='EVENT_DATE') %}
  {%- set w = bigquery_load_window() -%}
  {%- if w is none -%}
    TRUE /* bigquery_range_predicate_text: 최초 run ⇒ 전량 적재 */
  {%- else -%}
  (
    {%- for r in w %}
...
{% macro bigquery_range_predicate_sk(col='DATE_SK') %}
  {%- set w = bigquery_load_window() -%}
  {%- if w is none -%}
    TRUE /* bigquery_range_predicate_sk: 최초 run ⇒ 전량 적재 */
  {%- else -%}
  (
    {%- for r in w %}
    {% if not loop.first %}OR {% endif %}({{ col }} between {{ r[0] | replace('-','') }} and {{ r[1] | replace('-','') }})
