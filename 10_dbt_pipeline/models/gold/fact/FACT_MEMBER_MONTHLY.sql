...
agg as (
    select
        COALESCE({{ month_key_clamp('TRY_TO_NUMBER(MBRFEE_MT)') }}, {{ month_key_clamp("TRY_TO_NUMBER(TO_CHAR(PAY_DE,'YYYYMM'))") }}, 0) as MONTH_KEY,  -- 회비월(YYYYMM 검증) 우선, 무효/NULL 이면 납입월(검증) 폴백, 둘 다 무효면 0=Unknown월
