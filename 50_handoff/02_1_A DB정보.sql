...
--   본 파일은 2026-08-12 측정본이고, 그 뒤 A 원천이 두 번 바뀌었다:
--     · BRONZE_CRM 45 → 46 (+ TM_CM_MKTNG_UTM) · TM_CM_CMPGN_MNG 34 → 36 컬럼
--     · BRONZE_ERP 1 → 2 (+ EXPENSE_RESOLUTION) · BDGT_ACMSLT_LEDGER 65 → 67 컬럼(순서 변경)
--   ⇒ 이 파일의 A3~A5 GET_DDL 과 A1/A2 집계는 **그 변경을 반영하지 않았다.**
--     예) 1341행에 삭제된 컬럼 `MNYRS_COST_DIV_YN` 이 아직 남아 있다.
--   🟢 현행 구조 정본 = 99_provided_definition/11~13번 (04번 「병합 규칙」 2026-08-29 개정 참조).
--   🟢 이 파일을 다시 정본으로 올리려면 **02번 5·6단계를 A 계정에서 재실행해 갱신**한 뒤
--      04번 병합 규칙의 우선순위를 함께 되돌린다. 🔴 갱신 없이 우선순위만 되돌리지 마라.
--   ⚠️ **내용을 지우지 않고 남겨 둔다** — 「그때 A 가 어떤 모양이었는가」의 유일한 기록이고,
--      2026-08-13 BDGT_ACMSLT_LEDGER 사고(07번 A.1 [원인 B])를 읽는 데 필요하다.
--
-- 본 파일의 성격
--   A 계정에서 실행한 조회 결과를 그대로 보존한 기록이다. B(03번 3.1)·C(07번 A.6)가
--   대조하는 **행수·용량 기준값**이며, 위 경고 범위 밖에서는 그대로 유효하다.
--   🔴 다만 DDL 정합성 판정(07번 A.1 (4))의 기준은 **더 이상 이 파일이 아니다** —
--      99_provided_definition/11~13번이고, 기계 대조는 scripts/handoff_ddl_gate.py 가 한다.
--
-- 섹션 / SECTIONS
...
--
--   현재 공유 대상: BRONZE_CRM(46) · BRONZE_AGENCY(4) · BRONZE_ERP(2)
--                   + SILVER.BIGQUERY_REFINED_DATA(1) + ML.ML_RST_DATA_*(16) = 69 테이블
--                   🔴 이 수치는 2026-08-29 기준이며 **아래 A1/A2 결과와 다르다**(A1/A2 는 08-12 측정).
