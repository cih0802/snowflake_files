...
--    ⇒ 갱신 시 고칠 곳 = 위 「최근 갱신일」 · 아래 「메타데이터 · 갱신일자」 · 「변경 이력」 3곳.
...
--     + [TABLE]  GN_DW.BRONZE_CRM.SND_MEMBER_OPEN_LOG        (8컬럼, 신규 · 메일 오픈 로그)
--     + [TABLE]  GN_DW.BRONZE_CRM.TC_MKTNG_DTL_CD            (16컬럼, 신규 · 마케팅 통합 코드)
--     + [TABLE]  GN_DW.BRONZE_CRM.TM_MM_FDRM_MBER_DT_DTLS    (5컬럼, 신규 · 일시→정기 전환)
--     - [TABLE]  GN_DW.BRONZE_CRM.TM_CM_MKTNG_CMPGN_MNG      (삭제)
--     - [TABLE]  GN_DW.BRONZE_CRM.TM_CM_MKTNG_UTM            (삭제)
--       ⇒ 위 2개는 TC_MKTNG_DTL_CD 로 **통합**되었다. 코드 기준이 늘어날 때마다 테이블이 늘어나는
--          구조를 1개로 합친 것이다. 신규 코드 컬럼도 앞으로 TC_MKTNG_DTL_CD 에 추가된다.
--          🔴 TC_MKTNG_DTL_CD 는 기존 코드 테이블(TC_CMMN_CD / TC_CMMN_DTL_CD)과 **연관성이 없다**
--             — 조인하지 마라. CD_ID 는 현재 3종(C001 마케팅캠페인명 · C002 채널 · U001 UTM)이다.
--          🔴 SILVER / GOLD dbt 모델 중 TM_CM_MKTNG_CMPGN_MNG · TM_CM_MKTNG_UTM 을 참조하는 것은
--             전부 TC_MKTNG_DTL_CD 기준으로 재작성해야 한다(구조가 코드/상세코드 2단으로 바뀐다).
--     + [COLUMN] GN_DW.BRONZE_CRM.TM_CM_CMPGN_MNG : MKTG_CHANNEL (36 → 37 컬럼 · 캠페인채널 정보)
--     🔴 [COLUMN] GN_DW.BRONZE_CRM.SND_MEMBER_LIST : OPEN_DT **삭제** (77 → 76 컬럼)
--        ⇒ 오픈 이력이 SND_MEMBER_OPEN_LOG · SND_MEMBER_MAIL_LINK_LOG 두 테이블로 **확장**되었다.
--        ⇒ 🔴 **위치 기반 CSV 적재에 직결된다.** OPEN_DT 가 마지막 컬럼이었으므로 밀림은 없지만
--           이전 판 CSV 에는 77번째 열이 남아 있다 — 07번 A.1 (4) 대조를 다시 통과시킨 뒤 적재할 것.
--     🟠 [TABLE]  GN_DW.BRONZE_CRM.TM_PM_INSTT_ACNUT         (16컬럼) — 종전 판 **누락분 보완**
...
--       현업 공유 메시지 문안 + 명명 규칙으로 부여했다(병합 규칙 3항 · 현업 확인 대상).
--     🔴 원천 문서 이상 1건 — 99_provided_definition/18_silver_bigquery_refined.sql 의 내용이
--        16_bronze_gsc_ddl.sql 과 **바이트 동일**(BRONZE_GSC DDL)로 교체되었다.
--        ⇒ SILVER.BIGQUERY_REFINED_DATA 의 원천 정의가 현재 워크스페이스에 **없다**
--           (99_provided_definition/*.sql 전건 grep 0건). 06번은 그 대조 근거를 잃었다 —
--           06번 DDL 본문은 근거가 없어 손대지 않았다. 18번 재공유를 현업에 요청할 대상이다.
...
-- DB 소유권 → GN_DW_ADMIN. 이후 스키마/객체를 ADMIN 이 직접 CREATE → 생성 시점부터 ADMIN 소유
...
create or replace TABLE GN_DW.BRONZE_CRM.SND_MEMBER_MAIL_LINK_LOG (
  LOG_SEQ NUMBER(19,0) COMMENT '로그순번',
  REQ_SEQ_NO NUMBER(19,0) COMMENT '발송요청순번 (참조: SND_REQ_MST.SEQ_NO)',
  R_NUM NUMBER(19,0) COMMENT '순번 (참조: SND_MEMBER_LIST.R_NUM)',
  ...
  CLICK_DT TIMESTAMP_NTZ(9) COMMENT '클릭일시',
...
--   한글 코멘트는 컬럼정의서 CSV 미수록분이라 명명 규칙으로 부여했다(현업 확인 대상).
create or replace TABLE GN_DW.BRONZE_CRM.SND_MEMBER_OPEN_LOG (
  LOG_SEQ NUMBER(19,0) COMMENT '로그순번',
  REQ_SEQ_NO NUMBER(19,0) COMMENT '발송요청순번 (참조: SND_REQ_MST.SEQ_NO)',
  R_NUM NUMBER(19,0) COMMENT '순번 (참조: SND_MEMBER_LIST.R_NUM)',
  MBER_NO VARCHAR(50) COMMENT '회원번호',
...
--      코드 기준이 늘 때마다 테이블이 늘어나는 구조를 막기 위한 통합이다(현업 공유 메시지).
--   🔴 기존 코드 테이블(TC_CMMN_CD / TC_CMMN_DTL_CD)과 **연관성이 없다** — 조인 대상이 아니다.
--      컬럼명이 유사하지만 별개 체계다.
--   CD_ID 현재 3종 = C001(마케팅캠페인명) · C002(채널) · U001(UTM). 신규 코드도 여기에 추가된다.
--   한글 코멘트는 컬럼정의서 CSV 미수록분이라 명명 규칙으로 부여했다(현업 확인 대상).
--   ⚠️ 감사컬럼 이름이 다른 CRM 테이블과 다르다 — FRST_RGSTR_DT / LAST_UPDT_ID 다
--      (타 테이블은 FRST_REGIST_DT / LAST_UPDUSR_ID). 원천 그대로 두었다 — 고치면 위치 대조가 깨진다.
create or replace TABLE GN_DW.BRONZE_CRM.TC_MKTNG_DTL_CD (
  CD_ID VARCHAR(50) COMMENT '코드ID (C001 마케팅캠페인명 · C002 채널 · U001 UTM)',
  CD_NM VARCHAR(200) COMMENT '코드명',
  DTL_CD_ID VARCHAR(50) COMMENT '상세코드ID',
  DTL_CD_NM VARCHAR(200) COMMENT '상세코드명',
  SORT_ORDR NUMBER(10,0) COMMENT '정렬순서',
  RM VARCHAR(1000) COMMENT '비고',
  USE_YN VARCHAR(1) COMMENT '사용여부',
  CD_ATRB1 VARCHAR(100) COMMENT '코드속성1',
  ...
  CD_ATRB3 VARCHAR(100) COMMENT '코드속성3',
  FRST_RGSTR_ID VARCHAR(30) COMMENT '최초등록자ID',
  FRST_RGSTR_DT TIMESTAMP_NTZ(9) COMMENT '최초등록일시',
...
--    ⇒ 두 테이블을 참조하는 SILVER/GOLD dbt 모델은 TC_MKTNG_DTL_CD 기준으로 재작성해야 한다.
--    ⇒ 옛 정의가 필요하면 이 파일의 2026-09-01 판(_archive/ 또는 파일명 *_20260730)을 참조.
...
--   한글 코멘트는 컬럼정의서 CSV 미수록분이라 명명 규칙으로 부여했다(현업 확인 대상).
create or replace TABLE GN_DW.BRONZE_CRM.TM_MM_FDRM_MBER_DT_DTLS (
  MBER_NO VARCHAR(10) COMMENT '회원번호 (전환 후 정기회원 · 참조: TM_MM_FDRM_MBER_INFO)',
...
create or replace TABLE GN_DW.BRONZE_CRM.TM_PM_INSTT_ACNUT (
  ACNUT_SER_NO NUMBER(10,0) COMMENT '계좌일련번호',
  CPR_DIV_CD VARCHAR(3) COMMENT '법인구분코드',
  ACNUT_DIV_CD VARCHAR(3) COMMENT '계좌구분코드',
  BANK_CD VARCHAR(10) COMMENT '은행코드',
  ACCTNO VARCHAR(20) COMMENT '계좌번호',
  ACNUT_PRP VARCHAR(100) COMMENT '계좌용도',
  ACNUT_ABRV VARCHAR(30) COMMENT '계좌약칭',
  CHRG_DEPT_CD VARCHAR(10) COMMENT '담당부서코드',
  REGIST_USE_YN VARCHAR(1) COMMENT '등록사용여부',
  USE_YN VARCHAR(1) COMMENT '사용여부',
  FRST_RGSTR_ID VARCHAR(30) COMMENT '최초등록자ID',
  FRST_REGIST_DT TIMESTAMP_NTZ(9) COMMENT '최초등록일시',
  LAST_UPDUSR_ID VARCHAR(30) COMMENT '최종수정자ID',
...
--    빠뜨리고 있던 테이블이다(handoff_ddl_gate 축1 적발). 한글 코멘트는 명명 규칙 부여.
create or replace TABLE GN_DW.BRONZE_CRM.TM_PM_SETLE_CMPNY_ACNT (
  SETLE_CMPNY_ACNT_NO NUMBER(10,0) COMMENT '결제사계좌번호',
  SETLE_CMPNY_ACNT_DIV_CD VARCHAR(3) COMMENT '결제사계좌구분코드',
  SETLE_CMPNY_ACNT_ID VARCHAR(20) COMMENT '결제사계좌ID',
  CPR_DIV_CD VARCHAR(3) COMMENT '법인구분코드',
  ACNT_PRP VARCHAR(100) COMMENT '계좌용도',
  ACNUT_SER_NO NUMBER(10,0) COMMENT '계좌일련번호 (참조: TM_PM_INSTT_ACNUT)',
  RM VARCHAR(1000) COMMENT '비고',
  USE_YN VARCHAR(1) COMMENT '사용여부',
  FRST_RGSTR_ID VARCHAR(30) COMMENT '최초등록자ID',
  FRST_REGIST_DT TIMESTAMP_NTZ(9) COMMENT '최초등록일시',
...
create or replace TABLE GN_DW.BRONZE_AGENCY.SYNC_ERR_INFO (
  ERR_SEQ NUMBER(38,0) DEFAULT GN_DW.BRONZE_AGENCY.SEQ_SYNC_ERR_INFO.NEXTVAL COMMENT '오류 순번(시퀀스 SEQ_SYNC_ERR_INFO)',
