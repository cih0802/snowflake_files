...
> **분류 기준 = 도구 커버리지** — 「도구 1개당 대표 1문항 + 원천 규칙 1문항」이 기본 배치이고,
> 같은 도구 안의 축 나열(연령대+지역 · 후원구분+법인구분 · 부서+월)은 **한 문항으로 병합**한다.
> 🔴 **system/orchestration instruction 에 이미 처방이 박힌 특수 사례는 추천질문에 중복 노출하지 않는다**
...
tools:
  ...
  - tool_spec:
      ...
      description: "서비스 발송 팩트(FSE, 38.47M). 활성 지표: 발송수, 발송 대상 고유 회원수. 차원: 발송일·연·월, 서비스유형(SUBTYPE)·채널, 발송상태 라벨(축A: 발송완료/에러/예약취소)·통신사 도달결과 라벨(축B: 전달 및 실패 사유), 성별·회원상태·회원구분. 문자/메일 발송·채널별·발송결과별 질문에 사용. 발송결과는 두 축이며 섞지 않는다(축A=우리 발송 시스템 상태 · 축B=통신사 도달 결과)."
  - tool_spec:
      type: cortex_analyst_text_to_sql
...
instructions:
  ...
  <!-- 🔴 [2026-08-05 O38] 이 사본은 구버전이다. 배포본(VERSION$4)에는 **"목표는 두 가지"** 절이 추가돼
    ...
    - 대행사 산정 비율(_SRC: CTR_SRC·CVR_SRC·CPC_CLICK_SRC·CPC_CALL_SRC·DEV_UNIT_PRICE_SRC 등)은 행 단위 참고값이며 SUM/AVG 재집계 금지.
  response: |
...
> 🔴 **[2026-08-14 O76] 표 열수 교정** — 아래 두 표는 행이 **5열**인데 헤더가 **4열**이어서
> 렌더러가 마지막 칸(정정 근거)을 **잘라 버리고 있었다**(`doc_line_length_gate` 표 열수 축이 검출 · 10행).
> 값·판정은 **무변경**이고 **헤더에 5번째 열을 추가하고 4열 행에 빈 칸을 채웠다.**
