...
> 잔류 차단: **캠페인/소재별 분해**(CAMPAIGN_SK·AD_CREATIVE_SK 전건 0, Q10) + **공8 방송 개발단가**(분모 커버리지 5.2% → 41% 왜곡).
...
| 공8 | GA 개발단가(원) | AD_COST ÷ DVLP_CNT(방송) | FACT_AD_COMBINED | **미노출** | N | 방송 | **P2** ❌ | 배포 취소(2026-07-28). 분모 `DVLP_CNT` 커버리지 **5.2%**(1,982/37,886)인데 분자 광고비는 100% → **41% 과대계상**(223,466 vs 정합 157,969). 정합해도 방송 광고비의 29%만 반영되어 지표명이 커버리지를 오인시킴 → SV 미노출, 적재 확대 후 재검토. 또한 원 정의의 "SRC=GA4"와 실현체(방송 DVLP_CNT)가 불일치(§6-I) |
...
**SV_AD 소계 = 4** (공7·9·10 = **P1 승격** / 공8 = **P2 잔류**(커버리지 결함) — 2026-07-28)
...
| Phase | 수 | 구성 |
|---|---:|---|
| **P1** (즉시 배포) | **72** | MONTHLY 39(공81 제외) + EVENT 8 + SERVICE 22(신32·33 제외) + PARTICIPATION 0(base 전용) + **AD 3(공7·9·10, 2026-07-28 승격)** |
| **P2** (입고 후) | **9** | 공81 · 신32 · 신33 (GA cross) + **공8(방송 개발단가, 커버리지 결함)** + SV_GA 2 + SV_BUDGET 3 |
> 합 81. P2 트리거: G-5(GA4 전기간)·E-6(FTG_B)·E-1/E-4(ERP·AGENCY 비용)·O3(ROI 비용배분)·**방송 DVLP_CNT 적재 확대(공8)**.
> **2026-07-28 개정**: SV_AD 3건 P2→P1(69→72 / 12→9). 공8은 분모 커버리지 5.2% 결함으로 P2 잔류. **Q10(캠페인 연결키)은 P2 트리거에서 제외** — 공7·9·10은 Q10 없이 산출 가능하고, Q10은 이제 "캠페인/소재별 **분해**"(신#9·DIM_AD_CREATIVE CS)만 차단한다.
...
| 보류 | 4 | 신9 · 신10 · 신11 · **공8**(방송 개발단가, 분모 커버리지 5.2%) |
...
- **§6-I** *(신규 2026-07-28)* 공8 "GA 개발단가"가 실제로는 **방송(VIDEO/REBROADCAST) `DVLP_CNT`** 기반으로 실현됨 — 원 정의의 "SRC=GA4"와 불일치. 게다가 분모 커버리지 5.2%로 **41% 왜곡**되어 SV 미노출 결정. 지표명·정의·적재범위 3건 모두 재확인 필요. *(잔여)*
