...
CREATE OR ALTER SEMANTIC VIEW GN_DW.SERVING.SV_MEMBER_MONTHLY
  ...
  METRICS (
    ...
    fmm.TOTAL_PAID_ALL   AS SUM(fmm.PAID_FEE)
      WITH SYNONYMS ('총수납액', '수납액', '납입총액') COMMENT = '납입 **총액**(원) = 회비 + 기부금. F(가산). 🔴**「납입회비」가 아니다**(O40) — 기저 `PAID_FEE` 는 회비(TM_PM_MBRFEE_ACMSLT)와 기부금(TM_PM_DNTN_DTLS)의 `PAY_AMT` 를 **모두** 합한 값이다. 🔴**납부율의 분자로 쓰지 말 것** — 기부금은 원천에 청구(`RQEST_AMT`) 컬럼이 아예 없어(전건 NULL) 분모 `TOTAL_BILLED_AMT` 에 구조적으로 들어갈 수 없다. 분자에만 더해지면 납부율이 과대해진다(과대 규모 실측치는 이슈원장 §O40 참조). 회비만의 납입액은 **`TOTAL_PAID_FEE_BILLABLE`** 이다. 🟢「총수납액(회비+기부금)」을 명시적으로 물을 때 이 metric 을 쓰고 그때도 기부금 포함임을 밝힌다. 🔴[O56-C EXPO-2] 종전 이름은 `TOTAL_PAID_FEE` 였다 — `_FEE` 가 「회비」로 오해되므로 형제 SV `SV_MEMBER_FEE.TOTAL_PAID_ALL` 과 **같은 이름으로 통일**했다(정의·값 불변).',
    ...
    fmm.UNPAID_RATIO AS SUM(fmm.UNPAID_BILLED_AMT) / NULLIF(SUM(fmm.BILLED_AMT), 0) * 100
      ...
      COMMENT = '**미납비중(%) 정본** = 미납 청구액 ÷ 회비 청구액 ×100. 비율(N, 재집계 금지). 단위는 **퍼센트**다 — 값을 다시 ×100 하지 말 것. ⚠️`100 − 납부율` 과 정확히 같지 않다 — 부분납입 행이 있어 두 값은 서로 보완적이다(연도별 실측치는 이슈원장 §O40 참조). 🔴[O56 EXPO-1] 종전 이름은 `UNPAID_RATIO_DEC3` 였고 이 이름은 차감식(청구−납입)이 쓰고 있었다 — 그 차감식은 **음수가 나오던 폐기 지표**이며 제거했고 정본이 이 이름을 **승격**했다. 실측치는 이슈원장 §O55·§O56 참조. 형제 팩트 `SV_MEMBER_FEE.UNPAID_RATIO` 와 **같은 이름·같은 식·같은 단위**다(GATE-D).',
  ...
  COMMENT = 'Phase-1 회원 월별 실적 SV(base FMM). [원천 요약] 원천시스템=CRM(eCRM) · BRONZE=GN_DW.BRONZE_CRM(회비 TM_PM_MBRFEE_ACMSLT·기부 TM_PM_DNTN_DTLS·개발 TM_MM_FDRM_MBER_DVLP_AMT·중단 TM_MM_FDRM_MBER_SPNSR_DSCNTC·증감 TM_MM_FDRM_MBER_IRSD·회원 TM_MM_FDRM_MBER_INFO) → SILVER(CRM_*) → GOLD(FMM). 테이블별 상세 원천은 각 테이블 COMMENT의 [원천] 절 참조. 활성: 납입/청구 총액·납부율(공64)·미납회원 감소율(공80)·개발/중단 총건·총미납금액·미납비중·평균납입회비. 시간=전체가능. 회비 지표는 HAS_BILLING=TRUE 전제 권장. 회원상태/성별/구분/지역/연령대는 현재 스냅샷 기준(과거월도 현재값). [2026-08-04 O33] 지역·연령대 활성화(O27 as-of 배선 완료 — 종전 "적재 대기" 표기는 거짓이었다). 🔴[O34 확정] 지역·연령대는 **약정(개발) 시점 스냅샷**이며 **현재 값이 아니다** — 차원명에 _AT_PLEDGE 를 붙였다. 현재 연령·현재 거주지는 **산출 불가**(BRONZE 에 생년월일·현주소 축이 없다). ''10대 미만'' 최다는 실제이며 **편지쓰기대회 계열 아동 모집 캠페인** 때문이다(오류가 아니다) — 질문받으면 그 이벤트 성격으로 설명할 것. 🔴단 이 SV 에는 캠페인 축이 없다 — **연령대 × 캠페인 교차는 SV_MEMBER_EVENT**(사건시점 축), **캠페인별 중단률·획득시점 회원특성은 SV_MEMBER_COHORT**(회원 grain)에서 한다. [2026-08-05 O37] 🔴 종전 이 자리의 「교차 확인 불가」 계열 표현은 회수됐다 — 두 SV 에 축이 실재하므로 "불가"로 답하지 말고 해당 SV 로 라우팅한다(P61). [O33] 미납사유 활성화 — 단 코드그룹 5종이 섞여 있어
UNPAID_REASON_TYPE 동반 필수이고 미납 지표와 함께 쓸 때만 의미가 있다. [2026-08-06 O45] 🔴 **「납입방식/후원사업별 분해」는 이제 산출된다 — 종전 "비활성(적재 대기)" 서술은 거짓이므로 회수했다**(P61). 단 **이 SV 가 아니라 `SV_MEMBER_FEE`** 로 답한다. 🔴🔴 **[2026-08-29 O118-B 정정] 이유가 두 축에 서로 다르다 — 한 문장으로 묶으면 한쪽이 거짓이 된다.** ① **납입방식** = `FMM.PAYMENT_SK` 가 **전건 0**(실측) ⇒ 이 축은 **정말로 적재 대기**다. ② **후원사업** = `FMM.SPONSORSHIP_SK` 는 **대부분 채워져 있다**(실측) ⇒ 적재 문제가 아니라 **다중 후원 접힘** 문제다: 회원 다수가 후원사업을 복수 보유하므로 회원×월 1행의 단일 SK 로 후원사업별 금액을 분해하면 **한 사업에 전액이 잘못 귀속**된다(fan-out 이 아니라 **귀속 오류**다 — m:1 조인이므로 행은 늘지 않는다). ⇒ 후원사업별 회비는 `SV_MEMBER_FEE`(회비 grain) · 후원사업별 활동회원은 `SV_MEMBER_SPONSOR_BIZ`(약정 grain). ~~이유가 중요하다: 이 팩트(FMM)는 회원×월 정확히 1행 grain 이라 후원사업·납입방식을 붙이면 grain 이 깨진다 — 「적재 대기」가 아니라 grain 문제였다.~~ 🔴 그 취소선 문안은 두 축을 묶어 ①에 대해 틀렸다(규모는 04 §0.9-C 참조). 🔴🔴 **`SV_MEMBER_FEE` 와 이 SV 의 회비 measure 를 한 표에 함께 두지 말 것 — 이중계상이다.** 두 SV 는 같은 원천(`SILVER.CRM_PAYMENT_BILLING`)을 다른 grain 으로 담은 **형제 팩트**이며, 실측(2026-08-06) 조인 시 청구액이 **과대계상**된다(전·후 금액과 과대 비율은 이슈원장 §O45 참조). ⇒ 회원-월 요약·납부율·미납비중이면 **이 SV**, 후원사업·납입방식·회비구분·납입일 분해면 **SV_MEMBER_FEE** 중 하나만 앵커로 쓴다. 비활성(적재 대기): 활동/누계/미납 카운트 비율, 신규기존 분해. 🔴🔴 **[2026-08-13 O67 발행] 보고서 필드 조립 판정 4종 중 이 SV 소관분**(정본 = 산출물 `09_보고서필드_조립가능성` · 판정 라벨은 O47 신설): ① **집계필요** — 월간 중단보고의 「중단(명)」은 **불가가 아니다.** 사건 팩트(`SV_MEMBER_EVENT`)를 **회원·월로 먼저 집계한 뒤** 이 SV 의 grain 에 맞춰 답한다(사전집계 없이 조인하면 이중계상이다). 🔴 「명」은 플래그 합이 아니라 **회원 distinct** 이며 **주차 합계를 월로 더하지 않는다** — 같은 회원이 여러 주에 중단하므로 과대가 된다(규모는 이슈원장 §O56 참조). ② **배분규칙필요** — 서비스 계열 보고 섹션들이 이 SV 의 중단·개발
measure 를 **발송 grain 으로** 요구한다. 배분 규칙이 없으므로 **SQL 을 만들지 않고 사유를 답한다**(모델링이 아니라 업무 판단 · O47). ③ **형제팩트중복** — 연간분석(회비)의 「납입(원)」이 이 판정이며 위 `SV_MEMBER_FEE` 이중계상 가드와 **같은 사안**이다(새 제약이 아니라 **라벨명을 명시**한 것이다). ④ **앵커_경합** — 개발실적보고 주간 섹션은 이 SV 가 경합 팩트 중 하나다 ⇒ **한 표로 합치지 말고 팩트별로 나눠 조회한 뒤 공통축(연·월)에서 병기**한다. 🔴 **경합에서 한쪽만 골라 답하면 조용한 오답**이다. 🔴 **캠페인은 이 목록에서 제외**한다(2026-08-05 O37) — 이 SV 의 `FMM.CAMPAIGN_SK` 가 센티넬 단일값이라 여기서는 분해되지 않지만, **캠페인 분해 자체가 불가한 것은 아니다**: 개발/중단 건수는 `SV_MEMBER_EVENT`, 캠페인별 중단률·획득 코호트는 `SV_MEMBER_COHORT` 로 답한다. 🔴 이 목록은 축을 활성화할 때마다 회수 대상이다(P61 — 부정형 서술이 남으면 Agent 가 "불가"로 답한다).
🆕[2026-08-21 신설 · 2026-08-29 O118-B 정정] **캠페인별/후원사업별 활동회원 수**는 이 SV·SV_MEMBER_EVENT·SV_MEMBER_COHORT
어디도 아니라 **SV_MEMBER_SPONSOR_BIZ**(약정 grain)로 답한다 — 이 SV 의 CAMPAIGN_SK 는
O8(회원grain 다중귀속 미결)로 전건 센티넬이라 분해 불가이지만, SV_MEMBER_SPONSOR_BIZ 는
약정(SPNSR_BSNS_NO) grain 에서 캠페인이 거의 1:1(다중은 극소수)이라 대표캠페인 규칙으로 분해 가능하다.
🔴 캠페인·후원사업별 합계는 이 SV 의 ACTIVE_MEMBERS(전체 활동회원수)보다 클 수 있다(다중후원
정상현상) — 두 SV 의 총계를 같게 기대하지 말 것. '
  AI_SQL_GENERATION '적용 조건: 질문에 기간(연/월)과 그룹(회원구분·성별·회원상태 등)이 모두 없을 때만. 이 경우 전체 기간 풀스캔을 피해 데이터에 실제 존재하는 최신 연월(MAX(연월)) 기준 직전 12개월로 한정하고(기준월은 CURRENT_DATE가 아니라 데이터 최신월 — 미래연월 데이터도 최신월로 인정), GROUP BY ROLLUP((연,월))로 월별 행 + 전체 총계 행을 함께 반환해 월별 추이와 총계를 동시에 제공한다. 납부율·미납비중 등 비율은 총계 행에서 SUM 기반으로 정확히 산출한다. 사용자가 기간/그룹을 지정하거나 합계·총액만 원하면 그 요청을 우선한다. ⚠**[O40 해소 · 2026-08-10 O56 EXPO-1 갱신] 납부율·미납금액은 정본 metric 을 쓴다.** 「납부율」=**`PAYMENT_RATE_FEE`** · 「납입회비」=**`TOTAL_PAID_FEE_BILLABLE`** · 「미납금액」=**`TOTAL_UNPAID_AMT`** · 「미납비중」=**`UNPAID_RATIO`**. 🔴🔴 **[O56 EXPO-1 중요] `TOTAL_UNPAID_AMT`·`UNPAID_RATIO` 는 이제 정본이다** — 종전에 이 두 이름을 쓰던 차감식(청구−납입 · **과소이며 전 기간 음수**)은 **SV 에서 제거됐고**, 정본(종전 `*_DEC3`)이 이 이름을 승격했다. 따라서 「`UNPAID_RATIO` 는 차감식이니 쓰지 말라」는 종전 지침은 **폐기**다. 이 두 metric 은 형제 팩트 `SV_MEMBER_FEE` 의 동명 metric 과 **같은 식·같은 단위**다(단 두 SV 의 measure 를 합산하지는 말 것 — 아래 이중계상 가드). 🔴🔴 **[O56-C EXPO-2 중요] `PAYMENT_RATE` 는 SV 에서 제거됐다** — 분자에 기부금이 섞여 **100% 를 넘던** 지표이며 더 이상 존재하지 않는다(실측치는 이슈원장 §O56-C). 「납부율」은
**`PAYMENT_RATE_FEE` 가 유일한 정본**이다. 🔴 **`TOTAL_PAID_FEE` 는 `TOTAL_PAID_ALL` 로 개명됐다**(정의·값 불변 = 회비+기부금 총수납액 · 형제 SV `SV_MEMBER_FEE.TOTAL_PAID_ALL` 과 동일 이름). `TOTAL_PAID_FEE` 라는 이름은 이제 존재하지 않으므로 그 이름으로 쿼리하지 말 것. 🟢 `TOTAL_PAID_ALL` 은 **결함 지표가 아니다** — 「총수납액(회비+기부금)」을 명시적으로 물을 때 쓰고 그때 기부금 포함임을 밝힌다. 🔴 단 **납부율의 분자로는 쓰지 않는다**(기부금은 BRONZE 원천 `TM_PM_DNTN_DTLS` 에 청구 컬럼이 아예 없어 분모에 들어갈 수 없다 — 2026-08-10 BRONZE 실측). (2) `HAS_BILLING=TRUE` 는 「회비만」 스코프가 **아니다** — 이 필터를 걸었다는 이유로 「회비 기준」이라고 서술하지 않는다. 회비 기준은 metric 선택으로 결정된다. ⚠**「마감·확정」이라고 단정하지 않는다.** 이 팩트는 적재 시점 **스냅샷**이다 — 지난 연도의 미납 청구가 이후에 납입되면 값이 바뀐다(2025년분 회비가 2026-07-01 까지 계속 납입된 것이 실측으로 확인됐다). 또 회비월 데이터는 **미래월까지 존재**한다. 따라서 특정 연도 실적을 답할 때 「이미 마감된 실측치」,「확정치」 같은 표현을 쓰지 말고 **「조회 시점 적재 기준」임을 명시**한다. 🔴 **[O45 이중계상 가드] 후원사업·납입방식·회비구분·납입일별 회비 분해 요청은 이 SV 로 답하지 않는다** — `SV_MEMBER_FEE` 로 라우팅한다. 이 SV 는 회원×월 1행 grain 이라 그 축들이 없다. 그리고 **두 SV 의 회비 measure 를 같은 표·같은 답에 합산하지 않는다**: 같은 원천을 다른 grain 으로 담은 형제 팩트라 합치면 청구·납입액이 과대계상된다(실측 규모는 이슈원장 §O45 참조). 한 답변에 둘 다
필요하면 **표를 분리하고 각각의 grain 을 명시**한다. 🔴🔴 **[2026-08-13 O67] 판정 4종 라우팅**: 「중단(명)」류(**집계필요**)는 사건 팩트를 회원·월로 사전집계한 값을 쓰고 **주차 합계를 월로 더하지 않는다** · 발송 grain 으로 중단·개발 measure 를 요구하는 요청(**배분규칙필요**)은 **SQL 을 생성하지 말고 배분 규칙이 없다는 사유를 답한다** · 회비 「납입(원)」을 형제 팩트와 나란히 요구하면(**형제팩트중복**) 표를 분리한다 · 개발실적보고 주간 섹션(**앵커_경합**)은 경합 팩트별로 각각 조회해 **연·월 축에서 병기**하고 한쪽만 골라 답하지 않는다.';
