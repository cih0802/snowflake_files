...
--   | 05_7_SV_DDL_AD.sql                    | SV_AD                    | GRANT 3 + 스모크 (2026-08-10 O54: helper 뷰 생성 블록 제거) |
--   | 05_8_SV_DDL_DEV_ACHIEVEMENT.sql       | SV_DEV_ACHIEVEMENT       | GRANT 3 + 스모크 (2026-08-05 O38 신설·배포완료) |
--   | 05_9_SV_DDL_MEMBER_FEE.sql            | SV_MEMBER_FEE            | GRANT 3 + 스모크 (2026-08-06 O45 신설·배포완료) |
--   | 05_10_SV_DDL_MEMBER_SPONSOR_BIZ.sql   | SV_MEMBER_SPONSOR_BIZ    | 🆕 **[2026-08-29 O118 등재]** 이 행이 **누락돼 있었다** |
--
--   🔴🔴 **[2026-08-29 O118] 이 파일의 「SV 9종」 기재는 전부 stale 이다 — 라이브는 10종이다.**
--      `05_10_SV_DDL_MEMBER_SPONSOR_BIZ.sql` 이 실재하고 그 SV 도 배포돼 있는데 **이 인덱스에만 없었다.**
--      ⇒ 인덱스가 정본을 주장하면서 대상 1개를 빠뜨렸다(P62-B 계열 — 정본이 불완전하면 그것을 읽은
--        모든 판정이 1개씩 적게 센다). 🟢 실측·경위 = `04_SV_설계.md` **§0.9**.
--      🔴 **아래 판정 주석의 「9행」도 stale 이다** — 개수를 고쳐 적지 말고 **쿼리 결과를 정본으로** 삼아라
--        (이 파일 §8-10 스스로 「개수를 주석에 박지 않는다」고 적어 두고 바로 다음 줄에 박아 뒀다 · `R3-9 ㉦`).
--
...
--   ⛔ [2026-08-10 O54] **회수** — 종전 이 자리의 「`05_7` 의 helper 뷰 `FACT_AD_COMBINED` 가 `SV_AD` 의
--      단일 base 이며 유일한 교차 의존이다」는 서술은 **거짓이 됐다**. `SV_AD` 의 base 는
--      `GOLD.WIDE_AD_COMBINED`(dbt 소유 뷰)로 재배선됐고 `05_7` 의 helper 생성 블록은 제거됐다
--      (실측: SEMANTIC_TABLES 전건 BASE_TABLE_SCHEMA='GOLD'). ⇒ 교차 의존은 이제 **0** 이다.
...
--
-- ▶ 선행 조건 → 🔴 **정본은 아래 §공통 규약 「선행 조건」 절이다**(2026-08-10 O55 DUP-1 단일화).
--   요약: `dbt build` **하나**가 유일한 선행 조건이고 실행 역할은 `GN_DW_ADMIN` 이다.
--   ⛔ 종전 이 자리에 있던 ②「`08_After_Deploy_DBT.sql` §G 의 SERVING helper 뷰 선행 필수」와
--      그에 달린 O36 경고는 **폐지됐다** — 경위·근거는 §공통 규약에 한 번만 적는다.
--
-- ▶ 실행 순서 (신규 계정 재현) — 🔴 정본 = `02_GN_DW_building/06_RUNBOOK.md` §11.2-C
--   07_ENVIRONMENT_RBAC_setup.sql → GOLD/SILVER DDL → dbt build
--   → **05_1 ~ 05_9 (전부 또는 필요분)**
--      🔴 **[2026-08-29 O118] 범위 정정 = `05_1 ~ 05_10`** (`05_10` 이 이 목록에서도 빠져 있었다).
--   → 09_1_AGENT_생성.sql(껍데기) → 09_2_AGENT_버전업.sql(스펙 본문)
--   ⛔ [2026-08-10 O54·O55] 종전 이 순서에 있던 `08_After_Deploy_DBT.sql §G(helper 뷰)` 단계는 **제거**했다
--      — SV 가 helper 를 참조하지 않으므로 선행 단계가 아니고, O55 에서 `§G` 절 자체가 삭제됐다.
...
--
-- ▶ 파일 규약 (정본 · 2026-08-10 O55 DUP-1 단일화) 🔴
--   종전에는 아래 5개 항목이 `05_1`~`05_9` **9개 파일에 그대로 복제**돼 있었다. 그래서 사실 1건이
--   바뀔 때마다 같은 문장이 9곳에서 거짓이 됐다(O54 재배선 때 8파일 개별 교정 · P140).
--   ⇒ **여기가 정본이고 각 파일은 이 절을 가리킨다.** 같은 성격의 변경은 이 파일만 고친다.
--   1) **독립 실행** — 각 `05_N` 파일은 `USE ROLE`/`USE WAREHOUSE`/`USE SCHEMA` + SV 정의 + 자기 GRANT +
--      자기 스모크를 모두 포함한다. 필요한 파일만 단독 실행하면 된다.
--   2) **파일 간 실행 순서 의존이 없다** — 순서 문서가 stale 되는 실패 경로(P62)가 원천적으로 없다.
--   3) **최초 세팅과 변경 반영이 같은 파일이다** — 통째로 재실행한다. 별도 update 스크립트를 만들지 않는다.
--   4) **본문 DDL 은 `CREATE OR ALTER SEMANTIC VIEW` 다** — GRANT 와 **소유권**이 함께 보존된다(실측).
--      각 파일의 GRANT 절은 멱등 재확인용이다.
--      ⛔ `CREATE OR REPLACE` 로 되돌리지 말 것 — GRANT 파괴(P125 실사고)에 더해 **owner 를 실행 역할로
--      리셋**한다(2026-08-10 OWN-1: `SV_MEMBER_COHORT`·`SV_MEMBER_FEE` 가 `ACCOUNTADMIN` 으로 드리프트했다.
--      GRANT 절은 소유권을 복구하지 않는다 — 복구는 아래 §8-11).
--   5) **분할 이력** — 종전 SV 6종이 단일 파일 `05_SV_DDL.sql`(708행 · 현
--      `_archive/05_SV_DDL_ORIGINAL_BACKUP_20260805.sql`)에 있었다. SV 하나를 고칠 때마다 파일 전체를
--      재작성해야 해서 **손대지 않은 SV 의 COMMENT 를 훼손할 경로**였고(O27→O30 · P58), 신규 SV 추가도
--      기존 파일 편집을 강제했다 → SV 단위로 분할(O37). 본 파일은 인덱스 + 전체 배포 검증으로 전환됐다.
--
-- ▶ 선행 조건 (정본 · 모든 05_* 파일 공통) 🔴
--   ① **GOLD 적재 완료(`dbt build`) — 이것이 유일한 선행 조건이다.**
...
--     (`DIM_MONTH`·`DIM_MEMBER_CURRENT`) 선행 필수」는 폐지됐다.** SV 9종 base 가 전건 `GOLD` 로
...
--   `04_SV_설계.md` §0.3 가산성 · §0.8 DEC-34(base 선택) · §1~6 SV구조 · **§6.9 구조적 제약**
--   `03_SV_metric_배속.md` 지표별 분자/분모 직역 · **§8.5 미해결 + §8.5.1 근거 쿼리**
--   `01_SV-Agent 작업계획.md` §3 3단계 · 원칙10(fan-out) · R1·R5 가산성 · 원칙6 한글 synonyms
--   `20_issue/00_INDEX_이슈원장.md` 이슈·교훈 정본(작업 완료의 정본은 원장이다 · P107)
-- ============================================================================


/* =====================================================================================
   §전체 배포 검증 — SV 9종을 아우르는 검사 (개별 SV 스모크는 각 05_* 파일에 있다)
...
--   판정: 9행(SV 종수와 일치). 각 SV 의 metric·dimension 수가 해당 05_* 파일 DDL 의
--         METRICS/DIMENSIONS 절 항목 수와 일치.
--   ⚠ 개수를 주석에 박지 않는다 — DDL 을 세는 것이 정본이다(04 §6.3 이 "dim 20" 으로 틀렸던 사례).

-- (8-10b) [2026-08-10 O54 신설] base 스키마 게이트 — SV 가 SERVING helper 를 참조하지 않는지 검사
--   🔴 정확한 뷰명은 `SEMANTIC_TABLES` 다(`SEMANTIC_VIEW_TABLES` 는 존재하지 않는다) ·
--      스키마 컬럼명은 `BASE_TABLE_SCHEMA` 다(`BASE_TABLE_SCHEMA_NAME` 이 아니다). 실측 확인.
...
GROUP BY 1 ORDER BY 1;
--   판정: 9행 · base_schemas 전건 'GOLD' 단일. 'SERVING' 이 하나라도 보이면 재배선 누락이다.
...
--     🔴 `COPY CURRENT GRANTS` 를 빼면 소비 3역할 GRANT 가 사라진다.
--     🔴 소유권을 옮긴 **뒤에** 해당 파일을 `CREATE OR ALTER` 로 바꿔야 한다 — 순서를 뒤집으면
--        GN_DW_ADMIN 이 ACCOUNTADMIN 소유 객체를 ALTER 할 수 없어 실패한다.
...
--   현재: SV 정의·GRANT·개별 스모크는 `05_1`~`05_9` 로 이관. 이 파일은 인덱스·공통규약·전체검증.
...
--        인덱스·공통규약·전체검증 → `05_0_SV_DDL.sql` / SV 정의·배포 → `05_1`~`05_9_SV_DDL_*.sql`.
