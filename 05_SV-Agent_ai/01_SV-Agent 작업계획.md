...
> 🔴🔴 **[2026-08-14 O76] 아래 「마케팅 Agent Phase-2 유예」 사유는 절반이 stale 이다 — 설계 정본이 신설됐다.**
> 유예 사유 중 「`SV_AD` 미배포·FAD 스캐폴드」는 **2026-07-28 배포 + O45 마케팅캠페인 축 활성으로 해소**됐고,
> 마케팅 요건 5섹션을 답할 SV 는 **이미 6종이 라이브에 실재**한다(`SV_AD`·`SV_DEV_ACHIEVEMENT`·`SV_BUDGET`·
> `SV_MEMBER_EVENT`·`SV_MEMBER_COHORT`·`SV_MEMBER_FEE` · 2026-08-14 `SHOW SEMANTIC VIEWS` 실측 9종).
> 잔여 블로커는 **GA 계열 3필드(`G-5`) + 소재·캠페인유형 축(`Q10`)** 으로 좁혀졌고 그 둘은 Agent 개설을 막지 않는다.
...
| 5 | `08_AGENT_spec.md` (CREATE AGENT) | ✅ 스펙완료 (2026-07-22) — 2 Agent(AGENT_MEMBER 4SV·AGENT_EXECUTIVE 예산+MONTHLY/SERVICE) 스펙 작성. model=auto·WH=GN_DW_ANALYTICS_WH·orchestration 라우팅·custom instruction 6(06§4)·sample_questions·평가셋 매핑(07). workspace YAML=`cortex_project/agents/AGENT_MEMBER/agent_spec.yaml`·`agents/AGENT_EXECUTIVE/agent_spec.yaml`(2026-08-05 O38 경로 정정 — 구 루트 yaml 은 `_archive/` 이관). Cortex Search=Phase-2 유예. **배포(save/publish)·CoWork ADD AGENT·USAGE grant는 사용자(GN_DW_ADMIN)** — §4 절차 |
...
| **9** | **SV_AD 확장 (파이프라인 확장 반영)** | 🔄 **문서완료·배포 부분** (2026-07-28) — BRONZE→GOLD AGENCY 광고 확장 검토 후 **SV_AD 신설**. ① `SERVING.FACT_AD_COMBINED` helper 뷰(FAP+FAD+FAB 1:1 pre-join) + `SV_AD`(dim 20·metric 16) **CREATE 완료**, fan-out 스모크 **PASS**(광고비 51,439,193,917.80 SV=FACT 일치), CTR/개발단가 산출 확인. ② 문서 정합: `05`(§6 DDL)·`09`(analyst_ad)·`04`(§6 전면 재작성)·`03`(§5 P2→P1·§8.3/8.4 집계·§6-G/H/I)·`08`(§0/§1/§3.2)·`01`(본표). ③ **잔여 사용자 실행**: `13_SV_AD_배포_추가작업.sql` (소유권 이전 2건 + AGENT_EXECUTIVE 재배포 + USAGE 재부여 + SI 재확인) |
| **9-R** | **문서 리팩터링 — 13 분해 + 신규계정 재현 실증** | ✅ **완료** (2026-07-31, mq60369) — ① **실행순서 모순 해소**: 구 09 헤더는 "05 → 13 → 09", 구 13 §0은 "05 → 09 → 13"으로 **서로 반대**를 적고 있었다. 13에 객체 생성 DDL이 전무(소유권 보정·GRANT·검증·스모크뿐)하고 09가 필요로 하는 SV_AD는 05가 만들므로 **후자가 참** → 09 헤더 정정. ② **13 분해**: §4-1~3·§5 → `05 §8-4~8-13` / §2·§4-4 → `09 [5]` / §6 → `03 §8.5.1` / §7 → `04 §6.9` / §1 폐기(cs94293 1회성). 13은 포인터 스텁, 원본은 `_archive/13_..._원본_20260728.sql`. ③ **05 스모크 주석 해제**(기존엔 `/* */` 안에 있어 실행 불가였다) + **불변식 판정으로 재작성**. 절 번호는 §7 GRANT · §8 스모크·배포검증으로 정리. ④ **결함 4건 교정**: 구 05 §7-5 `FILTER fap.…` 컴파일 실패(실측) → 바깥 WHERE 교정 / `ORDER BY … DESC` NULLS FIRST로 top-N 오염 → `NULLS LAST` / "dim 20"→**19**(INFORMATION_SCHEMA 실측, 04 §6.3도 정정) / 계정별 절대 기대값 → 불변식. ⑤ **재현 검증**: 09 [1-ALT]+[3]+[4] 실행 후 Agent 2행(owner=GN_DW_ADMIN)·SI 2행·GRANTS 각 4행 · 스모크 7건 구조 불변식 전항 통과. ⑥ **05 슬림화·COMMENT 규약 신설**: 688→632행. SV COMMENT의 절대값(행수·합계·커버리지%·건수·적재기간) **전량 제거** — Agent가 COMMENT를 원천 답변 근거로 인용하므로 박아둔 수치는 적재량이 바뀌는 순간 거짓이 된다. 05 헤더에 **COMMENT 작성 규약 4개항**(수치 금지 · `[원천]`은 `시스템=… · BRONZE=DB.스키마.테이블(핵심컬럼) · SILVER=…` 이름만 · 코드값 열거 · 원천 변경 시 계보문서 동기화) 신설. 이력·판정경위·수치는 04 §6.9 / 03 §8.5·§8.5.1로 위임하고 05는 DDL+GRANT+검증만 남겼다. **05는 최초·재배포 공용 단일 파일**임을 헤더에 명시(별도 update 스크립트 없음). ⑦ ✅ **재배포까지 완료** — 배포본 4계층 COMMENT 전수 스캔 결과 수치 잔존 0건(#10 참조). ⚠ **잔여**: `09_AGENT_spec_구현.sql` 의 [1-ALT]·[4-B] YAML **배포용 사본**은 아직 구 수치를 담고 있어 정본(`cortex_project/*.agent.yaml`)과 drift 상태다 — 09 8행이 "정본 변경 시 반드시 함께 갱신"이라 규정하므로 09 사본 동기화 필요(P23 재발 방지) |
| **10** | **BRONZE 원천(provenance) 노출** | ✅ **배포·검증 완료** (2026-07-31, mq60369) — 원칙 13 신설. ① `05_SV_DDL.sql`: SV 6종의 테이블 COMMENT + SV 최상위 COMMENT에 `[원천]`/`[원천 요약]` 절 기입(시스템·BRONZE DB.스키마.테이블(핵심컬럼)·SILVER) + 헤더에 **COMMENT 작성 규약** 블록. ② 2 Agent instructions 보강 — system(원천 응답 규칙·Agent별 원천 지도·컬럼단위는 정본 안내)·response(원천 다르면 표 분리+각주)·orchestration(출처만 묻는 질문은 도구 미호출)·tool description(analyst_budget=ERP·analyst_ad=AGENCY+GA4)·sample_question 각 1건. ③ **배포 실행 완료**: 05를 `EXECUTE IMMEDIATE FROM 'snow://workspace/…/05_SV_DDL.sql'` 로 GN_DW_ADMIN 실행(파일 내 `USE ROLE` 가 적용되어 소유권 정합) → SV 6종 owner=GN_DW_ADMIN · GRANT 각 7행 복원 · helper뷰 4행 / `cortex_agent_deploy` 로 2 Agent VERSION$2 발행(VERSION$1·grant·SI 보존). ④ **검증 완료**: SV 최상위·논리테이블·dimension·metric **4계층 COMMENT 정규식 전수 스캔 → 수치 잔존 0건** · fan-out·수직분할 불변식 통과. ⑤ **수치 대신 행동 규칙**: 경계 연월·커버리지·규모를 기억해 말하지 말고 도구로 조회해 확인 후 조회 시점 기준임을 명시하도록 지시. ⚠ 잔여: CoWork UI에서 "이 데이터 원천은?" 수동 질의(트라이얼은 DATA_AGENT_RUN 차단) |
| **10-a** | **원천노출 사후 검토에서 나온 미결(2026-07-29)** | 🔄 **①②해소 / ③④대기** — ① ✅ **해소(2026-07-31)**: `cortex_project/AGENT_MEMBER.agent.yaml`·`AGENT_EXECUTIVE.agent.yaml`(⚠️2026-08-05 O38 로 `_archive/` 이관된 구 경로 — 이력 서술이므로 원문 보존)을 배포본 기준으로 재작성해 정본↔배포본 정합화 완료(`cortex_agent_deploy` → 두 Agent VERSION$2, VERSION$1 보존·grant 보존). AGENT_EXECUTIVE의 SV_AD(analyst_ad) 미반영 구버전 부채도 함께 해소. 추가로 **원천 답변 수치 금지 규칙**을 두 Agent instructions에 신설. ② ✅ **해소 방향 확정**: SV 정본은 `05_SV_DDL.sql`(원칙12)이며 `cortex_project/`의 SV yaml은 정본이 아니다 — 05를 `EXECUTE IMMEDIATE FROM` 으로 실행하는 것이 표준 경로임을 99 §5에 명시. ③ **평가셋 공백**: 원천 질문은 SQL을 만들지 않아 gold-SQL 대조로 채점 불가 → **가드레일(ⓖ) 신규 클래스**("텍스트 답변·도구 미호출 기대") 등록 필요. **수치 금지 규칙도 같은 클래스로 채점**해야 한다(원천 질문에 행수·금액을 답하면 실패). ④ `06_검증쿼리_VQR.md`에 원천 관련 VQR은 **부적합**(VQR=SQL 정본) → 등록하지 않는 것이 맞음을 명시 |
| **10-b** | **공8 재방송 개발단가 복원 (오진 교정)** | 🔄 **문서완료·배포 대기** (2026-07-29) — 사용자가 `13_SV_AD_배포_추가작업.sql` §진단쿼리를 실행해 **2026-07-28 배포취소 결정이 오진**임이 드러났다. **[원인]** `DVLP_CNT`는 `BRONZE_AGENCY.REBRDC_AD_CMPGN_DTLS` 전용이고 `VIDEO_AD_CMPGN_DTLS`에는 개발 컬럼이 **아예 없다**(비디오 리포트=전환콜 보고) → VIDEO의 공백은 **미적재 결손이 아니라 구조적 부재**인데 이를 분모 모집단(37,886행)에 포함시켜 "커버리지 5.2%·41.5% 왜곡"이 산출됐다. **[실측]** REBROADCAST 단독 커버리지 **96.03%**(1,982/2,064) · 정합 왜곡 **0.61%**(158,933→**157,969원**). 종전 주석의 "정합해도 방송 광고비 29%만 반영"도 뒤집힌 오기(실제 **70.7% 포함**, 29%는 제외되는 VIDEO 몫). **[조치]** ① `05_SV_DDL.sql`: `REBRDC_DEV_UNIT_PRICE` metric 신설(`BRDC_`→`REBRDC_` 명명 — VIDEO에 개발 개념 부재하므로 "방송"은 여전히 오인 유발) + TOTAL_DVLP_CNT/MEMBER_CNT COMMENT·최상위 COMMENT·AI_SQL_GENERATION(1)(6) 교정. ② `09`+`cortex_project/AGENT_EXECUTIVE.agent.yaml`: 스코프 3분류(디지털/방송/재방송)·"미제공" 문구 삭제·tool description·sample_question. ③ `04_SV_설계.md` §6.4/6.5/6.7 + `03_SV_metric_배속.md` 공8 P2→**P1**(P1 72→73·P2 9→8·보류 4→3). **[교훈]** "결함 metric 미노출" 원칙은 유지하되 **결함 판정 자체를 먼저 검증**한다 — 커버리지 분모에 해당 개념이 없는 세그먼트를 넣으면 정상 지표가 결함으로 오판된다(04 §6.4.1). ④ 잔여: **VIDEO 개발단가**는 대행사 리포트에 항목 신설 요청 필요(별건) |
| **11** | **마케팅 Agent 설계 (`AGENT_MARKETING`)** | 🔄 **설계완료·미배포** (2026-08-14 O76) — 사용자 지시 = 마케팅 정본 인벤토리(`99_provided_definition/04_마케팅_보고서필드 인벤토리.md`) 기반 Agent 신설. ✅ **신설 2종**: 설계 정본 `30_마케팅_AGENT_설계.md` + 스펙 정본 `cortex_project/agents/AGENT_MARKETING/agent_spec.yaml`(도구 6 = 리소스 6 · YAML 파싱 통과 · 참조 SV 6종 전부 라이브 실재 · 한 줄 2000자 초과 0). 🟢 **신설 SV 0종** — 요건 5섹션을 답할 SV 가 이미 배포돼 있었다. 🔴🔴 **`09_보고서필드_조립가능성` 마케팅 판정 6건을 실측으로 반증**(M1~M6 · 원인 = 섹션 4·5 앵커가 `FMM` 인데 요건 개념은 FME·FMC·FMF·위성 pre-join 에 있다 = **O47-B 앵커 임의성의 마케팅 축 재현**): 광고유형(디지털 205,059/205,059 전건) · CM위치(방송 32,840/36,416=90.2%) · 광고비(디지털 193,959·방송 35,046·재방송 2,007 — 예산이 아니라 **대행사 원천**) · 개발구분(`FME.DVLP_DIV_NM` 3,594,843 전건 · `FMC.ACQ_DVLP_DIV_CD` 1,585,949 전건) · 가입캠페인중단일(`FMC.FIRST_STOP_DATE_SK` 901,577/1,585,949=56.85% — 미중단 회원은 값 없음이 정상) · 성별·연령대 경로(사건·획득 시점 축 전건 ⇒ `DIM_MEMBER` SCD2 4.49배 팬아웃 회피 · `P151`). 🔴 **`09` 산출물 자체는 고치지 않았다** — 생성기 앵커 로직 변경은 507행 전량·골든 재승인에 영향하므로 별건(30 §7 ①). ⛔ **미배포**(사용자 결정 = 설계까지) · ⬜ 잔여 = NL 스모크 · VQR·평가셋 · 도구 설명 3중 동기화 비용 |
