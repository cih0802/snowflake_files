...
CREATE OR ALTER SEMANTIC VIEW GN_DW.SERVING.SV_EVENT_PARTICIPATION
  ...
  DIMENSIONS (
    ...
    event.EVENT_CATEGORY AS event.EVENT_CATEGORY WITH SYNONYMS ('행사구분코드') COMMENT = '행사 구분 **원천 코드**(라벨 아님). 🔴🔴 **두 원천의 코드체계가 한 컬럼에 혼재한다** — **일반행사(EVENT) = 백 단위**(''100''~''500'' · 코드군 MS286) · **캠페인행사(CRMN) = 한/두 자리**(''1''~''16'' · 코드군 MS002). 따라서 **EVENT_KIND_NAME 과 함께 보지 않으면 서로 다른 체계를 한 표에 섞는다.** ✅ **사람이 읽는 라벨은 EVENT_CATEGORY_NAME 축을 쓴다**(DEC-35 R1 — 두 코드군 모두 등급 B 확정). ⚠️ 센티넬 행(''(미매핑)'')은 코드가 없어 라벨도 NULL 이다 — 결측이 아니라 조인 대상 부재다. 실제값 19종: ''1''·''2''·''3''·''4''·''5''·''6''·''7''·''8''·''9''·''12''·''13''·''14''·''15''·''16''·''100''·''200''·''300''·''400''·''500'' + NULL',
    event.EVENT_CATEGORY_NAME AS event.EVENT_CATEGORY_NAME WITH SYNONYMS ('행사구분', '행사구분명', '행사종류구분') COMMENT = '행사 구분 **라벨 — 현업 응답용 정본 축**이다. 「행사 구분별」 질문은 이 축으로 답한다. 실제값 19종 — 일반행사 계열(코드군 MS286): 온라인·앱전용·회지·리플렛·오프라인 / 캠페인행사 계열(코드군 MS002): 국내사업장방문·해외사업장방문·좋은이웃콘서트·굿멤버스데이(본부)·굿멤버스데이(지부)·굿모닝·문화서비스·문화서비스_전시·문화서비스_서적·문화서비스_공연·기업제휴·특성화회원조직·I''m your pen·기타. ⚠️ 값에 아포스트로피가 포함된 라벨(I''m your pen)이 있어 따옴표 열거 규약 대신 **종수 선언 + 나열**로 적는다 — 종수는 게이트가 실측과 대조한다. 🔴🔴 **라벨공간이 원천별로 갈린다** — 두 코드군의 라벨이 한 축에 섞여 있으므로 이 축으로 분해할 때는 **EVENT_KIND_NAME 을 동반**해 어느 계통인지 밝힌다. ⚠️ 센티넬 행은 라벨 NULL 이다(코드 자체가 없다)',
    fep.PART_STATUS AS fep.PART_STATUS WITH SYNONYMS ('참여상태코드') COMMENT = '참여 상태 **원천 코드**(라벨 아님). 🔴🔴 **두 원천의 코드체계가 혼재한다** — 일반행사(EVENT) = 백 단위(''110''~''220'' · 코드군 MS304) · 캠페인행사(CRMN) = 한 자리(''1''~''6'' · 코드군 MS006) ⇒ **EVENT_KIND_NAME 동반 필수**. 🔴 **두 원천은 「참여」의 정의가 다르다** — MS304 는 **다단계 이벤트의 단계 통과 여부**이고 MS006 은 신청/참여/불참이다. 합산하면 정의가 혼입된다. ✅ 라벨은 PART_STATUS_NAME 축을 쓴다(DEC-35 R1). ⚠️ ''`)`'' 는 **원천 입력 오류(오염값)** 이며 라벨을 붙이지 않는다 — 정상 코드로 취급하지 말 것(원장 §O59-G). 실제값 19종: '')''·''1''·''2''·''3''·''4''·''5''·''6''·''110''·''120''·''130''·''140''·''150''·''160''·''170''·''180''·''190''·''200''·''210''·''220'' + NULL',
    fep.PART_STATUS_NAME AS fep.PART_STATUS_NAME WITH SYNONYMS ('참여상태', '참여상태명') COMMENT = '참여 상태 **라벨 — 현업 응답용 정본 축**이다. 실제값 18종: ''Success''·''Fail''·''1_step_right''·''1_step_fail''·''2_step_right''·''2_step_fail''·''3_step_right''·''3_step_fail''·''4_step_right''·''4_step_fail''·''5_step_right''·''5_step_fail''·''신청''·''참여''·''불참''·''대기''·''취소''·''대기(결제)'' + NULL. 🔴🔴 **라벨공간이 원천별로 갈린다 — 영문 라벨과 한글 라벨이 섞여 있다**: 영문 계열(Success·Fail·n_step_*)은 **일반행사(MS304)**, 한글 계열(신청·참여·불참·대기·취소·대기(결제))은 **캠페인행사(MS006)** 다. ⇒ 「참여 상태별」로 분해하면 **한 의미가 두 라벨로 갈라진다**(예: 참여 성공이 ''Success'' 와 ''참여'' 로 나뉜다) — **EVENT_KIND_NAME 을 반드시 동반**하고 두 계열을 합산하지 말 것. 🔴 두 원천은 참여의 정의 자체가 다르다(단계 통과 vs 신청/참여). ⚠️ MS304 라벨이 **사전에 영문으로 등재**돼 있어 그대로 노출한다 — 한글 표기는 현업 회신 대기(문서20 §M-1). ⚠️ NULL 은 오염값·센티넬처럼 사전에 없는 코드 행이다(라벨 창작 금지)',
    fep.PART_PATH AS fep.PART_PATH WITH SYNONYMS ('참여경로코드', '신청경로코드') COMMENT = '참여·신청 경로 **원천 코드**(라벨 아님). 🔴 **두 체계 혼재** — 일반행사 = 백 단위(코드군 MS303 · 원천 컬럼 참여경로) · 캠페인행사 = 한 자리(코드군 MS004 · 원천 컬럼 **신청경로**) ⇒ **EVENT_KIND_NAME 동반 필수**. ✅ 라벨은 PART_PATH_NAME 축을 쓴다. ⚠️ ''`)`'' 는 오염값이며 라벨 없음(원장 §O59-G). 실제값 13종: '')''·''1''·''2''·''3''·''4''·''5''·''6''·''100''·''200''·''300''·''500''·''600''·''700'' + NULL',
    fep.PART_PATH_NAME AS fep.PART_PATH_NAME WITH SYNONYMS ('참여경로', '신청경로') COMMENT = '참여·신청 경로 **라벨 — 현업 응답용 정본 축**이다. 실제값 12종: ''webmail''·''homepage''·''출석''·''텍스트''·''독자의견''·''독자이벤트''·''홈페이지''·''상담센터''·''초대''·''이메일''·''모바일웹''·''APP'' + NULL. 🔴🔴 **같은 의미가 두 라벨로 갈라져 있다** — ''homepage''(일반행사 MS303)와 ''홈페이지''(캠페인행사 MS004)가 **동시에 존재**한다. 사전 라벨이 원천별로 다르게 등재된 결과이며 우리가 통합 라벨을 만들면 **라벨 창작**이 된다(DEC-17-B) ⇒ 통합하지 않는다. ⇒ 「참여경로별」로 분해하면 **한 의미가 두 행으로 갈라지므로 EVENT_KIND_NAME 을 반드시 동반**하고, 두 계열을 하나로 합쳐 답하려면 **합쳤다는 사실과 대응 관계를 명시**한다. ⚠️ 오염값·사전 미등재 행은 라벨 NULL 이다',
  ...
  COMMENT = 'Phase-1 행사 참여 SV(base FEP). [원천 요약] 원천시스템=CRM(eCRM 행사관리) · BRONZE=GN_DW.BRONZE_CRM(행사 TM_MS_EVENT∪TM_MS_CRMN · 참여 TD_MS_EVENT_PRTCPNT_DTL∪TD_MS_CRMN_PRTCPNT) → SILVER(CRM_EVENT+CRM_EVENT_PARTICIPATION) → GOLD(FEP). 테이블별 상세 원천은 각 테이블 COMMENT의 [원천] 절 참조. 활성: 참여자수·참여건수·고유 참여회원수, 행사명/종류·참여일별 · **행사구분 라벨** · **참여 상태·경로·채널 라벨**. ⚠행사 미매칭분이 Unknown(EVENT_SK=0)으로 라우팅되므로 행사명별 집계는 부분집합이다 — 확정치로 단정 금지. 🔴🔴 **이 팩트는 두 원천의 결합이며 코드·라벨 축이 원천별로 갈린다**(일반행사 / 캠페인행사) — 구분·상태·경로 축으로 분해할 때는 **EVENT_KIND_NAME 동반이 필수**다. 🔴🔴 **단 그 판별자에는 사각지대가 있다** — `EVENT_KIND_NAME` 은 행사 마스터에서 오므로 **행사 미매칭 구간(약 23%)에서는 ''(미매핑)'' 이 되어 계열을 알려주지 못한다.** 그 버킷은 **별도 행으로 분리**하고 어느 계열로도 단정하지 않는다(팩트에 계열 판별 컬럼을 두는 구조 개선 = 원장 §O59-S). 🔴 특히 **같은 의미가 두 라벨로 나오는 축이 있다**(참여경로의 homepage/홈페이지 · 참여상태의 영문/한글 계열) — 사전 라벨이 원천별로 다르게 등재된 결과이며 통합 라벨을 창작하지 않는다(DEC-17-B). 🔴 **두 원천은 「참여」의 정의 자체가 다르다** — 일반행사는 다단계 통과 여부, 캠페인행사는 신청/참여/불참이다. 비활성(적재 대기): 모집/총원, 캠페인/후원사업별. 🔴 이 목록은 축을 활성화할 때마다 회수 대상이다(P61).'
  AI_SQL_GENERATION '적용 조건: 질문에 기간(연/월)과 그룹(행사종류·행사명·회원구분 등)이 모두 없을 때만. 이 경우 전체 기간 풀스캔을 피해 데이터에 실제 존재하는 최신 연월(MAX(연월)) 기준 직전 12개월로 한정하고(기준월은 CURRENT_DATE가 아니라 데이터 최신월 — 미래연월 데이터도 최신월로 인정), GROUP BY ROLLUP((연,월))로 월별 행 + 전체 총계 행을 함께 반환해 월별 추이와 총계를 동시에 제공한다. 행사명별 집계를 낼 때는 미매칭분이 Unknown으로 빠져 부분집합임을 답변에 명시한다. 사용자가 기간/그룹을 지정하거나 합계·총액만 원하면 그 요청을 우선한다. 핵심 규칙: (1) 🔴 **코드축이 아니라 라벨축으로 답한다** — 「행사 구분별」은 EVENT_CATEGORY_NAME, 「참여 상태별」은 PART_STATUS_NAME, 「참여·신청 경로별」은 PART_PATH_NAME, 「참여 채널별」은 PART_CHANNEL_NAME 을 쓴다. 코드축(EVENT_CATEGORY·PART_STATUS·PART_PATH·PART_CHANNEL)은 필터 재현·원천 대조용이며 현업 답변에 코드를 그대로 내지 않는다. (2) 🔴🔴 **이 SV 의 코드·라벨 축은 두 원천이 섞여 있어 EVENT_KIND_NAME 동반이 필수다** — 참여 팩트는 일반행사(EVENT)와 캠페인행사(CRMN) 두 마스터의 결합이고 **원천마다 코드군이 다르다.** EVENT_KIND_NAME 없이 이 축들로 그루핑하면 서로 다른 체계를 한 표에 섞는다. (2-B) 🔴🔴 **단 이 판별자에는 사각지대가 있다 — 반드시 알고 쓸 것.** `EVENT_KIND_NAME` 은 **행사 마스터에서 오는 값**이라, 참여 행이 행사에 매칭되지 않은 구간(''(미매핑)'')에서는 **계열을 알려주지 못한다.** 그 비중은 작지 않다(행사 미매칭 약 23%). ⇒ **''(미매핑)'' 버킷을 다른 계열과 합치거나 어느 한 계열로 단정하지 말고 별도 행으로 분리해 제시**하고, 그 구간에 대해서는 계열 판별이 불가함을 밝힌다. ⚠️ 참고로 그 구간의 코드값 자체는 여전히 어느 한 체계에 속하지만(코드 범위로 짐작 가능), **짐작을 근거로 계열을 단정하지 않는다** — 팩트에 계열 판별 컬럼을 두는 구조 개선이 등재돼 있다(원장 §O59-S). (3) 🔴🔴 **라벨공간이 원천별로 갈린 축이 있다 — 같은 의미가 두 라벨로 나온다.** PART_PATH_NAME 은 ''homepage''(일반행사)와 ''홈페이지''(캠페인행사)가 동시에 존재하고, PART_STATUS_NAME 은 영문 계열(Success·n_step_*=일반행사)과 한글 계열(신청·참여·불참=캠페인행사)로 갈린다. **두 라벨을 임의로 통합하지 않는다**(사전에 없는 통합 라벨을 만들면 라벨 창작이다) — 갈라진 채로 EVENT_KIND_NAME 과 함께 제시하고, 사용자가 합친 값을 원하면 **어떤 라벨들을 합쳤는지 명시**한다. (4) 🔴 **두 원천은 「참여」의 정의가 다르다** — 일반행사(MS304)의 상태는 **다단계 이벤트의 단계 통과 여부**이고 캠페인행사(MS006)는 신청/참여/불참이다. 두 계열의 「참여」를 합산해 하나의 참여율로 제시하지 말 것. (5) 🔴 **PART_CHANNEL 계열의 NULL 은 두 가지가 섞여 있다 — 하나로 설명하지 말 것**: ㉠ 캠페인행사 원천에는 채널 컬럼이 아예 없어 그 참여 행 **전량**이 NULL 이다(**구조적 부재** · 원천 스캔 확인) ㉡ 일반행사 계열의 **소수 행**도 NULL 이며 그쪽은 **결측 또는 오염값**이다. 채널별 분해에는 ㉠을 반드시 밝히고, 남는 NULL 을 「전부 캠페인행사」라고 단정하지 않는다. (6) ⚠️ **오염값 행이 있다** — 참여 3축에 원천 입력 오류값이 소수 존재하며 라벨이 NULL 이다. 정상 코드로 취급하거나 의미를 부여하지 말고, 라벨 NULL 버킷을 밝힐 때 함께 언급한다.';
