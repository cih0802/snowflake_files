...
> **선행**: `02_GN_DW_building/08_After_Deploy_DBT.sql`(GRANT·CoWork · 🔴 [2026-08-10 O55] §G helper 뷰 절은 삭제됐다) → `05_1~05_9_SV_DDL_*.sql`(SV 9종) → `09_1_AGENT_생성.sql` → **`09_2_AGENT_버전업.sql`** 순차 실행 완료(Agent 2 + 스펙 반영 + CoWork 연결 상태).
...
> 🔴 **[2026-08-10 O57] 기대값 전량 교체.** 이 파일은 **O56-C 의 28곳 회수 목록에서 누락**돼 폐기값(M3 93.86% 등)이 그대로 남아 있었다.
> 기대값 정본 = `07_평가셋_eval.md`(O57 재실측). **값 층은 O57 이 SQL 로 이미 판정했고, 여기 `⬜` 는 「NL 라우팅 미검증」을 뜻한다.**

| # | 질문 (그대로 입력) | 기대 SV | 기대값 (2026-08-10 O57 실측) | 판정 |
|---|---|---|---|---|
| M3 | 2024년 납부율은? | SV_MEMBER_MONTHLY | **85.77%** 🔴 종전 93.86% 는 폐기식 값 | ✅값 · ⬜NL |
| M4 | 연도별 납부율 추이(2023~2025) | SV_MEMBER_MONTHLY | **86.05 · 85.77 · 85.65%** 🔴 종전 93.66/93.86/93.98 폐기 | ✅값 · ⬜NL |
| M5 | 회원구분별 **총수납액** | SV_MEMBER_MONTHLY | 개인 756.64B · 기업 132.14B · 단체 6.36B · (NULL) 0.04B 🔴 차원은 라벨 `MEMBER_TYPE_NAME`(종전 `1/2/3` 코드 표기) · 질문 문구도 「납입회비」→「총수납액」 교정 | ✅값 · ⬜NL |
| M9 | 전체 개발건수와 중단건수 | SV_MEMBER_MONTHLY | **2,291,878** / 1,038,262 🔴 종전 3,594,843 은 **개발원천 행수**(O24 미반영) | ✅값 · ⬜NL |
| E2 | 개발 고유 회원수 | SV_MEMBER_EVENT | **1,585,923** 🔴 종전 1,585,949 는 원천 distinct 회원수(차이 26명) | ✅값 · ⬜NL |
| E4 | 전이유형별 건수와 회원수 | SV_MEMBER_EVENT | 개발 **2,291,878**/**1,585,923** · 중단 1,038,262/903,064 | ✅값 · ⬜NL |
| S1 | 전체 발송수 | SV_SERVICE | **38,470,780** | ✅값 · ⬜NL |
...
| # | 질문 (그대로 입력) | 기대 SV | 기대값 (2026-08-10 O57 실측) | 판정 |
|---|---|---|---|---|
| B1 | 전체 편성예산은? | SV_BUDGET | **547,614,848,306** 🔴 종전 503,070,876,000 은 **2026년 단독 적재 시점** 값 | ✅값 · ⬜NL |
| B2 | 전체 집행예산(ERP)은? | SV_BUDGET | **297,045,080,903** 🔴 종전 199,287,107,812 | ✅값 · ⬜NL |
| B3 | 전체 집행율은? | SV_BUDGET | **54.24%** 🔴 종전 39.61% | ✅값 · ⬜NL |
| B4 | 예산구분별 편성·집행·집행율 | SV_BUDGET | 지출 **298.60B/151.18B/50.63%** · 수입 **249.01B/145.86B/58.58%** | ✅값 · ⬜NL |
| O1 | 전사 **총수납액**(회비+기부금)은? | SV_MEMBER_MONTHLY | **895,178,309,108** 🔴 질문 문구 교정(「납입회비」가 아니다 · 회비만은 `TOTAL_PAID_FEE_BILLABLE` **768,800,286,349**) | ✅값 · ⬜NL |

> ⚠️ **B2·B3·B4 집행 계열은 2026 진행연도라 재적재마다 변한다** — 테스트 전 `07 §5 gold SQL` 로 기대값을 재실행할 것.
> 🆕 **추가 문항 권장(O57)**: 「전체 개발 건수는?」 → **2,291,878**. 3,594,843 으로 답하면 **FAIL**(감액·후원중단 계상 · O24).
