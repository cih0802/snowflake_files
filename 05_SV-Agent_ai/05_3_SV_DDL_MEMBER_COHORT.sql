...
CREATE OR REPLACE SEMANTIC VIEW GN_DW.SERVING.SV_MEMBER_COHORT
  ...
  DIMENSIONS (
    ...
    fmc.ACQ_AREA_CD         AS fmc.ACQ_AREA_CD     WITH SYNONYMS ('지역코드') COMMENT = '획득시점 지역 원천코드(CM018 + 라벨 없는 센티넬 ''0''). 라벨은 ACQ_REGION',
    ...
    fmc.ACQ_SEX_CD          AS fmc.ACQ_SEX_CD      WITH SYNONYMS ('성별코드') COMMENT = '획득시점 성별 원천코드(CM013 1~8 + 라벨 없는 센티넬 ''0''). 라벨은 ACQ_GENDER',
    -- ── 이탈 특성 ─────────────────────────────────────────────────────────────
    ...
    acq_campaign.MARKETING_CAMPAIGN   AS acq_campaign.MARKETING_CAMPAIGN   WITH SYNONYMS ('마케팅캠페인', '마케팅 캠페인명') COMMENT = '획득 캠페인의 마케팅캠페인명. 실제값 예: ''24년 이전컨텐츠''·''그외 지역개발캠페인''·''유어턴(통합A/B)''·''기존회원캠페인 및 기타''·''25년 이전컨텐츠(영상광고)''·''TS/TM''. 카디널리티가 높다',
