...
CREATE OR ALTER SEMANTIC VIEW GN_DW.SERVING.SV_MEMBER_COHORT
  ...
  DIMENSIONS (
    ...
    -- 🔴🔴 [2026-08-29 O119] 아래 `CAMPAIGN_INFLOW_PATH` 열거를 **라이브 실측으로 전면 교체**했다
    --   (`sv_code_label_gate` 축1 「코드값 부재」 + 축2 「종수 불일치」 동시 FAIL).
    --   종전 문안은 「실제값 16종」이었고 그중 **7종이 라이브에 존재하지 않았다**:
    --     `회원 온라인개발`·`회원 콜개발`·`기업`·`회원 기타`·`마케팅콜개발`·`회원 오프라인개발`·`직원개발`.
    --   반대로 라이브에만 있고 열거에 없던 값이 **3종**이었다: `콜개발`·`기타 기업개발`·`오프라인`.
    --   🔴 **원인은 결손이 아니라 라벨 개칭이다** — `회원 콜개발`→`콜개발` · `회원 오프라인개발`→`오프라인` ·
    --      `기업`→`기타 기업개발` 처럼 MM293 라벨 집합이 갈렸고 COMMENT 가 따라가지 못했다.
    --   🔴 그래서 위험한 방향이 **양쪽**이다: ㉠ 없는 값으로 필터하면 **0행을 사실로 답한다**
    --      ㉡ 새 값이 열거에 없어 Agent 가 그 채널을 **누락한 목록**을 낸다.
    --   ⚠️ 형제 축 `SV_MEMBER_EVENT.CAMPAIGN_INFLOW_PATH`(`MBER_INFLOW_PATH_NM_AT_EVENT`)가 **같은 축**이며
    --      그쪽은 열거가 아예 없다(게이트 🟠 「열거 누락」) — 같은 라벨 집합이므로 함께 갱신할 후보다.
    ...
    -- 🔴 [2026-08-29 O119] `DOMESTIC_OVERSEAS` 종수·열거를 라이브 실측으로 교체했다(축2 FAIL).
    --   종전 「3종」에 **`전체사업` 이 빠져 있었다** — 부재가 아니라 **미열거**다(형제 축
    --   `SV_MEMBER_EVENT.DOMESTIC_OVERSEAS` 도 같은 값 집합이며 같은 결함이었다 · 함께 시정했다).
