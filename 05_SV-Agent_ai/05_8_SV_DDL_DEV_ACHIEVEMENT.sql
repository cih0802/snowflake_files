...
--   base = `GN_DW.GOLD.WIDE_DEV_ACHIEVEMENT`(FTG_D 목표 × FME 실적 FULL OUTER 월 conform).
--
...
--
-- ▶ 선행 조건
--   ① GOLD 적재 완료(`dbt build`) — `WIDE_DEV_ACHIEVEMENT` 가 존재해야 한다
...
/* =====================================================================================
   8. SV_DEV_ACHIEVEMENT (member Agent) — base WIDE_DEV_ACHIEVEMENT(월×조직×개발구분)
...
CREATE OR REPLACE SEMANTIC VIEW GN_DW.SERVING.SV_DEV_ACHIEVEMENT
  TABLES (
    achv AS GN_DW.GOLD.WIDE_DEV_ACHIEVEMENT
...
FROM GN_DW.GOLD.WIDE_DEV_ACHIEVEMENT;
