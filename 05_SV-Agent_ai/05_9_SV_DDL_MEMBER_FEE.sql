...
CREATE OR ALTER SEMANTIC VIEW GN_DW.SERVING.SV_MEMBER_FEE
  ...
  METRICS (
    ...
    fee.UNPAID_RATIO AS SUM(fee.UNPAID_BILLED_AMT) / NULLIF(SUM(fee.BILLED_AMT), 0) * 100
      ...
      COMMENT = '미납비중(**%**) = 미납 청구액 ÷ 회비 청구액 ×100. N(비가산). DEC-3 정본 분자. 🔴[O52-A] 단위는 **퍼센트**다 — 값을 다시 ×100 하지 말 것. `SV_MEMBER_MONTHLY.UNPAID_RATIO` 와 **같은 이름·같은 식·같은 단위**이며 전체 합계가 일치해야 한다(GATE-D 실측 일치 확인 · 값은 이슈원장 §O55·§O56 참조). 🔴[O56 EXPO-1] 종전 그쪽 정본 이름은 `UNPAID_RATIO_DEC3` 였고 짧은 이름은 폐기된 차감식이 쓰고 있었다 — 차감식 제거 후 이름이 일치했다.',
    -- ── 규모 ──────────────────────────────────────────────────────────────────
  ...
  COMMENT = 'Phase-1 회비 분해 SV(base WIDE_MEMBER_FEE). grain = 회원 × 회비월 × 후원사업 × 회비구분 × 납입유형 × 결제수단. **후원사업·납입방식(결제수단)·회비구분·납입일별 회비 청구/납입/미납의 정본**이다. [원천 요약] 원천시스템=CRM(eCRM) · BRONZE=GN_DW.BRONZE_CRM(회비 TM_PM_MBRFEE_ACMSLT · 기부 TM_PM_DNTN_DTLS · 후원사업 TM_CM_SPNSR_BSNS_INFO) → SILVER(CRM_PAYMENT_BILLING·CRM_SPONSOR_BIZ·CRM_CODE) → GOLD(FACT_MEMBER_FEE→WIDE_MEMBER_FEE). 테이블별 상세 원천은 테이블 COMMENT 의 [원천] 절 참조. 활성: 청구액·납입회비·총수납액·미납금액·납부율·미납비중·고유 납입회원수 · 후원사업·납입방식·회비구분·납입유형·납입일 축 · 회원 속성(현재 스냅샷) · **획득 귀속축**(브랜드·캠페인·상위캠페인·홍보방법·마케팅캠페인·부서·후원사업·연령대·지역). 시간=전체가능. 🔴🔴 **`SV_MEMBER_MONTHLY` 와 한 표에 합산 금지 — 이중계상이다.** 두 SV 는 같은 원천을 다른 grain 으로 담은 형제 팩트이며, 실측(2026-08-06) 조인 시 청구액이 부풀어 오른다(전·후 금액과 과대 비율은 이슈원장 §O45 참조). ⇒ 회원-월 요약·상태별 분해는 `SV_MEMBER_MONTHLY`, 회비 분해는 이 SV **중 하나만** 앵커로 쓰고 둘 다 필요하면 **표를 분리**한다(전체 합계는 두 SV 가 일치해야 한다 = GATE-D). 🔴 **「후원사업」은 세 축이 있다** — 이 SV=납입 대상 · `SV_MEMBER_EVENT`=개발 사건 시점 · `SV_MEMBER_COHORT`=획득 시점. 어느 축으로 답했는지 반드시 밝힌다. 🔴 **「부서」는 두 축이 있다** — 이 SV 의 ACQ_DEPARTMENT=획득 시점 ·
`SV_MEMBER_EVENT.ORG_DEPARTMENT`=사건 부서. 비활성: 회원 단위 미납 플래그(SV_MEMBER_MONTHLY 소관) · 결제수단 라벨 미특정 5종(O45-B 현업 확인) · 상위 조직 분해(CONF-4). 🔴🔴 **[2026-08-13 O67 발행] 보고서 필드 조립 판정 4종 중 이 SV 소관분**(정본 = 산출물 `09_보고서필드_조립가능성`): ① **배분규칙필요** — 주간 중단보고·미납중단안내서비스보고가 이 SV 의 「납입방식(결제수단)」을 각각 **사건 grain·발송 grain 으로** 요구한다. 결제수단은 **회비 행의 속성**이므로 다른 grain 으로 내리려면 배분 규칙이 필요하고 그것은 업무 판단이다(O47) ⇒ **SQL 을 만들지 말고 사유를 답한다.** 🟢 회비 grain 에서 「납입방식별 청구·납입·미납」을 묻는 질문은 이 SV 가 **정본**이며 정상 답변한다 — 두 경우를 혼동하지 말 것. ② **형제팩트중복** — 월간 중단보고·연간분석(개발/활동)의 「납입방식」과 연간분석(회비)의 「납입(원)」이 이 판정이다. 위 이중계상 가드와 **같은 사안**이며(**라벨명을 명시**한 것이다) 처방도 같다: 회원-월 요약과 **한 표에 합치지 말고 표를 분리**한다. 🟢 「납입(원)」은 이 SV 를 앵커로 바꾸면 **답이 나온다** — 불가가 아니다. ③ **앵커_경합** — 개발실적보고 주간 섹션에서 이 SV 는 경합 팩트 중 하나다 ⇒ 회비 축만 이 SV 로 답하고 **목표·실적과 한 표로 합치지 않는다**(주간 실적은 `SV_MEMBER_EVENT` · 월 목표는 `SV_DEV_ACHIEVEMENT`).'
  AI_SQL_GENERATION '핵심 규칙: (1) 🔴🔴 **`SV_MEMBER_MONTHLY` 의 회비 measure 와 이 SV 의 measure 를 같은 답변의 같은 표에 넣지 않는다** — 같은 원천을 다른 grain 으로 담은 형제 팩트라 합치면 청구·납입액이 과대계상된다(실측 규모는 이슈원장 §O45 참조). 둘 다 필요하면 **표를 분리하고 각 표의 grain 을 명시**한다. 「회원-월 요약·납부율 추이」는 SV_MEMBER_MONTHLY, 「후원사업·납입방식·회비구분·납입일 분해」는 이 SV 다. (2) **납부율은 PAYMENT_RATE_FEE 를 쓴다** — TOTAL_PAID_ALL(회비+기부금)을 분자로 만들지 않는다. 기부금은 청구액이 없어 분모에 못 들어가므로 비율이 100% 를 넘는다(O40 실사고). 「총수납액」을 명시적으로 물을 때만 TOTAL_PAID_ALL 을 쓰고 기부금 포함임을 밝힌다. (3) **「명」은 DISTINCT_PAYING_MEMBERS 로만 답한다** — 이 팩트는 회원당 여러 행이라 행 수·BILLING_ROWS 를 「명」으로 쓰면 크게 과대해진다. 그리고 이 metric 은 비가산이므로 월별 값을 더해 연간 회원수를 만들지 않는다. (4) **BILLING_ROWS 를 「건수」라 부르지 않는다** — 정본 (건) 은 금액÷10,000 규약이다(CONF-2). (5) **결제수단·후원사업별 합계는 전체 합계보다 작다** — 결제수단 라벨 미특정 5종과 후원사업 미매칭이 ''(미매핑)''으로 모인다. 캠페인·결제수단별로 답할 때는 미매핑 버킷의 존재를 밝히고, **총계는 축 없이** 답한다. (6) **「후원사업」·「부서」는 어느 축인지 반드시 밝힌다**: 이 SV 의 SPONSORSHIP=**납입 대상**, ACQ_SPONSORSHIP=**획득 시점**, ACQ_DEPARTMENT=**획득 시점 부서**다. 「부서별 개발실적」은 SV_MEMBER_EVENT(사건 부서), 「후원사업별 개발·중단
건수」도 SV_MEMBER_EVENT, 「캠페인별 이탈률」은 SV_MEMBER_COHORT 로 라우팅한다. 🔴 축을 밝히지 않으면 값이 맞아도 사용자가 틀린 결론을 얻는다. (7) **획득 귀속축은 현재 속성이 아니다** — ACQ_* 는 전부 최초 약정 당시 값이다. 「현재 연령대·현재 거주지·현재 소속 부서」 질문에는 산출 불가를 밝히고 획득 시점 기준으로 답하되 전제를 명시한다. (8) **미납·납부율에 「마감·확정」이라 단정하지 않는다** — 조회 시점 적재 스냅샷이며 과거 미납이 이후 납입되면 값이 바뀐다. 회비월은 미래월까지 존재한다. (9) 적용 조건(기간·그룹 모두 미지정 시): 전체 기간 풀스캔을 피해 데이터에 실제 존재하는 최신 연월(MAX(연월)) 기준 직전 12개월로 한정하고(기준월은 CURRENT_DATE 가 아니라 데이터 최신월), GROUP BY ROLLUP((연,월))로 월별 행 + 총계 행을 함께 반환한다. 비율은 총계 행에서 SUM 기반으로 산출한다. (10) metric 정렬에는 `NULLS LAST` 를 명시한다 — 회비구분이 NULL 인 기부금 행 등이 상위를 점유하는 것을 막는다. (11) 🔴 **비율 순위는 규모를 반드시 동반 제시한다**(P77 · O38-E 선례): 납부율 100% 인 구간이 실제로 존재하는데 전부 **청구 규모가 주력 연도의 수천분의 일**인 초기 연도·미래월이다(실측 2026-08-06 확인). 「납부율이 가장 높은 연도/후원사업/결제수단」을 물으면 `TOTAL_BILLED_AMT` 를 **같은 표에 함께** 내고, 극소 규모 항목을 단독 1위로 결론하지 않는다. 하한을 적용했으면 그 사실을 밝히고, 사용자가 하한을 정하지 않았으면 되묻는다. (12) 🔴 **`MONTH_KEY=0`(=Unknown월) 버킷을 실적 연월로 읽지 않는다** — 회비월·납입월이 모두 무효인 행이 모이는 자리다. 연월 추이를 낼 때는 이 버킷을 별도로 표기하고 「0년」·「0월」처럼 연월인 척 제시하지 않는다. (13)
🔴🔴 **[2026-08-13 O67] 판정 4종**: 「납입방식」을 **사건·발송 grain 으로** 요구하면(**배분규칙필요**) SQL 을 만들지 말고 배분 규칙 부재를 답한다 — 회비 grain 요청은 정상 답변이다 · 회원-월 요약과 회비 measure 를 나란히 요구하면(**형제팩트중복**) 표를 분리하고, 연간분석(회비) 「납입(원)」은 **이 SV 를 앵커로** 답한다 · 개발실적보고 주간 섹션(**앵커_경합**)은 회비 축만 이 SV 로 내고 목표·주간 실적은 각각 그 정본 SV 로 **표를 나눠** 제시한다.';

-- ── GRANT — 🟢 [2026-08-12 O61 · R1-3 전량독해에서 적발·교정] 이 파일 본문은 `CREATE OR ALTER` 다
--    (헤더 line 6 OWN-1 해소) ⇒ **GRANT 도 소유권도 파괴되지 않으므로 아래는 멱등 재확인**이다.
--    🔴 종전 이 자리의 *"CREATE OR REPLACE 가 GRANT 를 파괴하므로 같은 파일에서 재부여한다"* 는
--    **폐기된 규약을 인용한 stale 주석**이었다 — 같은 파일 헤더와 정면으로 모순됐다.
--    형제 파일 `05_1`(line 167~170)은 이미 교정된 문안을 갖고 있었다 ⇒ **파일 간 문안 드리프트**다.
--    ⚠️ 판정은 소유자 세션이 아니라 **소비 역할 세션**으로 한다(P126 · 검사기 `scripts/sv_unit_gate.py`).
