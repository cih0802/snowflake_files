...
**목적**: GOLD 24개 테이블의 각 컬럼이 **실제 SILVER 물리 테이블·컬럼** 중 무엇에서 오는지 명시. (2026-07-20 기준 SILVER·GOLD 적재 완료 — 원천별 갭은 §4에 이력으로 유지.)
...
**SILVER 적재 완료 = 32개 테이블 (CRM 22 + GA4 5 + AGENCY 2 + ERP 2 + IDENTITY_MEMBER_XREF 1).** 어드민(ADMIN)은 ❌제외 확정 → SILVER 불요. `CRM_BIZ_TARGET`만 0행(데이터 입고 대기).
...
- **GA4(5)**: GA4_TRAFFIC_SOURCE, GA4_EVENT_DIM, GA4_DEVICE, GA4_EVENT, GA4_IDENTITY
- **AGENCY(2)**: AGENCY_AD_CREATIVE, AGENCY_AD_PERFORMANCE
- **ERP(2)**: ERP_BUDGET, ERP_BUDGET_ITEM
- **신원(1)**: IDENTITY_MEMBER_XREF
...
| **DIM_BIGQUERY_SOURCE**.UTM_*·SOURCE_MEDIUM | `BIGQUERY_TRAFFIC_SOURCE.UTM_SOURCE`·`UTM_MEDIUM`·`UTM_CONTENT`·`UTM_TERM`·`SOURCE_MEDIUM` | |
| **DIM_BIGQUERY_EVENT**.CATEGORY/LABEL/ACTION | `BIGQUERY_EVENT_DIM.EVENT_CATEGORY`·`EVENT_LABEL`·`EVENT_ACTION` | |
| **DIM_DEVICE**.DEVICE_TYPE | `BIGQUERY_DEVICE.DEVICE_TYPE` | PC/M/APP |
...
| **FACT_BIGQUERY_BEHAVIOR** (FBQ) | 방문/세션/이벤트/스크롤/체류(VISITS·SESSION_CNT·EVENT_CNT·SCROLL_DEPTH·ENGAGEMENT_*·BOUNCE_RATE 등) | `BIGQUERY_EVENT`(GA_SESSION_ID·PERCENT_SCROLLED·ENGAGEMENT_TIME_MSEC·IS_ACTIVE_USER 등) | 비/준가산 주의 |
| FBQ | IDENTITY_SK(회원 귀속) | `BIGQUERY_IDENTITY` | §3 |
...
|---|---|---|---|
| FBQ → 회원 귀속 | `BIGQUERY_EVENT`/`BIGQUERY_IDENTITY` → `CRM_MEMBER` | `DIM_MEMBER_IDENTITY`(BIGQUERY_MEMBER_ID ↔ MEMBER_DK, 1:N) | 모든 BigQuery↔회원 통합의 근간. **user_id 4.22% → session-fill(`USER_ID_FILLED`·`ID_RESOLUTION`, 06_GA4 §5-A)로 커버리지 보강**, 파생지표는 신뢰도 필터 |
...
## 4. 원천별 갭 이력 (2026-07-20 적재 완료 — FTG_B 데이터·모금성비용만 잔여) — 인수인계 필독
...
> 🔄 **[2026-07-13 현업 위임]** 잔여 4건(ORG_SK·SEND_TYPE·EVENT·FEE_TYPE) + O8·O10은 **우리끼리 잠정 확정 + 검증 게이트**로 전환(상세 `04_silver_design/02_…BRONZE-GOLD연결 §3`). 실측 근거: SEND_GBN 대/중/소+라벨 실재·EVENT_DIV_CD 5값·ACMSLT는 거래테이블. 게이트 미통과 객체는 스키마-only.
...
- **grain 정합**: 월 팩트(FMM·FTG_*·FBD)는 SILVER 일자/약정에서 MONTH_KEY 롤업, 일 팩트(FBQ·FSE·FEP)는 DATE_SK — 혼재 없음 ✓

### 5-C. 커버리지 요약
- GOLD 24테이블 중 **SILVER(CRM·BigQuery)로 적재 가능**: DIM 13 + FACT 5(FMM·FME·FTG_D·FSE·FBQ·FEP 중 CRM/BigQuery분) — 부분 갭은 5-A.
...
- **테이블 수 정정** — CRM 원천 = **41테이블**(구 40 아님; 2026-06 현업 +1 `TM_CM_MKTNG_CMPGN_MNG`). 본 문서 SILVER 기준(32테이블)엔 영향 없음.
