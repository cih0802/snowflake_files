...
| 비가산 measure | 컬럼 유지 + 뷰 COMMENT에 재합산 금지 명시(FBQ) |
...
### 6. WIDE_BIGQUERY_BEHAVIOR (FBQ)

```sql
CREATE OR REPLACE VIEW GN_DW.GOLD.WIDE_BIGQUERY_BEHAVIOR
  COMMENT = 'BigQuery 행동 팩트 평탄화 (FBQ × DATE·IDENTITY·BIGQUERY_EVENT·BIGQUERY_SOURCE·DEVICE·CAMPAIGN). 비가산 지표(율·평균·사용자수) 상위 재합산 금지.'
AS
SELECT
    ...
    i.BIGQUERY_MEMBER_ID  AS IDENTITY_BIGQUERY_MEMBER_ID,
