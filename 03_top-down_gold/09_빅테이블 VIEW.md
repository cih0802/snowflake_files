...
| **CPC_CLICK_SRC** | `CPC_CLICK_SRC` | NUMBER | YES | GOLD.FACT_AD_DIGITAL (g) | 디지털 **클릭당** 단가(대행사 산정 · O174 개명) — 비가산 N. DW 재계산=SUM(AD_COST)/SUM(CLICKS) 🔴[O51-F 실측] 채움이 **소수**다. 커버리지를 모르고 비중·순위를 내면 결론이 뒤집힌다. 🟢DW 재계산은 전건 가능하다 — 집계에는 base 를 쓸 것. 실측 규모는 이슈원장 §O51-F. |
...
| **CPC_CALL_SRC** | `CPC_CALL_SRC` | NUMBER | YES | GOLD.FACT_AD_BROADCAST (b) | 방송 **콜당** 단가(대행사 산정) — 비가산 N, 재합산 금지. 🔴 클릭 분모가 아니다(O174 개명 · 디지털 CPC_CLICK_SRC 와 합산 금지). ⚠️**VIDEO 전용** — 다른 원천 행은 개념 자체가 없어 NULL 이며 결측이 아니다. |
...
| **CPC_CLICK_SRC** | `CPC_CLICK_SRC` | NUMBER | YES | GOLD.FACT_AD_DIGITAL | [비가산 N] 대행사 산정 **클릭당** 단가 ← DGT.CPC(O174 개명 · 방송 CPC_CALL_SRC 와 분모가 다르다 ⇒ 합산 금지). DW 재계산=AD_COST/CLICKS ⚠️[WIDE_AD_COMBINED] 디지털 원천 전용 컬럼이다 — 방송행(AD_SOURCE_TYPE 이 디지털이 아닌 행)은 **NULL 이며 그것은 결측이 아니라 원천 부재**다(위성 완전분할). 혼합 집계 전에 AD_SOURCE_TYPE 으로 스코프할 것. |
