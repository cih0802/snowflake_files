...
--------------------------------------------------------------------------------
  ...
  7. [2026-07-13 실측] ERP·AGENCY·GA4는 BRONZE 적재됨(실측 검토 게이트=04_silver_design/02 §3 후 적재).
     단 사업목표(FTG_B)·모금성비용(FBD)은 원천 부재, ADMIN(앱푸시·조회수)은 제외 확정 → 해당 컬럼만 생성·미채움.
...
-- ============================================================================
-- FACT 4: FACT_TARGET_BIZ (FTG_B) — 사업 목표 팩트 (사업목표 원천 부재: 예산원장≠사업목표)
-- ============================================================================
CREATE OR REPLACE TABLE GN_DW.GOLD.FACT_TARGET_BIZ (
    ...
    DW_UPDATE_TS        TIMESTAMP_NTZ   COMMENT '최종 갱신 시각 (공통감사)',
    DW_BATCH_ID         VARCHAR         COMMENT '적재 배치 식별자 = dbt invocation_id (공통감사)'
) COMMENT = '사업 목표 팩트 (MONTH_KEY × ORG × SPONSORSHIP · 사업목표 원천 부재 — 스키마-only, 33 E-6)';
...
--           · [금지] FK 섹션만 부분 재실행 → '제약 이미 존재' 오류.
--             특정 DIM 만 CREATE OR REPLACE → 이를 참조하던 자식 FK 소실.
--             부분 수정이 필요하면 해당 FK 를 ALTER TABLE ... DROP CONSTRAINT 후 재부여.
-- ============================================================================
