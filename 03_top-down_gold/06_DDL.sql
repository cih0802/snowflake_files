...
--     ACT_TARGET_DEV·FACT_TARGET_BIZ. 🟢본 차원은 DIM_DATE 의 **월 축 사영**이므로 별도 원천이 없고 값이 갈라질 수 없다. ⚠️월키가 YYYYMM 규약을 벗어난 원천 행은 팩트에서 0 으로 라우팅된다 — 이 차원에는 그 멤버가 없다.
...
--   PK(정보성) = MEMBER_DK
--   [컬럼별 설계 및 실측 이력]
--   · MEMBER_SK: DIM_MEMBER 의 **버전 대리키**. ⚠️이 테이블은 IS_CURRENT 행만 담으므로 회원 1명당 1값이지만, 그 의미는 여전히 '현재 버전 행의 키'다 — 회원 식별에는 MEMBER_DK 를 쓴다(MEMBER_SK 는 재빌드 시 달라질 수 있다).
...
--   · EFFECTIVE_FROM: SCD2 유효 시작 시각. 🔴이 테이블은 현재행만 담으므로 이 값은 '현재 상태가 시작된 시점'이다. 🔴과거 시점 상태가 필요하면 이 테이블이 아니라 DIM_MEMBER 를 EFFECTIVE_FROM/EFFECTIVE_TO 로 시점조인할 것 — 예측·피처 생성은 그 시점조인이 정답이며 현재값을 과거 행에 붙이면 정답 누설이다.
...
--   · ACQ_DATE_SK: 획득일 대리키(FK→DIM_DATE) — 획득 사건의 발생일. 0=캘린더 범위밖·무효. ⚠️회원번호 생성일(DIM_MEMBER_CURRENT.FIRST_JOIN_DATE)과 다르다 — 이 값은 **개발약정 사건일**이다.
...
CREATE OR REPLACE TABLE GN_DW.GOLD.DIM_MEMBER_ACQUISITION (
    ...
    ACQ_DEPARTMENT           VARCHAR         COMMENT 'ACQ_DEPARTMENT.',
...
--   [컬럼별 설계 및 실측 이력]
--   · MEMNUM: memnum(#111) — 🔴 전건 NULL(미배선). 원천 실재 = SILVER.GA4_EVENT.PAGE_LOCATION 의 memnum= (규모·종수는 문서10 §26). 조회 시 항상 0행
--   · HOMEPAGE_ID: 홈페이지/앱 ID. 원천: TM_MM_FDRM_MBER_INFO.HMPG_ID
--   · CHILD_CODE: 결연아동코드(#122, URL 파싱) — 🔴 전건 NULL(미배선). 원천 실재 = SILVER.GA4_EVENT.PAGE_LOCATION 의 childnum= (규모·종수는 문서10 §26). 조회 시 항상 0행
--   · DW_BATCH_ID: 적재 배치 식별자 = dbt invocation_id (공통감사)
-- ============================================================================
...
--   · CAMPAIGN_TYPE: 캠페인 유형(#17) = 캠페인 카테고리 라벨(SILVER MM294, 56종). 예: 국내사례캠페인·굿즈캠페인·해외캠페인. ⚠️숫자코드 아님(2026-07-16 라벨화)
--   · DOMESTIC_OVERSEAS: 국내/해외(#15) = SILVER CMPGN_TYPE1_NM(MM295): 국내 / 통합 / 해외. (종전 전건 NULL — 2026-07-16 BRONZE 재입고로 채움)
--   · BIZ_CASE_TYPE: 사업/사례(#16) = SILVER CMPGN_TYPE2_NM(MM296): 굿즈 / 기타 / 사례 / 사업. ⚠️종전 모델이 유형1(국내/해외)을 여기 매핑한 의미혼입을 2026-07-16 교정
--   · INFLOW_PATH: 개발인입경로 라벨(SILVER MM293). 예: 디지털·방송·영상광고·지역개발·마케팅콜개발·대면모금·직원개발. 🔴 이 축은 **모집 채널**이다 — 2026-07-16 신설 시 「현업 주요캠페인 분류축」이라 적었던 표기는 거짓이므로 회수한다(O37). 캠페인 카테고리 = CAMPAIGN_TYPE(MM294) · 상위캠페인 = PARENT_CAMPAIGN_
--     NAME
--   · MARKETING_CAMPAIGN: 마케팅캠페인명(SILVER MK_CMPGN_NM). Q16 해소(2026-07-16 신설)
...
--   · MKTG_UTM: UTM 원천코드 — 코드사전이 아니라 SILVER 신설 원천 TM_CM_MKTNG_UTM(MK_UTM)과 연동. 🔴 라벨이 아니다 — 사람이 읽는 이름은 MKTG_UTM_NM.
...
--   [컬럼별 설계 및 실측 이력]
--   · MKTG_CAMPAIGN_SK: 대리키 = gold_sk(MK_CMPGN_CD). 0 = (미매핑) Unknown 멤버
--   · MKTG_CAMPAIGN_BK: 업무키 = 원천 MK_CMPGN_CD (SILVER.CRM_MARKETING_CAMPAIGN)
...
--   · DW_BATCH_ID: 적재 배치 식별자 = dbt invocation_id (공통감사)
--   · SPONSORSHIP_DIV_CD: [O89] 정기일시후원구분 코드 raw ← 원천 TM_CM_SPNSR_BSNS_INFO.SPNSR_DIV_CD (SILVER CRM_SPONSORSHIP.SPNSR_DIV_CD). 코드사전 CM035. 실측 2종(1·2) · 50개 사업 전건 채움. 🔴**최상위 분류축**이다 — 라벨은 SPONSORSHIP_DIV_NAME. ⚠️회비/기부금 지류 구분(FE
--     E_DIV)과 다른 축이다
...
--     PM018 / 12→PM033 / 5→PM019. 매핑 커버리지와 0(비미납 또는 구조적 사유부재 = 수기처리·F코드부재·DIM미존재) 내역은 문서10 §26. ⚠️ 대표 1개로 축약 — 복수사유 분포는 SILVER 직접 조회. ⚠️ 중단사유는 별도 트랙(FME)
...
--   · JOIN_DATE: 캠페인 가입일(#27) 🔴[G군 grain 상이] 전건 NULL 슬롯 유지 — 정본은 FACT_MEMBER_EVENT.JOIN_DATE 또는 DIM_MEMBER_CURRENT.FIRST_JOIN_DATE
--   · STOP_DATE: 가입캠페인 중단일(#26) 🔴[G군 grain 상이] 전건 NULL 슬롯 유지 — 정본은 FACT_MEMBER_EVENT.STOP_DATE 또는 DIM_MEMBER_CURRENT.LAST_STOP_DATE
...
--   · STOP_REASON_NM: 중단사유명 — 정본 공#162. MM005 라벨(SILVER CRM_MEMBER_DISCONTINUE.DSCNTC_RSN_NM 전파). 코드는 STOP_REASON. ⚠️USE_YN 무필터 조인 — 실적재 종 중 일부가 폐지코드이며 필터를 걸면 그 라벨이 사라진다(종수·규모는 문서10 §26). 개발원천 행은 개념 부재로 NULL (O25)
--   · STOP_CHANNEL_NM: 중단경로명 — MM287 라벨(SYSTEM/CRM/홈페이지). 코드는 STOP_CHANNEL(1/2/3). 개발원천 행은 개념 부재로 NULL. 215지표 밖 — 현업 수요 확인 대상 (O25)
--   · AGE_AT_EVENT: 연령대 코드 raw — **사건(개발약정) 시점 값**. 원천 BRONZE TM_MM_FDRM_MBER_DVLP_AMT.AGE(정본 CM014) 사건행별 값을 SILVER CRM_MEMBER_DEV 경유로 무변환 전파. 1=10대 미만 2=10대 3=20대 4=30대 5=40대 6=50대 7=60대 8=70대 9=70대 이상 10=단체 11=기업 12=기타
...
--   · REGION_AT_EVENT: 지역명 — **사건(개발약정) 시점** 지역 라벨(CM018 약칭, 지표 공#131). 코드는 AREA_CD_AT_EVENT. 원천 SILVER CRM_MEMBER_DEV.AREA_NM(CM018 사전 조인) 전파. ⚠️ 센티넬 코드 '0' 은 사전에 라벨이 없어 NULL 이다 — '미상'으로 창작하지 않는다. ⚠️ **현재 거주지가 아니다**(O34). 
...
--   · GENDER_AT_EVENT: 성별명 — **사건(개발약정) 시점** 성별 라벨. CM013 사전 조인(하드코딩 아님, P31). 코드는 SEX_AT_EVENT. 실제값 계열: 국내(남자)·국내(여자)·외국인(남자)·외국인(여자)·외국인(기타)·단체·기업·기타. 🔴 DIM_MEMBER_CURRENT.GENDER_NAME(CM017 라벨·현재 스냅샷)과 **코드체계가 다르다** — 두 축
...
--   [컬럼별 설계 및 실측 이력]
--   · AD_PERF_DK: 행 식별자(GRAIN·PK) MD5(AD_SOURCE_TYPE|ROW_HASH|DUP_SEQ). 위성 3종 조인키. 발급지점=SILVER.AGENCY_AD_ROW_* (DEC-11)
...
--   · AD_COST: 광고비(원) (#6). 원천별 컬럼 상이 — COST_TYPE 은 SILVER 보유
...
--   · GA_CONV_MEMBERS: GA전환수(명) — **DIGITAL 전용**. ⚠️O16 교정 2026-07-28: 종전 REBRDC 개발회원수가 혼입돼 합계의 상당 부분을 차지했다(혼입 비중은 문서10 §26 · 재방송 개발실적은 FACT_AD_BROADCAST.DVLP_MEMBER_CNT 로 이관)
--   · GA_CONV_CNT: GA전환수(건/VU) — **DIGITAL 전용**. ⚠️O16 교정 2026-07-28: 종전 REBRDC 개발건수가 혼입돼 합계의 과반을 차지했다(혼입 비중은 문서10 §26) → FACT_AD_BROADCAST.DVLP_CNT 로 이관. 합계 소수=비건수, 어의 현업확인 잔여(O5)
...
CREATE OR REPLACE TABLE GN_DW.GOLD.FACT_AD_PERFORMANCE (
    ...
    GA_CONV_CNT         NUMBER(18,4)    COMMENT 'GA전환수(건/VU).',
...
--   · EVENT_BK: [DEC-30] degenerate key — 원천 행사키 ← CRM_EVENT_PARTICIPATION.EVENT_KEY(전건 채움). 🔴고아 행사 식별자 보존용: 마스터 부재 행사의 행이 EVENT_SK=0 으로 뭉개져 서로 구별되지 않았다(고아 규모·SILVER 대비 종수는 문서10 §26). 🔷(EVENT_BK,MEMBER_DK,PARTCPT_SEQ
...
--   · EVENT_KIND: 원천 계열 판별 **코드** — 팩트 자체 보유(SILVER.CRM_EVENT_PARTICIPATION.DW_SOURCE_TABLE 분기). 값 2종: EVENT=일반행사 원천 · CRMN=캠페인행사 원천. 🔴DIM_EVENT.EVENT_KIND 와 어휘가 같지만 **조인과 무관하게 전건 값을 갖는다** — 행사 미매칭(EVENT_SK=0) 구간에서도 계열
...
--   · BILLED_AMT: 청구액(원) = SUM(RQEST_AMT). FMM·SILVER 와 총계 일치 실측(GATE-D · 값은 문서10 §26)
...
--     건으로 명시」와 「목표 미입력(원천 NULL)」을 함께 담는다 — 구분이 필요하면 FACT_TARGET_DEV.GOAL_CNT IS NULL 로 팩트에서 본다
