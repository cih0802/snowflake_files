...
--------------------------------------------------------------------------------
  ...
  6. FACT_BIGQUERY_BEHAVIOR 의 비가산 지표(BOUNCE_RATE·AVG_SESSION_DURATION 등)는
     그레인 기준 값 — 상위 레벨 재합산 금지(테이블 COMMENT 명시).
  7. [적재 상태] CRM·BIGQUERY·ERP·AGENCY SILVER→GOLD 적재 완료(27테이블 + WIDE VIEW 12개).
...
CREATE OR REPLACE TABLE GN_DW.GOLD.DIM_MEMBER_ACQUISITION (
    ...
    ACQ_DEPARTMENT_AT_ACQ   VARCHAR(300)    COMMENT '회원 획득 당시 부서명 (Snapshot 기반 시점 동결).',
...
--   [컬럼별 설계 및 실측 이력]
--   · MEMNUM: memnum(#111) — 🔴 전건 NULL(미배선). 원천 실재 = SILVER.BIGQUERY_EVENT.PAGE_LOCATION 의 memnum= (규모·종수는 문서10 §26). 조회 시 항상 0행
--   · HOMEPAGE_ID: 홈페이지/앱 ID. 원천: TM_MM_FDRM_MBER_INFO.HMPG_ID
--   · CHILD_CODE: 결연아동코드(#122, URL 파싱) — 🔴 전건 NULL(미배선). 원천 실재 = SILVER.BIGQUERY_EVENT.PAGE_LOCATION 의 childnum= (규모·종수는 문서10 §26). 조회 시 항상 0행
--   · DW_BATCH_ID: 적재 배치 식별자 = dbt invocation_id (공통감사)
-- ============================================================================
...
--   · SPONSORSHIP_SK: 후원사업 (FK→DIM_SPONSORSHIP, SILVER.CRM_MEMBER_DEV.SPNSR_BSNS_ID 경유). 🔴 **DEV 사건 전용 배선이다** — 개발(DEV) 행은 전건 실배선(원천 채움 100% · DIM_SPONSORSHIP 고아 0)이고
