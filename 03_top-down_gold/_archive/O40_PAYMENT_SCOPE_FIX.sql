...
--   §1 컬럼 2종 + §1-C COMMENT 는 **정본 `06_DDL.sql` 442·443·468 에 접혔다**.
--   물리 실측(2026-08-05 재구축 후): `FMM.PAID_FEE_BILLABLE`·`UNPAID_BILLED_AMT`·`HAS_BILLING` 존재.
--   §2 build · §4 SV metric 4종 배포도 2026-08-05 완료(이슈원장 O40 절).
--
-- 🔴 **재실행이 사문화 문구를 되살린다** — §1-B COMMENT 2건에 「**적재 대기**: dbt build 전이면
--   전건 NULL」이 들어 있다. build 는 이미 끝났고 정본에서 그 문구를 제거했다. 실행하면
--   정상 지표에 "대기 중" 이 부활한다 — O28·O29 에서 겪은 사문화 주석 유형이며
--   O40 자신이 §4 말미에서 경고했던 바로 그 함정이다.
--
-- 🟢 살아 있는 자산 = **§3 검증 5종**(3-A 적재 · 3-B 상한 · 3-C 음수 · 3-D 정합 · 3-E BRONZE 독립대조).
--   재적재·build 후 납부율 회귀 검증에 그대로 쓸 수 있다.
--   ⚠️ 기대값(2025 175,381,890,496 · 29,251,314,636 · 85.65%)은 재구축 이전 측정치다 —
--      BRONZE 재적재본이 동일 스냅샷인지 확인 전에는 합불 판정 근거로 쓰지 말 것(PROC-3 c).
-- ============================================================================
