...
--   §1·§2 FACT COMMENT 3컬럼은 **정본 `06_DDL.sql` 491·493·578 에 접혔다**.
--   §3 WIDE 분은 모델 `post_hook` 이 소유 — `WIDE_MEMBER_EVENT.sql`·`WIDE_SERVICE_EVENT.sql`
--   에 동일 문구 실측 확인. 교정 제외분(`FMM.DEV_MEMBERS` = '개발(명) COUNT')도 훼손 없음.
--   ⇒ 재구축 + `dbt build` 만으로 전량 복원된다.
--
-- 🟢 §0 측정근거 · §4 검증 3관문은 살아 있는 자산이다.
--   ⚠️ 기대값(2,291,878 · 1,038,262 · 38,470,780)은 2026-08-05 재구축 **이전** 측정치이고
--      현재 BRONZE 가 미적재라 재검증하지 못했다 → 재적재 후 재측정(PROC-3 c).
--
-- ⏸ **§5 잔여는 미해소**: O39-B(전건 0 컬럼군 · `ACTIVE_CNT` 영향범위 선결) ·
--   O39-C(`*_MEMBERS`→`*_FLAG` 개명 여부 · 사용자 결정 대기).
-- ============================================================================
