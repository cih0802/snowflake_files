...
--   §1 FACT/테이블 COMMENT · §2 O29 단위 경고는 **정본 `06_DDL.sql` 에 접혔다**
--   (694 = FAB.DURATION_SEC · 777~803 = FEP 카운트·PART_STATUS·테이블 COMMENT).
--   §2-B WIDE 분은 **모델 `post_hook` 이 소유**한다 —
--   `10_dbt_pipeline/models/gold/wide/WIDE_EVENT_PARTICIPATION.sql`·`WIDE_AD_BROADCAST.sql`
--   에 회수본(`EVENT_KIND` 거짓문구 정정 포함)이 실려 있음을 실측 확인했다.
--   ⇒ 재구축 + `dbt build` 만으로 전량 복원된다.
--
-- 🔴 **§3-3 이 잡으려던 거짓문구가 아직 한 곳에 남아 있다**(2026-08-05 발견):
--   `03_top-down_gold/10_WIDE VIEW 코멘트.sql:507`
--     `COLUMN EVENT_KIND COMMENT 'DIM_EVENT.EVENT_KIND — 온라인/오프라인'`
--   실제 도메인은 `EVENT`/`CRMN` 이라 이 문구는 거짓이고, 이 파일로 COMMENT 를 재생성하면
--   `=''온라인''` 0행 무증상 오답이 부활한다. → O41 잔여 조치.
--
-- 🟢 살아 있는 자산 = **§0 측정근거 · §3 검증 6종 · §4 잔여 10건**(현업 회신·SILVER 파싱 등 미결).
--   ⚠️ §4 의 3·4·6·8 은 여전히 미해소다 — 이 파일을 아카이브했다고 잔여가 닫힌 것이 아니다.
-- ============================================================================
