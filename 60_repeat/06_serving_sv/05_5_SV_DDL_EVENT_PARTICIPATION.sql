...
CREATE OR ALTER SEMANTIC VIEW GN_DW.SERVING.SV_EVENT_PARTICIPATION
  ...
  COMMENT = 'Phase-1 행사 참여 SV(base FEP). [원천 요약] 원천시스템=CRM(eCRM 행사관리) · BRONZE=GN_DW.BRONZE_CRM(행사 TM_MS_EVENT∪TM_MS_CRMN · 참여 TD_MS_EVENT_PRTCPNT_DTL∪TD_MS_CRMN_PRTCPNT) → SILVER(CRM_EVENT+CRM_EVENT_PARTICIPATION) → GOLD(FEP). 테이블별 상세 원천은 각 테이블 COMMENT의 [원천] 절 참조. 활성: 참여자수·참여건수·고유 참여회원수, 행사명/종류·참여일별 · **행사구분 라벨** · **참여 상태·경로·채널 라벨**. ⚠행사 미매칭분이 Unknown(EVENT_SK=0)으로 라우팅되므로 행사명별 집계는 부분집합이다 — 확정치로 단정 금지. 🔴🔴 **이 팩트는 두 원천의 결합이며 코드·라벨 축이 원천별로 갈린다**(일반행사 / 캠페인행사) — 구분·상태·경로 축으로 분해할 때는 **계열 판별자 동반이 필수**다. 🟢🟢 **동반 축은 팩트가 보유한 `PART_EVENT_KIND_NAME`** 이다(2026-08-12 O61 · D2) — 팩트가 원천 분기로 보유해 **전건 값을 가지며 사각지대가 없다.** 🔴 **차원축 `EVENT_KIND_NAME` 을 판별자로 쓰지 말 것** — 그것은 행사 마스터에서 오므로 행사 미매칭 구간이 ''(미매핑)'' 이 되어 계열을 알려주지 못한다(종전 처방이 이 사각지대를 갖고 있었다 · 경위 = 원장 §O59-S ③). ⚠️ 계열이 확정되는 것과 **어느 행사인지**가 확정되는 것은 다르다 — 행사 속성별 집계는 여전히 부분집합이다. 🔴 특히 **같은 의미가 두 라벨로 나오는 축이 있다**(참여경로의 homepage/홈페이지 · 참여상태의 영문/한글 계열) — 사전 라벨이 원천별로 다르게 등재된 결과이며 통합 라벨을 창작하지 않는다(DEC-17-B). 🔴 **두 원천은 「참여」의 정의 자체가 다르다** — 일반행사는 다단계 통과 여부,
캠페인행사는 신청/참여/불참이다. 비활성(적재 대기): 모집/총원, 캠페인/후원사업별. 🔴 이 목록은 축을 활성화할 때마다 회수 대상이다(P61).'
  AI_SQL_GENERATION '적용 조건: 질문에 기간(연/월)과 그룹(행사종류·행사명·회원구분 등)이 모두 없을 때만. 이 경우 전체 기간 풀스캔을 피해 데이터에 실제 존재하는 최신 연월(MAX(연월)) 기준 직전 12개월로 한정하고(기준월은 CURRENT_DATE가 아니라 데이터 최신월 — 미래연월 데이터도 최신월로 인정), GROUP BY ROLLUP((연,월))로 월별 행 + 전체 총계 행을 함께 반환해 월별 추이와 총계를 동시에 제공한다. 행사명별 집계를 낼 때는 미매칭분이 Unknown으로 빠져 부분집합임을 답변에 명시한다. 사용자가 기간/그룹을 지정하거나 합계·총액만 원하면 그 요청을 우선한다. 핵심 규칙: (1) 🔴 **코드축이 아니라 라벨축으로 답한다** — 「행사 구분별」은 EVENT_CATEGORY_NAME, 「참여 상태별」은 PART_STATUS_NAME, 「참여·신청 경로별」은 PART_PATH_NAME, 「참여 채널별」은 PART_CHANNEL_NAME 을 쓴다. 코드축(EVENT_CATEGORY·PART_STATUS·PART_PATH·PART_CHANNEL)은 필터 재현·원천 대조용이며 현업 답변에 코드를 그대로 내지 않는다. (2) 🔴🔴 **이 SV 의 코드·라벨 축은 두 원천이 섞여 있어 계열 판별자 동반이 필수다** — 참여 팩트는 일반행사(EVENT)와 캠페인행사(CRMN) 두 원천의 결합이고 **원천마다 코드군이 다르다.** 판별자 없이 이 축들로 그루핑하면 서로 다른 체계를 한 표에 섞는다. ✅ **동반할 축은 PART_EVENT_KIND_NAME**(팩트 보유)이다. (2-B) 🔴🔴 **왜 PART_EVENT_KIND_NAME 인가 — 차원축 EVENT_KIND_NAME 을 쓰지 말 것.** `EVENT_KIND_NAME` 은 **행사 마스터(차원)에서 오는 값**이라 참여 행이 행사에 매칭되지 않은 구간에서는 ''(미매핑)'' 이 되어 **계열을 알려주지 못한다**(그 비중은 작지 않다). 반면
`PART_EVENT_KIND_NAME` 은 팩트가 **원천 분기로 보유**해 전건 값을 가지므로 미매칭 구간에서도 계열이 확정된다. ⇒ **계열 분해·계열 동반은 항상 PART_EVENT_KIND_NAME 으로 한다.** ⚠️ 단 **행사명·행사구분 등 차원 속성별 집계는 여전히 부분집합**이다 — 계열이 확정되는 것과 어느 행사인지가 확정되는 것은 다르다. 차원축으로 분해해 ''(미매핑)'' 버킷이 나오면 **별도 행으로 분리해 제시**하고 코드 범위로 계열을 짐작해 단정하지 않는다. (3) 🔴🔴 **라벨공간이 원천별로 갈린 축이 있다 — 같은 의미가 두 라벨로 나온다.** PART_PATH_NAME 은 ''homepage''(일반행사)와 ''홈페이지''(캠페인행사)가 동시에 존재하고, PART_STATUS_NAME 은 영문 계열(Success·n_step_*=일반행사)과 한글 계열(신청·참여·불참=캠페인행사)로 갈린다. **두 라벨을 임의로 통합하지 않는다**(사전에 없는 통합 라벨을 만들면 라벨 창작이다) — 갈라진 채로 PART_EVENT_KIND_NAME 과 함께 제시하고, 사용자가 합친 값을 원하면 **어떤 라벨들을 합쳤는지 명시**한다. (4) 🔴 **두 원천은 「참여」의 정의가 다르다** — 일반행사(MS304)의 상태는 **다단계 이벤트의 단계 통과 여부**이고 캠페인행사(MS006)는 신청/참여/불참이다. 두 계열의 「참여」를 합산해 하나의 참여율로 제시하지 말 것. (5) 🔴 **PART_CHANNEL 계열의 NULL 은 두 가지가 섞여 있다 — 하나로 설명하지 말 것**: ㉠ 캠페인행사 원천에는 채널 컬럼이 아예 없어 그 참여 행 **전량**이 NULL 이다(**구조적 부재** · 원천 스캔 확인) ㉡ 일반행사 계열의 **소수 행**도 NULL 이며 그쪽은 **결측 또는 오염값**이다. 채널별 분해에는 ㉠을 반드시 밝히고, 남는 NULL 을 「전부 캠페인행사」라고 단정하지 않는다. (6) ⚠️ **오염값 행이 있다** — 참여 3축에
원천 입력 오류값이 소수 존재하며 라벨이 NULL 이다. 정상 코드로 취급하거나 의미를 부여하지 말고, 라벨 NULL 버킷을 밝힐 때 함께 언급한다.';
