...
CREATE OR ALTER SEMANTIC VIEW GN_DW.SERVING.SV_SERVICE
  ...
  COMMENT = 'Phase-1 서비스 발송 SV(base FSE). [원천 요약] 원천시스템=CRM(UMS 발송) · BRONZE=GN_DW.BRONZE_CRM(TM_MS_EMAIL/MSG_AT/PSTMTR_SNDNG + TD_MS_*_SNDNG_DTLS + SND_REQ_MST) → SILVER(CRM_SEND_*) → GOLD(FSE). 테이블별 상세 원천은 각 테이블 COMMENT의 [원천] 절 참조. 활성: 발송수·고유 발송회원수, 서비스구분/채널/발송일별 · **발송상태 라벨(축A)** · **통신사 도달결과 라벨(축B)**. 시간=전체가능. 🔴🔴 **발송 결과는 두 축이며 서로 다른 것을 뜻한다**(DEC-35 §23-G): **축A = 우리 발송 시스템의 상태**(SEND_STATUS/_NAME · 채널별로 코드체계가 다르므로 CHANNEL 동반 필수 · 라벨은 MSG_AT 계열만) · **축B = 통신사가 반환한 도달 결과**(SEND_RESULT_CD/_GROUP/_NAME · 채널 간 conformed). 두 축을 합산하거나 한 표에 섞지 말고 어느 축으로 답했는지 밝힌다. 🔴 **「성공/실패」를 전 채널 기준으로 단정하지 말 것** — EMAIL·SND 는 사전 라벨이 없어 의도적 NULL 이고 PSTMTR 은 원천 자체가 없다(문서20 §M-4). 비활성(적재 대기): 성공/실패/오픈 **measure**(FSE 의 SUCCESS/FAIL/OPEN 컬럼은 전건 미적재 — 다만 위 축B 라벨로 **도달 결과 분해는 산출된다**), 서신/선물금/증액 참여·+5일 코호트(신31~53), 캠페인별. 🔴 이 목록은 축을 활성화할 때마다 회수 대상이다(P61 — 부정형 서술이 남으면 Agent 가 "불가"로 답한다). 🔴🔴 **[2026-08-13 O67 발행] 보고서 필드 조립 판정 4종 중 이 SV 소관분**(정본 = 산출물 `09_보고서필드_조립가능성`): ① **배분규칙필요 — 이 SV 소관분이 가장 많다.**
서비스효과성분석·증액마케팅·개발메일결과 보고가 **발송을 앵커로 두고** 회원-월 팩트의 「개발(건)·개발(명)·중단(건)」을 함께 요구한다. 발송은 발송 건 grain, 그 measure 는 회원-월 grain 이므로 **굵은 값을 발송 행마다 내리면 복제된다** ⇒ 배분(귀속) 규칙이 필요하고 그것은 모델링이 아니라 **업무 판단**이다(O47) ⇒ **SQL 을 만들지 말고 사유를 답한다.** 🟢 발송 규모만 묻는 질문은 이 SV 로 정상 답변한다. ② **앵커_경합 — 앱참여수치분석 좌측 섹션은 이 SV 와 GA 행동 팩트가 동수 경합**이다. 🔴 **GA 쪽에는 SV 가 없다**(배포된 SV 목록에 GA 팩트가 없다) ⇒ 이 섹션은 **SV 층에서 절반만 답할 수 있다**: 발송·회원 축은 이 SV 로 답하고 **GA 지표는 GOLD 뷰(`WIDE_GA_BEHAVIOR`) 직접 조회로 안내**한다. 🔴 존재하지 않는 GA SV 를 가리키지 말고, 한쪽만으로 섹션 전체를 답한 것처럼 서술하지 않는다(원천도 CRM ↔ GA4 로 달라 교차 금지다).'
  AI_SQL_GENERATION '적용 조건: 질문에 기간(연/월)과 그룹(채널·서비스유형·회원구분 등)이 모두 없을 때만. 이 경우 전체 기간 풀스캔을 피해 데이터에 실제 존재하는 최신 연월(MAX(연월)) 기준 직전 12개월로 한정하고(기준월은 CURRENT_DATE가 아니라 데이터 최신월 — 미래연월 데이터도 최신월로 인정), GROUP BY ROLLUP((연,월))로 월별 행 + 전체 총계 행을 함께 반환해 월별 추이와 총계를 동시에 제공한다. 사용자가 기간/그룹을 지정하거나 합계·총액만 원하면 그 요청을 우선한다. 핵심 규칙: (1) 🔴 **발송상태를 묻는 질문은 코드축이 아니라 라벨축으로 답한다** — 「발송상태별」은 SEND_STATUS_NAME, 「통신사 결과·실패 사유별」은 SEND_RESULT_NAME 을 쓴다. 코드축(SEND_STATUS·SEND_RESULT_CD)은 필터 재현·원천 대조용으로만 쓰고 현업 답변에 코드를 그대로 내지 않는다. (2) 🔴🔴 **축A 와 축B 를 섞지 않는다** — 축A(SEND_STATUS_NAME)는 우리 발송 시스템 상태, 축B(SEND_RESULT_NAME)는 통신사 도달 결과다. 두 축의 건수를 합산하거나 한 표에 나란히 세우지 말고, 어느 축으로 답했는지 문장에 명시한다. (3) 🔴 **라벨 NULL 은 결측이 아니다** — 축A 라벨은 MSG_AT 계열만 채워지고(EMAIL·SND 는 사전 라벨 부재 · PSTMTR 은 원천 부재), 축B 는 사전에 없는 통신사 코드 행이 NULL 이다. 라벨축으로 분해할 때는 **NULL 버킷을 반드시 함께 표기**하고, 성공률·도달률의 분모를 라벨 보유 행으로만 잡았다면 그 사실을 밝힌다. (4) 🔴 **코드축 단독 필터 금지** — SEND_STATUS·SUBTYPE 은 채널별로 코드체계가 달라서 CHANNEL 동반 없이 필터하면 서로 다른 체계를 섞는다. 축B 는 SEND_RESULT_GROUP(메시지 타입 판별자)을 동반한다. (5) **SUBTYPE
은 라벨이 없다** — 코드군을 특정할 수 없어 라벨을 만들지 않았다(등급 D). 의미를 창작하지 말고 코드값 + 채널을 함께 제시하고 라벨 부재를 밝힌다(문서20 §M-2). (6) 🔴🔴 **[2026-08-13 O67] 판정 4종**: 발송을 앵커로 두고 「개발(건)·개발(명)·중단(건)」을 함께 요구하는 요청(**배분규칙필요**)은 **SQL 을 생성하지 말고** 배분 규칙이 필요한 사안임을 답한다 — 회원-월 measure 를 발송 행에 내리면 복제된다. 앱참여수치분석 좌측 섹션(**앵커_경합**)은 발송·회원 축만 이 SV 로 답하고 **GA 지표는 `GOLD.WIDE_GA_BEHAVIOR` 직접 조회로 안내**한다(GA 팩트에는 SV 가 없다) — 두 원천을 한 표에 합치지 않는다.';
