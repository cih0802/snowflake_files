...
-- [재설계] 모든 Semantic View는 GN_DW.GOLD View만 참조 (BRONZE 직접참조 없음)
--          BRONZE 직접참조하던 3개 SV(MARKETING/AD_PLATFORM/WEB_APP)를 GOLD 정형화 View 경유로 변경
--          VQR 내부 SQL 경로도 GN_DW.GOLD.* 로 치환
------------------------------------------------------
...
CREATE OR REPLACE SEMANTIC VIEW SV_PAYMENT_ANALYSIS
	tables (
		PAYMENT as GN_DW.GOLD.V_PAYMENT_ANALYSIS comment='납입이력+캠페인+회원특성 통합.'
	)
	facts (
...
SQL 'SELECT "상위캠페인명", SUM("납입금액") AS "총납입금액", AVG("납입금액") AS "평균납입금액" FROM GN_DW.GOLD.V_PAYMENT_ANALYSIS WHERE "청구연도" = ''2025'' AND "납입금액" > 0 AND "상위캠페인명" IS NOT NULL GROUP BY "상위캠페인명" HAVING COUNT(*) >= 10 ORDER BY "총납입금액" DESC'), 
		"납입회비_상위10%_캠페인" AS ( 
QUESTION '2025년 납입회비가 높았던 상위 10% 캠페인은 어떤 유입경로, 타겟 조합을 가졌는가?' 
ONBOARDING_QUESTION false
SQL 'WITH ranked AS (SELECT "상위캠페인명", SUM("납입금액") AS "총납입금액", PERCENT_RANK() OVER (ORDER BY SUM("납입금액") DESC) AS pct FROM GN_DW.GOLD.V_PAYMENT_ANALYSIS WHERE "청구연도" = ''2025'' AND "납입금액" > 0 AND "상위캠페인명" IS NOT NULL GROUP BY "상위캠페인명" HAVING COUNT(*) >= 10) SELECT * FROM ranked WHERE pct <= 0.10 ORDER BY "총납입금액" DESC'),
		"미납비중_높은_세부캠페인" AS ( 
QUESTION '2025년 미납비중이 가장 높은 세부캠페인 상위 3개와 해당 캠페인들의 회원특성(연령대,성별,지역)은?' 
ONBOARDING_QUESTION false
SQL 'WITH top3 AS (SELECT "세부캠페인명", SUM("미납금액") AS "총미납", SUM("청구금액") AS "총청구", ROUND(SUM("미납금액")*100.0/NULLIF(SUM("청구금액"),0),1) AS "미납비중" FROM GN_DW.GOLD.V_PAYMENT_ANALYSIS WHERE "청구연도" = ''2025'' AND "청구금액" > 0 AND "상위캠페인명" IS NOT NULL GROUP BY "세부캠페인명" HAVING COUNT(*) >= 50 ORDER BY "미납비중" DESC LIMIT 3) SELECT t."세부캠페인명", t."미납비중", p."연령대", p."성별", p."지역", COUNT(*) AS "건수" FROM top3 t JOIN GN_DW.GOLD.V_PAYMENT_ANALYSIS p ON t."세부캠페인명" = p."세부캠페인명" WHERE p."청구연도" = ''2025'' GROUP BY t."세부캠페인명", t."미납비중", p."연령대", p."성별", p."지역" ORDER BY t."세부캠페인명", "건수" DESC'),
		"월별_납입추이" AS ( 
QUESTION '2025년 월별 납입금액 추이는?'
ONBOARDING_QUESTION false
SQL 'SELECT "회비청구월", SUM("납입금액") AS "총납입금액", COUNT(DISTINCT "회원번호") AS "회원수" FROM GN_DW.GOLD.V_PAYMENT_ANALYSIS WHERE "청구연도" = ''2025'' AND "납입구분" = ''납입'' GROUP BY "회비청구월" ORDER BY "회비청구월"'), 
		"상위캠페인별_납입회비" AS ( 
QUESTION '2025년 상위캠페인별 납입회비금액을 분석해줘' 
ONBOARDING_QUESTION false
SQL 'SELECT "상위캠페인명" AS "상위캠페인", SUM("납입금액") AS "총납입금액(원)", AVG("납입금액") AS "평균납입금액(원)", COUNT(DISTINCT "회원번호") AS "회원수" FROM GN_DW.GOLD.V_PAYMENT_ANALYSIS WHERE "청구연도" = ''2025'' AND "상위캠페인명" IS NOT NULL AND "납입금액" > 0 GROUP BY "상위캠페인명" ORDER BY "총납입금액(원)" DESC'), 
		"공통브랜드별_신규회원_납입" AS ( 
QUESTION '2025년 공통브랜드별 신규회원의 총납입금액을 알려줘'
ONBOARDING_QUESTION false
SQL 'SELECT "공통브랜드명", SUM("납입금액") AS "총납입금액", COUNT(DISTINCT "회원번호") AS "회원수" FROM GN_DW.GOLD.V_PAYMENT_ANALYSIS WHERE "청구연도" = ''2025'' AND "개발구분" = ''신규'' AND "공통브랜드명" IS NOT NULL GROUP BY "공통브랜드명" ORDER BY "총납입금액" DESC'), 
		"공통상위캠페인별_납입분석" AS ( 
QUESTION '2025년 공통상위캠페인명별 총납입금액과 평균납입금액을 알려줘'
ONBOARDING_QUESTION false
SQL 'SELECT "공통상위캠페인명", SUM("납입금액") AS "총납입금액", AVG("납입금액") AS "평균납입금액", COUNT(DISTINCT "회원번호") AS "회원수" FROM GN_DW.GOLD.V_PAYMENT_ANALYSIS WHERE "청구연도" = ''2025'' AND "납입금액" > 0 AND "공통상위캠페인명" IS NOT NULL GROUP BY "공통상위캠페인명" ORDER BY "총납입금액" DESC'), 
		"국내해외_납입비교" AS ( 
QUESTION '2025년 국내해외구분별 총납입금액과 미납비중을 비교해줘'
ONBOARDING_QUESTION false
SQL 'SELECT "국내해외구분", SUM("납입금액") AS "총납입금액", SUM("청구금액") AS "총청구금액", ROUND(SUM("미납금액")*100.0/NULLIF(SUM("청구금액"),0),1) AS "미납비중(%)", COUNT(DISTINCT "회원번호") AS "회원수" FROM GN_DW.GOLD.V_PAYMENT_ANALYSIS WHERE "청구연도" = ''2025'' GROUP BY "국내해외구분" ORDER BY "총납입금액" DESC') 
	)
...
CREATE OR REPLACE SEMANTIC VIEW SV_MEMBER_LIFECYCLE
	tables (
		DISCONTINUED as GN_DW.GOLD.V_DISCONTINUED_DETAIL comment='중단회원 상세. 유지일수/유지개월수 포함. 가입일자/중단일자 DATE컬럼 및 주차(WEEKOFYEAR) 포함.',
		DISC_PAYMENT as GN_DW.GOLD.V_DISCONTINUED_PAYMENT_ANALYSIS comment='중단회원 미납이력 교차분석. 유지기간구간별 중단사유와 미납비율 분석 가능.',
		CONVERSION as GN_DW.GOLD.V_TEMP_MEMBER_CONVERSION comment='일시회원→정기회원 전환.'
	)
	facts (
	...
	dimensions (
		...
		DISCONTINUED.JOIN_DATE as "가입일자" with synonyms=('가입일자','가입날짜') comment='가입일자(DATE 타입, YYYY-MM-DD).',
		DISCONTINUED.DISC_DATE as "중단일자" with synonyms=('중단일자','중단날짜') comment='중단일자(DATE 타입, YYYY-MM-DD).',
		DISCONTINUED.DISC_WEEK as "중단주차_연도내" with synonyms=('중단주차','주차') comment='중단일의 해당연도 내 주차번호(WEEKOFYEAR).',
		DISCONTINUED.MEMBER_CLASS as "회원분류" with synonyms=('회원분류','신규기존') comment='회원분류(신규/기존).',
	...
	comment='회원 라이프사이클 분석. 중단회원 상세(유지기간/캠페인별/사유/미납이력). 일시→정기 전환 분석.'
	ai_verified_queries (
		"고려인_캠페인_유지기간" AS ( 
QUESTION '2025년 중단회원 중 고려인 캠페인으로 가입한 회원들의 가입 후 유지기간은?'
ONBOARDING_QUESTION false
SQL 'SELECT "세부캠페인명", COUNT(*) AS "중단건수", ROUND(AVG("유지개월수"),1) AS "평균유지개월수", ROUND(AVG("유지일수"),0) AS "평균유지일수" FROM GN_DW.GOLD.V_DISCONTINUED_DETAIL WHERE "중단연도" = ''2025'' AND "세부캠페인명" LIKE ''%고려인%'' GROUP BY "세부캠페인명" ORDER BY "평균유지개월수" DESC'), 
		"중단사유별_현황" AS ( 
QUESTION '2025년 중단사유별 중단회원수는?' 
ONBOARDING_QUESTION false
SQL 'SELECT "중단사유", COUNT(*) AS "중단건수", ROUND(AVG("유지개월수"),1) AS "평균유지개월수" FROM GN_DW.GOLD.V_DISCONTINUED_DETAIL WHERE "중단연도" = ''2025'' GROUP BY "중단사유" ORDER BY "중단건수" DESC'),
		"주간_중단리포트" AS ( 
QUESTION '2026년 1월 2주차 중단회원 리포트를 보여줘' 
ONBOARDING_QUESTION false
SQL 'SELECT "회원분류", COUNT(*) AS "중단건수", ROUND(AVG("후원금액"),0) AS "평균후원금액", ROUND(AVG("유지개월수"),1) AS "평균유지개월수" FROM GN_DW.GOLD.V_DISCONTINUED_DETAIL WHERE "중단연도" = ''2026'' AND "중단월" = ''202601'' AND "중단주차_연도내" = 2 GROUP BY "회원분류"'), 
		"구간별_중단사유_미납" AS ( 
QUESTION '고려인 캠페인 유지기간 구간별 중단사유, 미납이력에 차이가 있나?' 
ONBOARDING_QUESTION false
SQL 'SELECT "유지기간구간", "중단사유", COUNT(*) AS "중단건수", ROUND(AVG("미납비율"),1) AS "평균미납비율", ROUND(AVG("미납건수"),1) AS "평균미납건수" FROM GN_DW.GOLD.V_DISCONTINUED_PAYMENT_ANALYSIS WHERE "중단연도" = ''2025'' AND "세부캠페인명" LIKE ''%고려인%'' GROUP BY "유지기간구간", "중단사유" ORDER BY "유지기간구간", "중단건수" DESC')
	)
...
------------------------------------------------------
-- SV_MEMBER_DEVELOPMENT
------------------------------------------------------
CREATE OR REPLACE SEMANTIC VIEW SV_MEMBER_DEVELOPMENT
	tables (
		DEV_DETAIL as GN_DW.GOLD.V_MEMBER_DEV_DETAIL comment='회원 개발 상세이력.',
		DISCONTINUATION as GN_DW.GOLD.V_DISCONTINUATION_REPORT comment='중단회원.',
		RETENTION as GN_DW.GOLD.V_RETENTION_BY_PERIOD comment='캠페인별 유지율. 중단테이블 교차 반영. 가입연도 필터 가능.'
	)
	facts (
		DEV_DETAIL.AMOUNT as "금액" with synonyms=('금액','약정금액') comment='약정금액(원).',
		DEV_DETAIL.DEV_PERF as "개발실적_건" with synonyms=('개발건','개발건수','개발실적') comment='개발실적(건) = 후원금액 ÷ 10,000. 이미 건수 환산 완료된 값.',
		...
		RETENTION.RET_3M as "3개월유지율" with synonyms=('3개월유지율') comment='3개월유지율(%).',
		RETENTION.RET_AVG_MONTHS as "평균활동개월수" with synonyms=('평균활동개월수') comment='평균활동개월수.',
		RETENTION.RET_CURRENT as "현재유지율" with synonyms=('현재유지율') comment='현재유지율(%).',
	...
	dimensions (
		...
		DEV_DETAIL.COMMON_BRAND_NAME as "공통브랜드명" with synonyms=('공통브랜드','공통브랜드명') comment='공통브랜드명.',
		DEV_DETAIL.COMMON_PARENT_CAMPAIGN as "공통상위캠페인명" with synonyms=('공통상위캠페인','공통상위캠페인명','공통캠페인') comment='공통상위캠페인명.',
		DEV_DETAIL.DEPT_NAME as "실적부서명" with synonyms=('부서','실적부서명') comment='실적부서명.',
		DEV_DETAIL.DEV_TYPE as "개발구분" with synonyms=('개발구분','개발유형','신규','재후원','증액') comment='신규/증액/재후원/후원중단/감액.',
	...
	comment='회원개발 종합분석. 유지율은 중단테이블 교차 반영. 가입연도 필터 필수. 개발실적_건 = 후원금액÷10000(이미 환산완료).'
	ai_verified_queries (
		"캠페인별_신규개발" AS ( 
QUESTION '2025년 상위캠페인별 신규 개발건수는?'
ONBOARDING_QUESTION false
SQL 'SELECT "상위캠페인명", COUNT(*) AS "개발건수" FROM GN_DW.GOLD.V_MEMBER_DEV_DETAIL WHERE "개발구분" = ''신규'' AND "신청월" LIKE ''2025%'' GROUP BY "상위캠페인명" ORDER BY "개발건수" DESC'),
		"유지율_높은_캠페인" AS ( 
QUESTION '2025년 가입 회원 중 평균 활동기간이 가장 긴 캠페인과 짧은 캠페인은?' 
ONBOARDING_QUESTION false
SQL 'SELECT "상위캠페인명", "총가입자수", "중단자수", "현재유지율", "평균활동개월수", "12개월유지율" FROM GN_DW.GOLD.V_RETENTION_BY_PERIOD WHERE "가입연도" = ''2025'' AND "총가입자수" >= 10 ORDER BY "평균활동개월수" ASC LIMIT 10'), 
		"개발건_이미환산" AS ( 
QUESTION '개발건수를 후원금액/10000으로 계산해줘'
ONBOARDING_QUESTION false
SQL 'SELECT "상위캠페인명", SUM("개발실적_건") AS "개발건수(건)" FROM GN_DW.GOLD.V_MEMBER_DEV_DETAIL WHERE "개발구분" IN (''신규'',''증액'',''재후원'') AND "신청월" LIKE ''2025%'' GROUP BY "상위캠페인명" ORDER BY "개발건수(건)" DESC'), 
		"공통브랜드별_신규개발" AS ( 
QUESTION '2025년 공통브랜드별 신규회원 개발건수를 보여줘'
ONBOARDING_QUESTION false
SQL 'SELECT "공통브랜드명", COUNT(*) AS "개발건수", SUM("개발실적_건") AS "개발실적(건)", SUM("금액") AS "총금액" FROM GN_DW.GOLD.V_MEMBER_DEV_DETAIL WHERE "개발구분" = ''신규'' AND "신청월" LIKE ''2025%'' AND "공통브랜드명" IS NOT NULL GROUP BY "공통브랜드명" ORDER BY "개발건수" DESC'),
		"공통상위캠페인별_LTV" AS ( 
QUESTION '2025~2026년 1분기 공통상위캠페인별 LTV를 산출해줘' 
ONBOARDING_QUESTION false
SQL 'SELECT "공통상위캠페인명", SUM("금액") AS "총개발금액", SUM("개발실적_건") AS "총개발건수", AVG("금액") AS "평균후원금액" FROM GN_DW.GOLD.V_MEMBER_DEV_DETAIL WHERE "개발구분" IN (''신규'',''증액'',''재후원'') AND "신청월" >= ''202501'' AND "신청월" <= ''202603'' AND "공통상위캠페인명" IS NOT NULL GROUP BY "공통상위캠페인명" ORDER BY "총개발금액" DESC'),
		"공통상위캠페인별_유지율" AS ( 
QUESTION '2025~2026년 공통상위캠페인별 평균활동개월수와 현재유지율을 보여줘' 
ONBOARDING_QUESTION false
SQL 'SELECT "공통상위캠페인명", SUM("총가입자수") AS "총가입자수", ROUND(AVG("현재유지율"),1) AS "현재유지율", ROUND(AVG("평균활동개월수"),1) AS "평균활동개월수" FROM GN_DW.GOLD.V_RETENTION_BY_PERIOD WHERE "가입연도" IN (''2025'',''2026'') AND "공통상위캠페인명" IS NOT NULL GROUP BY "공통상위캠페인명" ORDER BY "총가입자수" DESC'),
		"국내해외별_개발현황" AS ( 
QUESTION '국내해외구분별 신규 개발건수와 평균후원금액을 비교해줘'
ONBOARDING_QUESTION false
SQL 'SELECT "국내해외구분", COUNT(*) AS "개발건수", SUM("개발실적_건") AS "개발실적(건)", AVG("금액") AS "평균후원금액" FROM GN_DW.GOLD.V_MEMBER_DEV_DETAIL WHERE "개발구분" = ''신규'' AND "신청월" LIKE ''2025%'' GROUP BY "국내해외구분" ORDER BY "개발건수" DESC')
	)
...
------------------------------------------------------
-- SV_MARKETING_MESSAGING
------------------------------------------------------
CREATE OR REPLACE SEMANTIC VIEW SV_MARKETING_MESSAGING 
	tables (
		SMS_SEND as GN_DW.GOLD.V_SMS_ALIMTALK_SEND comment='문자/알림톡 전체 발송.',
		ALIM_INC as GN_DW.GOLD.V_ALIMTALK_INCREASE_CROSS comment='알림톡수신회원 x 증액건 크로스분석.',
		SEND_CONV as GN_DW.GOLD.V_SEND_CONVERSION_ANALYSIS comment='발송유형별 전환분석.'
	)
	facts (
	...
	comment='마케팅발송 분석. 알림톡/문자 발송, 전환분석, 알림톡수신회원 증액 크로스분석.'
	ai_verified_queries (
		"알림톡_증액_중단" AS ( 
QUESTION '2025년 알림톡 서비스를 받은 회원 기준 연간 서비스 발송횟수, 증액건, 중단건은?' 
ONBOARDING_QUESTION true 
SQL 'WITH alim_members AS (SELECT "회원번호", COUNT(*) AS "발송횟수" FROM GN_DW.GOLD.V_SMS_ALIMTALK_SEND WHERE "제목" LIKE ''%알림톡%'' AND LEFT("발송일시",4)=''2025'' AND "발송상태"=''발송완료'' GROUP BY "회원번호"), inc AS (SELECT "회원번호", COUNT(*) AS "증액건수" FROM GN_DW.GOLD.V_ALIMTALK_INCREASE_CROSS WHERE "개발연도" LIKE ''2025%'' AND "알림톡수신여부"=''Y'' GROUP BY "회원번호"), disc AS (SELECT "회원번호", COUNT(*) AS "중단건수" FROM GN_DW.GOLD.V_DISCONTINUED_DETAIL WHERE "중단연도"=''2025'' GROUP BY "회원번호") SELECT COUNT(DISTINCT a."회원번호") AS "알림톡수신회원수", SUM(a."발송횟수") AS "총발송횟수", ROUND(AVG(a."발송횟수"),1) AS "평균발송횟수", COALESCE(SUM(i."증액건수"),0) AS "총증액건수", COALESCE(SUM(d."중단건수"),0) AS "총중단건수" FROM alim_members a LEFT JOIN inc i ON a."회원번호"=i."회원번호" LEFT JOIN disc d ON a."회원번호"=d."회원번호"'),
		"발송유형별_전환" AS ( 
QUESTION '발송유형별 전환율과 평균전환소요일은?' 
ONBOARDING_QUESTION true 
SQL 'SELECT "발송유형", "전환건수", "전환율", "평균전환소요일" FROM GN_DW.GOLD.V_SEND_CONVERSION_ANALYSIS ORDER BY "전환건수" DESC')
	)
...
CREATE OR REPLACE SEMANTIC VIEW SV_AD_PLATFORM
	tables (
		DIGITAL_AD as GN_DW.GOLD.V_DIGITAL_AD_DETAIL comment='디지털대행사 광고운영상세.',
		GA_AUDIENCE as GN_DW.GOLD.V_AD_GA_AUDIENCE comment='GA 잠재고객.',
		META_AD as GN_DW.GOLD.V_AD_META comment='메타(Facebook/Instagram) 광고.'
	)
	facts (
	...
	dimensions (
		...
		DIGITAL_AD.CAMPAIGN_TYPE as "캠페인 유형" with synonyms=('캠페인유형','캠페인 유형') comment='캠페인 유형.',
	...
	comment='디지털 광고 플랫폼 분석. 디지털대행사 + GA + 메타.'
	ai_verified_queries (
		"매체별_효율" AS ( 
QUESTION '2025년 매체별 광고효율(노출/클릭/CPC)은?' 
ONBOARDING_QUESTION false 
SQL 'SELECT "매체", SUM("노출수") AS "총노출", SUM("클릭수") AS "총클릭", ROUND(SUM("클릭수")*100.0/NULLIF(SUM("노출수"),0),2) AS CTR, ROUND(SUM("GA 광고비")/NULLIF(SUM("클릭수"),0),0) AS CPC FROM GN_DW.GOLD.V_DIGITAL_AD_DETAIL WHERE "연도" = ''2025년'' GROUP BY "매체" ORDER BY "총노출" DESC'), 
		"메타_캠페인_성과" AS ( 
QUESTION '메타 캠페인별 지출금액과 구매건수는?' 
ONBOARDING_QUESTION false 
SQL 'SELECT "캠페인이름", SUM("지출금액_KRW") AS "총지출", SUM("구매") AS "총구매", ROUND(SUM("지출금액_KRW")/NULLIF(SUM("구매"),0),0) AS "구매당비용" FROM GN_DW.GOLD.V_AD_META GROUP BY "캠페인이름" ORDER BY "총지출" DESC LIMIT 10'), 
		"캠페인유형별_효율" AS ( 
QUESTION '캠페인 유형별 매체별 효율을 분석해줘' 
ONBOARDING_QUESTION false 
SQL 'SELECT "캠페인 유형", "매체", SUM("노출수") AS "총노출", SUM("클릭수") AS "총클릭", SUM("GA 광고비") AS "총광고비", SUM("GA 개발건수") AS "총GA개발건수", ROUND(SUM("GA 광고비")/NULLIF(SUM("GA 개발건수"),0),0) AS "개발단가" FROM GN_DW.GOLD.V_DIGITAL_AD_DETAIL WHERE "연도" = ''2025년'' GROUP BY "캠페인 유형", "매체" ORDER BY "캠페인 유형", "총GA개발건수" DESC')
	)
...
------------------------------------------------------
-- SV_WEB_APP_ANALYTICS
------------------------------------------------------
CREATE OR REPLACE SEMANTIC VIEW SV_WEB_APP_ANALYTICS
	tables (
		APP_ENGAGEMENT as GN_DW.GOLD.V_APP_ENGAGEMENT comment='APP 방문 분석 (페이지경로, 이벤트, 세션수, 페이지뷰)',
		GA_TOTAL as GN_DW.GOLD.V_GA_VISITS_TOTAL comment='나의후원 전체 방문수 (PC+모바일+APP)',
		GA_PC as GN_DW.GOLD.V_GA_VISITS_PC comment='나의후원 PC 방문수',
		GA_MOBILE as GN_DW.GOLD.V_GA_VISITS_MOBILE comment='나의후원 모바일 방문수',
		GA_APP as GN_DW.GOLD.V_GA_VISITS_APP comment='나의후원 APP 방문수',
		GA_FEEDBACK as GN_DW.GOLD.V_GA_FEEDBACK_PAGE comment='피드백 서비스페이지 유입 (이탈률, 참여율, 평균세션시간)'
	)
	facts (
...
CREATE OR REPLACE SEMANTIC VIEW SV_MEMBER_JOURNEY
	tables (
		JOURNEY as GN_DW.GOLD.V_MEMBER_JOURNEY comment='회원별 후원 전후 통합 여정. 가입/매체/발송/증액/중단 연결.'
	)
	facts (
...
ONBOARDING_QUESTION true 
SQL 'SELECT "상위캠페인명", COUNT(DISTINCT "회원번호") AS 회원수, ROUND(AVG("총발송수"),1) AS 평균발송수, ROUND(SUM(CASE WHEN "증액여부"=''Y'' THEN 1 ELSE 0 END)*100.0/NULLIF(COUNT(*),0),1) AS 증액율, ROUND(SUM(CASE WHEN "중단여부"=''Y'' THEN 1 ELSE 0 END)*100.0/NULLIF(COUNT(*),0),1) AS 중단율 FROM GN_DW.GOLD.V_MEMBER_JOURNEY GROUP BY "상위캠페인명" ORDER BY 회원수 DESC LIMIT 20'),
		"중단회원_여정리스트" AS ( 
QUESTION '중단한 회원들의 가입캠페인, 잠재고객, 발송서비스, 중단사유 리스트를 보여줘'
ONBOARDING_QUESTION true 
SQL 'SELECT "회원번호", "상위캠페인명", "가입월", "가입금액", "잠재고객목록", "총발송수", "발송서비스목록", "중단사유", "중단월", "유지개월수" FROM GN_DW.GOLD.V_MEMBER_JOURNEY WHERE "중단여부" = ''Y'' ORDER BY "중단월" DESC LIMIT 100'),
		"증액회원_특성" AS ( 
QUESTION '증액한 회원들의 상위캠페인별 평균유지기간과 평균발송수는?' 
ONBOARDING_QUESTION false
SQL 'SELECT "상위캠페인명", COUNT(DISTINCT "회원번호") AS 증액회원수, ROUND(AVG("유지개월수"),1) AS 평균유지개월수, ROUND(AVG("총발송수"),1) AS 평균발송수, ROUND(AVG("증액금액"),0) AS 평균증액금액 FROM GN_DW.GOLD.V_MEMBER_JOURNEY WHERE "증액여부" = ''Y'' GROUP BY "상위캠페인명" ORDER BY 증액회원수 DESC LIMIT 20')
	)
