...
| 마케팅 보고서 | 4. 전환회원 특성 | 성별(회원) | CRM | 문자 | `공130` | `DIM_MEMBER.GENDER_NAME` |  | 필드인벤토리 |
...
| 2. 회원특성 | 2-1. [회원] 주간 중단보고 · GA 필드 | 가입경로 | CRM | 문자 | `(215밖)` | `DIM_MEMBER.ENROLL_PATH_NAME` |  | 필드인벤토리 |
...
| 2. 회원특성 | 2-1. [회원] 주간 중단보고 · GA 필드 | 회원구분 | CRM | 문자 | `(215밖)` | `DIM_MEMBER.MEMBER_TYPE_NAME` |  | 필드인벤토리 |
...
| 2. 회원특성 | 2-4. [회원] 연간분석 (회비) 분석 *월간기준 · GA 필드 | 회원상태 | CRM | 문자 | `공132` | `DIM_MEMBER.MEMBER_STATUS_NAME` |  | 필드인벤토리 |
...
| 3. 서비스 | 3-1. [회원] 서비스효과성분석보고 (알림톡/이메일) *일일기준 · CRM 필드 | 회원상태 | CRM | 문자 | `공132` | `DIM_MEMBER.MEMBER_STATUS_NAME` |  | 필드인벤토리 |
...
| 3. 서비스 | 3-2. [회원] 증액마케팅보고 *월간기준 · CRM 필드 | 성별 | CRM | 문자 | `공130` | `DIM_MEMBER.GENDER_NAME` |  | 필드인벤토리 |
...
| 3. 서비스 | 3-4. [회원] 미납중단안내서비스보고 *일간기준 · CRM 필드 | 회원상태 | CRM | 문자 | `공132` | `DIM_MEMBER.MEMBER_STATUS_NAME` |  | 필드인벤토리 |
...
| 3. 서비스 | 3-5. [회원] 참여콘텐츠분석보고 (온라인이벤트) *일간기준 · CRM 필드 | 성별 | CRM | 문자 | `공130` | `DIM_MEMBER.GENDER_NAME` |  | 필드인벤토리 |
...
| 3. 서비스 | 3-5. [회원] 참여콘텐츠분석보고 (온라인이벤트) *일간기준 · CRM 필드 | 회원상태 | CRM | 문자 | `공132` | `DIM_MEMBER.MEMBER_STATUS_NAME` |  | 필드인벤토리 |
...
| 3. 서비스 | 3-6. [회원] 참여콘텐츠분석보고 (문화이벤트) *일간기준 · CRM 필드 | 회원상태 | CRM | 문자 | `공132` | `DIM_MEMBER.MEMBER_STATUS_NAME` |  | 필드인벤토리 |
...
| 3. 서비스 | 3-6. [회원] 참여콘텐츠분석보고 (문화이벤트) *일간기준 · CRM 필드 | 성별 | CRM | 문자 | `공130` | `DIM_MEMBER.GENDER_NAME` |  | 필드인벤토리 |
...
| 3. 서비스 | 3-7. [회원] 앱참여수치분석보고 *월간기준 | 성별 | CRM | 문자 | `공130` | `DIM_MEMBER.GENDER_NAME` |  | 필드인벤토리 |
...
> 🔴 **[2026-08-03 O26 명명 전환]** `DIM_MEMBER` 의 **코드 컬럼은 BRONZE 원천명**으로, **라벨 컬럼은 분석 용어**로 분리했다.
> 본 표의 지표는 업무 용어(성별·회원상태·회원구분·가입경로)이므로 **라벨 컬럼**을 가리키도록 갱신했다:
> `GENDER`→`GENDER_NAME` · `MEMBER_STATUS`→`MEMBER_STATUS_NAME` · `MEMBER_TYPE`→`MEMBER_TYPE_NAME` · `ENROLL_PATH`→`ENROLL_PATH_NAME`.
> 원천코드가 필요하면 각각 `SEX`(CM013) · `MBER_STAT_CD`(MM010) · `MBER_DIV_CD`(MM018) · `JOIN_PATH_CD`(MM014) 를 쓴다.
> ⚠️ 공#130 성별 라벨은 **CM017 사전 라벨**(남자/여자/기타/단체/기업)로 교체됐다 — 종전 하드코딩 '여성/남성/미상'은
> 정본 5종을 3종으로 축약하고 법인·단체 54,992명을 '미상'으로 오라벨했다. 국내/외국인 구분은 `SEX_NM`(CM013).
