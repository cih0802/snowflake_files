<!-- LLM-METADATA
doc_id: BRONZE_SILVER_GOLD_COLUMN_MAPPING
doc_role: 현업용 source-target 컬럼 계보 매핑 (WIDE 마트 기준)
project: GN_DW (굿네이버스)
created: 2026-07-15
grounded_on: 08_silver의존.md · _sources.yml · silver/gold dbt models
generator: scripts/gen_column_mapping.py
generated: auto (do-not-edit)
END-METADATA -->

# BRONZE → SILVER → GOLD 컬럼 매핑 (현업용)

> ⚙️ **생성기**: `scripts/gen_column_mapping.py` — 본 파일은 자동 생성물입니다. 직접 수정 금지 — 생성기 scripts/gen_column_mapping.py 수정 후 재실행하세요.
> **읽는 법**: 현업이 조회하는 **최종 결과(GOLD/WIDE 마트)** 컬럼을 기준으로,
> 그 값이 어떤 **SILVER(정제)** → **BRONZE(원천)** 컬럼에서 왔는지 역방향으로 추적합니다.
> **지표#** = 해당 컬럼에 대응하는 지표사전 번호(공N=공통·신N=신규). 지표 기준 전체 추적은 `05_지표GOLD매핑.md` 참조.
> 상태: **OK** 사용가능 · **PARTIAL** 일부 대기 · **WAIT** 원천 입고 대기

## 0. 업무 질문 → 어느 마트를 볼까?

| 업무 질문 | 조회 마트 | 상태 |
|---|---|---|
| 지난달 캠페인·후원사업별 신규개발/이탈 실적 | `WIDE_MEMBER_MONTHLY` | ✅ |
| 회원 상태·지역·연령대별 활동회원·납입회비 | `WIDE_MEMBER_MONTHLY` | ✅ |
| 개발/중단 건별 사유·경로 상세 | `WIDE_MEMBER_EVENT` | ✅ |
| 알림톡/서신 발송 후 +5일 내 반응(증액·서신참여) | `WIDE_SERVICE_EVENT` | ✅ |
| 행사별 참여 회원·납입 실적 | `WIDE_EVENT_PARTICIPATION` | ✅ |
| 조직·부서별 회원개발 목표 대비 달성률 | `WIDE_TARGET_DEV` | ✅ |
| 예산 편성 대비 집행 현황(조직·예산과목별) | `WIDE_BUDGET` | ◐ 모금성비용/광고비 대기 |
| 디지털 광고 매체비·노출·클릭 성과 | `WIDE_AD_PERFORMANCE` | ◐ 캠페인연결/ROAS 대기 |
| 웹/앱 방문·세션·스크롤 등 GA 행동 | `WIDE_GA_BEHAVIOR` | ◐ 회원귀속 대기 |
| 광고 대비 CRM 후원 전환율(ROAS) | `WIDE_AD_PERFORMANCE+신원브리지` | 🔜 로드맵 |
| 연/추경 사업목표 대비 달성률 | `WIDE_TARGET_BIZ` | ⛔ 구조준비·원천 대기(E-6) |

## 1. `WIDE_MEMBER_MONTHLY`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `MONTH_KEY` |  | 실적 월(YYYYMM) | `CRM_MEMBER_DEV.OCCRRNC_DE 등 거래일` | `TM_MM_FDRM_MBER_DVLP_AMT.OCCRRNC_DE` | 거래일→월 롤업(YYYYMM) | OK |
| `MEMBER_DK` | 공110 | 회원 식별키 | `CRM_MEMBER.MEMBER_DK` | `TM_MM_FDRM_MBER_INFO.MBER_NO / TM_MM_ONCE_MBER_INFO.ONCE_MBER_NO` | 정기∪일시 회원번호 통합키 | OK |
| `DEV_MEMBERS` | 공148 | 개발(명) | `CRM_MEMBER_DEV(MBER_NO distinct)` | `TM_MM_FDRM_MBER_DVLP_AMT.MBER_NO` | 월×회원 COUNT(DISTINCT) | OK |
| `DEV_CNT` | 공4·5·149 | 개발(건) | `CRM_MEMBER_DEV.SPNSR_AMT` | `TM_MM_FDRM_MBER_DVLP_AMT.SPNSR_AMT` | SUM(금액)/10000 basis | OK |
| `INCREASE_MEMBERS` | 공150 | 증액(명) | `CRM_MEMBER_AMT_CHANGE(RDCAMT_YN='N')` | `TM_MM_FDRM_MBER_IRSD.SPNSR_AMT·RDCAMT_YN` | 증액분 회원 COUNT | OK |
| `DECREASE_CNT` | 공38 | 감액(건) | `CRM_MEMBER_AMT_CHANGE(RDCAMT_YN='Y')` | `TM_MM_FDRM_MBER_IRSD.SPNSR_AMT·RDCAMT_YN` | SUM(감액금액)/10000 | OK |
| `CHURN_CNT` | 신20 | 이탈(건)=취소+감액 | `CRM_MEMBER_DISCONTINUE + CRM_MEMBER_AMT_CHANGE` | `TM_MM_FDRM_MBER_SPNSR_DSCNTC + TM_MM_FDRM_MBER_IRSD` | SUM(취소+감액)/10000 (신규 표준지표) | OK |
| `ACTIVE_MEMBERS` | 공156 | 활동회원(명) | `CRM_MEMBER_STATUS_HIST + CRM_MEMBER_SPONSOR_BIZ` | `TH_MM_FDRM_MBER_STNG_DTLS + TM_MM_FDRM_MBER_SPNSR_BSNS` | 월말 시점 스냅샷 COUNT | OK |
| `UNPAID_CNT` | 공36 | 미납(건) | `CRM_PAYMENT_BILLING(PAY_STAT_CD=F∪NULL)` | `TM_PM_MBRFEE_ACMSLT.PAY_STAT_CD` | 미납 상태 건수(C-3 정의) | OK |
| `STOP_CNT` | 공35 | 중단(건) | `CRM_MEMBER_DISCONTINUE` | `TM_MM_FDRM_MBER_SPNSR_DSCNTC.SPNSR_DSCNTC_DE` | 월 중단 건수 | OK |
| `REGULAR_FEE` | 공66 | 정기회비(원) | `CRM_PAYMENT_BILLING.PAY_AMT` | `TM_PM_MBRFEE_ACMSLT.PAY_AMT` | 정기 납입액 SUM(원단위) | OK |
| `PAID_FEE` | 공69·70 | 납입회비(원) | `CRM_PAYMENT_BILLING.PAY_AMT` | `TM_PM_MBRFEE_ACMSLT.PAY_AMT + TM_PM_DNTN_DTLS.PAY_AMT` | 회비∪기부금 납입 SUM | OK |
| `BILLED_AMT` | 공71 | 청구(원) | `CRM_PAYMENT_BILLING.RQEST_AMT` | `TM_PM_MBRFEE_ACMSLT.RQEST_AMT` | 청구액 SUM(행기준) | OK |
| `REDONATE_FLAG` | 공34 | 재후원 여부 | `CRM_MEMBER_RESPONSOR` | `TM_MM_FDRM_MBER_RE_SPNSR` | 재후원 이력 존재 플래그 | OK |
| `INBOUND_CALL_CNT` |  | 인바운드콜수 | `(CRM 부재)` | `(원천 없음)` | 현업 수기입력(비-CRM, C-8) | WAIT |
| `TS_CALL_CNT` |  | TS콜수 | `(CRM 부재)` | `(원천 없음)` | 현업 수기입력(비-CRM, C-8) | WAIT |
| `MEMBER_GENDER` | 공130 | 성별 | `CRM_MEMBER.SEX` | `TM_MM_FDRM_MBER_INFO.SEX` | 1/3→M, 2/4→F, NULL→NULL 정규화 | OK |
| `MEMBER_REGION` | 공131 | 지역 | `CRM_MEMBER_DEV.AREA_CD(스냅샷)` | `TM_MM_FDRM_MBER_DVLP_AMT.AREA_CD` | CM018 코드→라벨(개발시점 스냅샷) | OK |
| `MEMBER_AGE_BAND` |  | 연령대 | `CRM_MEMBER_DEV.AGE(스냅샷)` | `TM_MM_FDRM_MBER_DVLP_AMT.AGE` | 연령 코드화(생년 raw 미적재) | OK |
| `MEMBER_STATUS` | 공132 | 회원상태 | `CRM_MEMBER.MBER_STAT_CD + STATUS_HIST` | `TM_MM_FDRM_MBER_INFO.MBER_STAT_CD` | MM010 코드→라벨 + SCD2 이력 | OK |
| `MEMBER_TYPE` |  | 회원구분(정기/일시) | `CRM_MEMBER.MEMBER_TYPE(파생)` | `TM_MM_FDRM_MBER_INFO / TM_MM_ONCE_MBER_INFO` | FDRM/ONCE 크로스워크(테이블 출처 기반) | OK |
| `CAMPAIGN_BRAND` | 공117 | 브랜드 | `CRM_CAMPAIGN.BRND_NM` | `TM_CM_BRND_MNG.BRND_NM` | 캠페인→브랜드 LEFT JOIN | OK |
| `CAMPAIGN_NAME` | 공18·120 | 캠페인명 | `CRM_CAMPAIGN.CMPGN_NM` | `TM_CM_CMPGN_MNG.CMPGN_NM` | - | OK |
| `SPONSORSHIP_NAME` | 공123·124 | 후원사업명 | `CRM_SPONSORSHIP.SPNSR_BSNS_NM` | `TM_CM_SPNSR_BSNS_INFO.SPNSR_BSNS_NM` | - | OK |
| `PAYMENT_METHOD` | 공125 | 결제수단 | `CRM_PAYMENT_METHOD.CARD_DIV_CD` | `TM_PM_SETLE_INFO.CARD_DIV_CD` | 코드→라벨 | OK |

## 2. `WIDE_SERVICE_EVENT`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `DATE_SK` |  | 발송일(YYYYMMDD) | `CRM_SEND_REQUEST 발송일` | `TM_MS_EMAIL_SNDNG / TM_MS_MSG_AT_SNDNG / TM_MS_PSTMTR_SNDNG` | 채널별 발송일 통합 | OK |
| `MEMBER_DK` | 공110 | 발송 대상 회원 | `CRM_SEND_MEMBER.MBER_NO` | `TD_MS_EMAIL_SNDNG_DTLS / TD_MS_MSG_AT_SNDNG_DTLS / TD_MS_PSTMTR_SNDNG_DTL` | 발송상세 회원키 | OK |
| `SEND_MEMBERS` | 공85 | 발송수(명) | `CRM_SEND_MEMBER(MBER_NO distinct)` | `TD_MS_*_SNDNG_DTLS` | COUNT(DISTINCT 회원) | OK |
| `SUCCESS_MEMBERS` | 공86 | 성공수(명) | `CRM_SEND_RESULT(성공)` | `TD_MS_EMAIL_LQY_SNDNG / TD_MS_MSG_AT_LQY_SNDNG / TD_MS_PSTMTR_LQY_SNDNG` | 성과 성공건 회원 COUNT | OK |
| `FAIL_MEMBERS` | 공87 | 실패수(명) | `CRM_SEND_RESULT(실패)` | `TD_MS_*_LQY_SNDNG` | 성과 실패건 회원 COUNT | OK |
| `LETTER_PART_MEMBERS` | 공88 | 서신참여(명) | `CRM_RELATION_ACTIVITY(LETTER)` | `TM_RM_RELATNSP_LETTER_INFO` | 발송×서신 매칭 | OK |
| `GIFT_PART_AMT` | 공91 | 선물금참여(원) | `CRM_RELATION_ACTIVITY.GFTMNEY` | `TM_RM_RELATNSP_GFTMNEY_INFO.GFTMNEY` | 선물금 SUM(원단위) | OK |
| `D5_LETTER_PART_MEMBERS` | 공139 | +5일 서신참여(명) | `CRM_RELATION_ACTIVITY × CRM_SEND_*` | `TM_RM_RELATNSP_LETTER_INFO` | 발송일+5일 윈도우 매칭 | OK |
| `D5_INCREASE_PART_MEMBERS` | 공143 | +5일 증액참여(명) | `CRM_MEMBER_AMT_CHANGE × CRM_SEND_*` | `TM_MM_FDRM_MBER_IRSD` | 발송일+5일 윈도우 매칭 | OK |
| `SEND_TITLE` | 공136 | 발송 제목 | `CRM_SEND_REQUEST.TIT` | `SND_REQ_MST.SEND_TITLE / TM_MS_*_SNDNG` | - | OK |
| `SEND_STATUS` | 공138 | 발송상태 | `CRM_SEND_MEMBER.SNDNG_RST_CD` | `TD_MS_*_SNDNG_DTLS` | 코드→라벨 | OK |
| `APP_PUSH_SEND_CNT` |  | 앱푸시 발송수 | `(ADMIN 제외)` | `(원천 미채택)` | 어드민 제외 확정(2026-07-09)→미채움 | WAIT |
| `SERVICE_CHANNEL` |  | 발송 채널 | `CRM_SEND_REQUEST.SEND_CHANNEL` | `SND_REQ_MST / TM_MS_*_SNDNG` | 이메일/문자/우편 채널 | OK |

## 3. `WIDE_AD_PERFORMANCE`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `PERF_DATE_SK` |  | 광고 실적일 | `AGENCY_AD_PERFORMANCE 실적일` | `DGT/REBRDC/VIDEO_AD_CMPGN_DTLS` | 유형별 실적일 통합 | PARTIAL |
| `AD_COST` | 공6 | 광고비(원) | `AGENCY_AD_PERFORMANCE 광고비` | `DGT/REBRDC/VIDEO_AD_CMPGN_DTLS(광고비 컬럼 3종)` | 유형별 비용 정제→UNION | OK |
| `IMPRESSIONS` | 공23 | 노출수 | `AGENCY_AD_PERFORMANCE` | `DGT_AD_CMPGN_DTLS(노출)` | 디지털(DGT)만 존재 | PARTIAL |
| `CLICKS` | 공24 | 클릭수 | `AGENCY_AD_PERFORMANCE` | `DGT_AD_CMPGN_DTLS(클릭)` | 디지털(DGT)만 존재 | PARTIAL |
| `INBOUND_CALL` | 공25 | 인입콜수 | `AGENCY_AD_PERFORMANCE` | `REBRDC/VIDEO_AD_CMPGN_DTLS` | TRY_TO_NUMBER 캐스팅 | PARTIAL |
| `GA_CONV_MEMBERS` |  | GA전환수(명) | `GA4_EVENT(전환 이벤트)` | `BRONZE_GA4.events_YYYYMMDD` | 전환정의(O5) 회신 대기 | WAIT |
| `CAMPAIGN_BRAND` | 공117 | 브랜드 | `CRM_CAMPAIGN.BRND_NM` | `TM_CM_BRND_MNG.BRND_NM` | 캠페인 이름매칭(Q10) 대기 | WAIT |
| `AD_MEDIA_NAME` |  | 매체명 | `AGENCY_AD_CREATIVE.MEDIA_NAME` | `DGT_AD_CMPGN_DTLS.MEDIA_NM 등` | 유형별 산재→UNION | PARTIAL |
| `AD_PLATFORM` |  | 플랫폼 | `AGENCY_AD_CREATIVE.PLATFORM` | `DGT/REBRDC/VIDEO_AD_CMPGN_DTLS` | - | PARTIAL |
| `DEVICE_TYPE` | 공14 | 디바이스 | `GA4_DEVICE.DEVICE_TYPE` | `BRONZE_GA4.events(device)` | PC/M/APP | PARTIAL |

## 4. `WIDE_MEMBER_EVENT`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `DATE_SK` |  | 거래일 | `CRM_MEMBER_DEV.OCCRRNC_DE` | `TM_MM_FDRM_MBER_DVLP_AMT.OCCRRNC_DE` | - | OK |
| `MEMBER_DK` | 공110 | 회원 식별키 | `CRM_MEMBER.MEMBER_DK` | `TM_MM_FDRM_MBER_INFO.MBER_NO` | - | OK |
| `DEV_CNT/STOP_CNT` | 공149·35 | 개발/중단 건·명 | `CRM_MEMBER_DEV + CRM_MEMBER_DISCONTINUE` | `TM_MM_FDRM_MBER_DVLP_AMT + TM_MM_FDRM_MBER_SPNSR_DSCNTC` | 개발·중단 이벤트 건수 | OK |
| `STOP_REASON` | 공162 | 중단 사유 **코드** | `CRM_MEMBER_DISCONTINUE.DSCNTC_RSN_CD` | `TM_MM_FDRM_MBER_SPNSR_DSCNTC.DSCNTC_RSN_CD` | **무변환 raw 코드**(MM005) | OK |
| `STOP_REASON_NM` | 공162 | 중단 사유 **라벨** | `CRM_MEMBER_DISCONTINUE.DSCNTC_RSN_NM` | 〃 (+ `TC_CMMN_DTL_CD` MM005) | 사유코드→라벨, `USE_YN` 무필터 | **OK (2026-08-03 신설)** |
| `STOP_CHANNEL` |  | 중단 경로 **코드** | `CRM_MEMBER_DISCONTINUE.DSCNTC_PATH` | `TM_MM_FDRM_MBER_SPNSR_DSCNTC.DSCNTC_PATH` | **무변환 raw 코드**(MM287) | OK |
| `STOP_CHANNEL_NM` |  | 중단 경로 **라벨** | `CRM_MEMBER_DISCONTINUE.DSCNTC_PATH_NM` | 〃 (+ `TC_CMMN_DTL_CD` MM287) | 경로코드→라벨(SYSTEM/CRM/홈페이지) | **OK (2026-08-03 신설)** |

> 🔴 **[2026-08-03 O25 계약 위반 정정]** 종전 `STOP_REASON` 행은 변환규칙을 **"사유코드→라벨"**, 상태를 **"OK"** 로
> 적었으나 **실적재는 raw 코드**(`1`·`14`·`16`…)였다. 즉 **계약이 스스로를 OK 로 자기인증**하고 있었다.
> 조치: 코드/라벨을 **분리 2컬럼**으로 명시하고 `STOP_REASON_NM`·`STOP_CHANNEL_NM` 을 실제 배선했다.
> 실측 검증 = STOP 1,038,262행 라벨 결손 **0** · MM005 사전 적중 **20/20**(활성 14 + 폐지 6).
> 📌 **교훈**: 계보표의 `상태` 열은 **작성자 선언**이며 실적재 검증이 아니다. 인용 전 값 확인이 필요하다.
> (같은 부류 = `06_BRONZE노출감사.md` 의 이름기반 판정 — 본 문서 §7-5 참조)

## 5. `WIDE_EVENT_PARTICIPATION`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `DATE_SK` |  | 행사일 | `CRM_EVENT.STRT_DE` | `TM_MS_EVENT.STRT_DE / TM_MS_CRMN` | 행사∪캠페인행사 | OK |
| `MEMBER_DK` | 공110 | 참여 회원 | `CRM_EVENT_PARTICIPATION.MBER_NO` | `TD_MS_EVENT_PRTCPNT_DTL / TD_MS_CRMN_PRTCPNT` | - | OK |
| `EVENT_NAME` |  | 행사명 | `CRM_EVENT.EVENT_NM` | `TM_MS_EVENT.EVENT_NM` | - | OK |
| `PARTICIPATION_STATUS` |  | 참여상태 | `CRM_EVENT_PARTICIPATION.PARTCPT_STAT_CD` | `TD_MS_EVENT_PRTCPNT_DTL.PARTCPT_STAT_CD` | 모집/참여/취소/당첨 | OK |
| `RECEIPT_AMT` |  | 참여 납입액 | `CRM_EVENT_PARTICIPATION.RCPMNY_AMT` | `TD_MS_EVENT_PRTCPNT_DTL.RCPMNY_AMT` | 원단위 | OK |
| `VIEW_CNT` |  | 행사 조회수 | `(ADMIN 제외)` | `(원천 미채택)` | 어드민 제외 확정→미채움 | WAIT |

## 6. `WIDE_TARGET_DEV`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `MONTH_KEY` |  | 목표 월(YYYYMM) | `CRM_DEV_TARGET(STDYY+STDR_MT)` | `TM_CM_MBER_DVLP_GOAL.STDYY·STDR_MT` | 연+월→YYYYMM | OK |
| `ORG_NM` | 공114·115·116 | 조직/부서 | `CRM_ORG.DEPT_NM(←DEPT_ID)` | `TM_CM_DEPT_INFO.DEPT_NM` | 부서 계층 전개 | OK |
| `DEV_TYPE` | 공121 | 개발구분 | `CRM_DEV_TARGET.MBER_DVLP_DIV_CD` | `TM_CM_MBER_DVLP_GOAL.MBER_DVLP_DIV_CD` | 코드→라벨 | OK |
| `GOAL_CNT` |  | 목표 건수 | `CRM_DEV_TARGET.GOAL_CNT` | `TM_CM_MBER_DVLP_GOAL.GOAL_CNT` | - | OK |

## 7. `WIDE_BUDGET`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `MONTH_KEY` |  | 예산 월(YYYYMM) | `ERP_BUDGET.MONTH_KEY` | `BDGT_ACMSLT_LEDGER(YEAR + 월컬럼)` | wide→long 언피벗(12개월) | OK |
| `BUDGET_ITEM(계층)` |  | 예산과목(장~세세목) | `ERP_BUDGET_ITEM(JANG~SUBDTL)` | `BDGT_ACMSLT_LEDGER.JANG_NM~SUBDTL_ITEM_NM` | 6단계 계층 보존·MD5 키 | OK |
| `YEAR_BUDGET_AMT` |  | 편성예산(원) | `ERP_BUDGET.YEAR_BUDGET_AMT` | `BDGT_ACMSLT_LEDGER.YEAR_BDGT_AMT_n` | 월별 언피벗 | OK |
| `EXEC_AMT` |  | 집행예산(원) | `ERP_BUDGET.EXEC_AMT` | `BDGT_ACMSLT_LEDGER.EXEC_AMT_n` | 월별 언피벗 | OK |
| `ORG_NM` | 공114·115·116 | 조직/부서 | `ERP_BUDGET_ITEM.BDGT_UNIT_NM` | `BDGT_ACMSLT_LEDGER.BDGT_UNIT_NM` | 예산단위명(이름) | OK |
| `FUNDRAISING_COST` |  | 모금성비용(원) | `(원천 부재)` | `(ERP 원장에 없음)` | 원천 부재(E-1)→미채움 | WAIT |
| `AD_COST` | 공6 | 광고비(원) | `AGENCY 보강 대기` | `DGT/REBRDC/VIDEO_AD_CMPGN_DTLS` | AGENCY_COST 보강(E-4) | WAIT |

## 8. `WIDE_GA_BEHAVIOR`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `DATE_SK` |  | 행동 일자 | `GA4_EVENT.event_date` | `BRONZE_GA4.events_YYYYMMDD.event_date` | 샤드 UNION | PARTIAL |
| `SESSION_CNT` | 공97 | 세션수 | `GA4_EVENT.ga_session_id` | `BRONZE_GA4.events(event_params: ga_session_id)` | param 승격·비가산 주의 | PARTIAL |
| `SCROLL_DEPTH` | 공107 | 스크롤 깊이 | `GA4_EVENT.percent_scrolled` | `BRONZE_GA4.events(event_params: percent_scrolled)` | param 승격 | PARTIAL |
| `PAGE_PATH` | 공105·106 | 페이지 경로 | `GA4_EVENT.page_location` | `BRONZE_GA4.events(event_params: page_location)` | degenerate | PARTIAL |
| `IDENTITY(회원귀속)` | 공112 | GA↔회원 연결 | `GA4_IDENTITY.member_id` | `BRONZE_GA4.events(user_id / user_pseudo_id)` | IDENTITY_SK→DIM_MEMBER_IDENTITY 실조인(2026-07-15)·⚠️G-5 재검증 | PARTIAL |
| `UTM_SOURCE/MEDIUM` | 공109 | 유입 소스/매체 | `GA4_TRAFFIC_SOURCE.UTM_*` | `BRONZE_GA4.events(traffic_source)` | - | PARTIAL |

## 9. `WIDE_TARGET_BIZ`

| GOLD 컬럼 | 지표# | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| `MONTH_KEY` |  | 목표 월(YYYYMM) | `ERP_BIZ_TARGET.MONTH_KEY` | `(원천 부재·E-6)` | 월키 클램프(무효→0) | WAIT |
| `ORG_NM(계층)` | 공114·115·116 | 조직/부서 | `DIM_ORG(이름 크로스워크)` | `TM_CM_DEPT_INFO.DEPT_NM` | ORG_NM 이름매칭(본부/지부 vs 부서 grain 불일치 가능) | WAIT |
| `SPONSORSHIP_NAME` | 공123·124 | 후원사업명 | `DIM_SPONSORSHIP.SPONSORSHIP_NAME` | `TM_CM_SPNSR_BSNS_INFO.SPNSR_BSNS_NM` | SPONSOR_BIZ_NM 이름매칭 | WAIT |
| `CAMPAIGN_NAME` | 공18·120 | 캠페인명 | `DIM_CAMPAIGN.CAMPAIGN_NAME` | `TM_CM_CMPGN_MNG.CMPGN_NM` | CAMPAIGN_NM 이름매칭(선택 grain·nullable) | WAIT |
| `ANNUAL_GOAL_CNT` | 공152 | 연사업목표(#152) | `ERP_BIZ_TARGET.TARGET_AMT(금액)` | `(원천 부재)` | ⚠️단위충돌: 금액≠건→오귀속 방지 NULL(GOLD DDL 금액 measure 신설 or 건 원천확보 시 배선) | WAIT |
| `SUPP_GOAL_CNT` | 공153 | 추경목표(#153) | `(원천 부재)` | `(원천 부재)` | 원천부재→NULL | WAIT |
| `ANNUAL_CUM_GOAL_CNT` | 공154 | 연사업누계목표(#154) | `(원천 부재)` | `(원천 부재)` | 원천부재→NULL | WAIT |
| `SUPP_CUM_GOAL_CNT` | 공155 | 추경누계목표(#155) | `(원천 부재)` | `(원천 부재)` | 원천부재→NULL | WAIT |

---

## 부록. DIM/FACT 계층 매핑 (파워유저·엔지니어용)

| 계층 | GOLD 객체 | 업무 의미 | SILVER 원천 | BRONZE 원천 | 변환 규칙 | 상태 |
|---|---|---|---|---|---|---|
| DIM | `DIM_MEMBER` | 회원 마스터(SCD2) | `CRM_MEMBER (+ STATUS_HIST)` | `TM_MM_FDRM_MBER_INFO / TM_MM_ONCE_MBER_INFO / TH_MM_FDRM_MBER_STNG_DTLS` | 정기∪일시 통합·상태이력 | OK |
| DIM | `DIM_MEMBER_IDENTITY` | 신원 브리지(GA↔회원) | `CRM_MEMBER + GA4_IDENTITY` | `TM_MM_FDRM_MBER_INFO + BRONZE_GA4.events` | exact 매칭 1,274명·활성(2026-07-15)·XREF dedup·⚠️G-5 재검증 | OK |
| DIM | `DIM_CAMPAIGN` | 캠페인 | `CRM_CAMPAIGN` | `TM_CM_CMPGN_MNG (+BRND_MNG+MKTNG_CMPGN_MNG)` | 브랜드/마케팅캠페인 조인 | OK |
| DIM | `DIM_SPONSORSHIP` | 후원사업 | `CRM_SPONSORSHIP` | `TM_CM_SPNSR_BSNS_INFO` | - | OK |
| DIM | `DIM_ORG` | 조직/부서 | `CRM_ORG` | `TM_CM_DEPT_INFO` | 부서 계층 재귀전개 | OK |
| DIM | `DIM_PAYMENT` | 결제수단 | `CRM_PAYMENT_METHOD` | `TM_PM_SETLE_INFO` | - | OK |
| DIM | `DIM_REASON` | 사유(미납/중단) | `CRM_MEMBER_DISCONTINUE (또는 CRM_CODE MM005)` | `TM_MM_FDRM_MBER_SPNSR_DSCNTC` | 사유코드→라벨 | OK |
| DIM | `DIM_EVENT` | 행사 | `CRM_EVENT` | `TM_MS_EVENT / TM_MS_CRMN` | 행사∪캠페인행사 | OK |
| DIM | `DIM_SERVICE` | 발송구분 | `CRM_SEND_REQUEST` | `SND_REQ_MST (SEND_GBN_TOP/MID/BOT)` | 대/중/소 코드→라벨 | OK |
| DIM | `DIM_GA_SOURCE` | GA 트래픽소스 | `GA4_TRAFFIC_SOURCE` | `BRONZE_GA4.events(traffic_source)` | UTM 파싱 | PARTIAL |
| DIM | `DIM_GA_EVENT` | GA 이벤트 분류 | `GA4_EVENT_DIM` | `BRONZE_GA4.events(event_params)` | category/label/action | PARTIAL |
| DIM | `DIM_DEVICE` | 디바이스 | `GA4_DEVICE` | `BRONZE_GA4.events(device)` | PC/M/APP | PARTIAL |
| DIM | `DIM_AD_CREATIVE` | 광고소재 | `AGENCY_AD_CREATIVE` | `DGT/REBRDC/VIDEO_AD_CMPGN_DTLS` | 유형별 정제→UNION | PARTIAL |
| DIM | `DIM_BUDGET_ITEM` | 예산과목 마스터 | `ERP_BUDGET_ITEM` | `BDGT_ACMSLT_LEDGER(장~세세목)` | DISTINCT 마스터·MD5 키 | OK |
| DIM | `DIM_DATE` | 날짜 차원 | `(ETL 생성)` | `(원천 무관)` | 팩트 일자범위로 생성 | OK |
| FACT | `FACT_MEMBER_MONTHLY` | 회원 월 팩트 | `CRM_MEMBER_* 다수` | `TM_MM_FDRM_* / TM_PM_*` | 월 grain 롤업 | OK |
| FACT | `FACT_MEMBER_EVENT` | 회원 이벤트 팩트 | `CRM_MEMBER_DEV + DISCONTINUE` | `TM_MM_FDRM_MBER_DVLP_AMT + _SPNSR_DSCNTC` | 일 grain | OK |
| FACT | `FACT_SERVICE_EVENT` | 발송 팩트 | `CRM_SEND_* + RELATION_ACTIVITY` | `TM_MS_*_SNDNG(+DTLS/LQY) + TM_RM_*` | 발송+반응(+5일) | OK |
| FACT | `FACT_EVENT_PARTICIPATION` | 행사참여 팩트 | `CRM_EVENT + PARTICIPATION` | `TM_MS_EVENT/CRMN + TD_MS_*_PRTCPNT*` | - | OK |
| FACT | `FACT_TARGET_DEV` | 개발목표 팩트 | `CRM_DEV_TARGET` | `TM_CM_MBER_DVLP_GOAL` | 월×조직×개발구분 | OK |
| FACT | `FACT_GA_BEHAVIOR` | GA행동 팩트 | `GA4_EVENT + IDENTITY_MEMBER_XREF` | `BRONZE_GA4.events_YYYYMMDD` | 샤드 통합·IDENTITY_SK 실조인(2026-07-15) | PARTIAL |
| FACT | `FACT_AD_PERFORMANCE` | 광고성과 팩트 | `AGENCY_AD_PERFORMANCE + GA4_EVENT` | `DGT/REBRDC/VIDEO_AD_CMPGN_DTLS + events` | 캠페인매칭(Q10)·전환(O5) 대기 | PARTIAL |
| FACT | `FACT_BUDGET` | 예산 팩트 | `ERP_BUDGET (+AGENCY 보강)` | `BDGT_ACMSLT_LEDGER` | 편성/집행 O·모금성비용 대기 | PARTIAL |
| FACT | `FACT_TARGET_BIZ` | 사업목표 팩트 | `ERP_BIZ_TARGET(스키마-only 0행)` | `(ERP 원장≠사업목표)` | dbt 스켈레톤 저작(0행)·이름 크로스워크 조인·단위충돌(금액vs건)→측정치 NULL·입고 시 자동화(E-6) | WAIT |

---

## 조인 매트릭스 (팩트↔차원 조인 계약)

> 정본: `10_dbt_pipeline/models/gold/_gold_ready_schema.yml`(`relationships` 테스트) + `models/gold/wide/*.sql`(실제 조인). dbt 테스트에서 추출되어 빌드와 항상 일치.

**범례** — **FK**: 팩트 조인 컬럼 · **PK**: 차원 대상 키 · `SK != 0`: 미매칭·범위밖·NULL 은 Unknown 멤버(SK=0)로 라우팅되어 무결성 테스트 제외(0은 정상 시드행) · ⚠️**warn**: `severity: warn`(회원 마스터 미완전으로 고아 관측, 순서9-B) · **degenerate**: 차원 조인 없이 팩트에 보관하는 키.

### A. 팩트 → 차원 조인 매트릭스 (정본: dbt `relationships`)

| Fact 테이블 | Join 컬럼 (FK) | Target 차원.PK | 제외조건/심각도 | 비고 |
|---|---|---|---|---|
| FACT_GA_BEHAVIOR | DATE_SK | DIM_DATE.DATE_SK | — | 일 grain 날짜 |
| FACT_GA_BEHAVIOR | IDENTITY_SK | DIM_MEMBER_IDENTITY.IDENTITY_SK | `SK != 0` | GA↔회원 신원(익명 95% Unknown) |
| FACT_GA_BEHAVIOR | GA_EVENT_SK | DIM_GA_EVENT.GA_EVENT_SK | — | GA 이벤트 유형 |
| FACT_GA_BEHAVIOR | GA_SOURCE_SK | DIM_GA_SOURCE.GA_SOURCE_SK | — | 트래픽 소스 |
| FACT_GA_BEHAVIOR | DEVICE_SK | DIM_DEVICE.DEVICE_SK | — | 디바이스 |
| FACT_GA_BEHAVIOR | CAMPAIGN_SK | DIM_CAMPAIGN.CAMPAIGN_SK | `SK != 0` | 캠페인 |
| FACT_EVENT_PARTICIPATION | DATE_SK | DIM_DATE.DATE_SK | — | 일 grain 날짜 |
| FACT_EVENT_PARTICIPATION | MEMBER_DK | DIM_MEMBER.MEMBER_DK | ⚠️ warn | 회원(SCD2·IS_CURRENT 필터) |
| FACT_EVENT_PARTICIPATION | EVENT_SK | DIM_EVENT.EVENT_SK | `SK != 0` | 행사 |
| FACT_EVENT_PARTICIPATION | CAMPAIGN_SK | DIM_CAMPAIGN.CAMPAIGN_SK | `SK != 0` | 캠페인 |
| FACT_EVENT_PARTICIPATION | SPONSORSHIP_SK | DIM_SPONSORSHIP.SPONSORSHIP_SK | `SK != 0` | 후원사업 |
| FACT_MEMBER_EVENT | DATE_SK | DIM_DATE.DATE_SK | — | 일 grain 날짜 |
| FACT_MEMBER_EVENT | MEMBER_DK | DIM_MEMBER.MEMBER_DK | ⚠️ warn | 회원(SCD2·IS_CURRENT 필터) |
| FACT_MEMBER_EVENT | CAMPAIGN_SK | DIM_CAMPAIGN.CAMPAIGN_SK | `SK != 0` | 캠페인 |
| FACT_MEMBER_EVENT | SPONSORSHIP_SK | DIM_SPONSORSHIP.SPONSORSHIP_SK | `SK != 0` | 후원사업 |
| FACT_MEMBER_EVENT | ORG_SK | DIM_ORG.ORG_SK | `SK != 0` | 조직 |
| FACT_MEMBER_EVENT | REASON_SK | DIM_REASON.REASON_SK | `SK != 0` | 사유 |
| FACT_MEMBER_MONTHLY | MEMBER_DK | DIM_MEMBER.MEMBER_DK | ⚠️ warn | 회원(SCD2·IS_CURRENT 필터) |
| FACT_MEMBER_MONTHLY | MONTH_KEY | (DIM_DATE 조인 없음) | degenerate | 월 grain — `FLOOR/MOD`로 연·월 파생, DIM_DATE 미조인 |
| FACT_MEMBER_MONTHLY | CAMPAIGN_SK | DIM_CAMPAIGN.CAMPAIGN_SK | `SK != 0` | 캠페인 |
| FACT_MEMBER_MONTHLY | SPONSORSHIP_SK | DIM_SPONSORSHIP.SPONSORSHIP_SK | `SK != 0` | 후원사업 |
| FACT_MEMBER_MONTHLY | PAYMENT_SK | DIM_PAYMENT.PAYMENT_SK | `SK != 0` | 결제수단 |
| FACT_MEMBER_MONTHLY | REASON_SK | DIM_REASON.REASON_SK | `SK != 0` | 사유 |
| FACT_SERVICE_EVENT | DATE_SK | DIM_DATE.DATE_SK | — | 일 grain 날짜 |
| FACT_SERVICE_EVENT | MEMBER_DK | DIM_MEMBER.MEMBER_DK | ⚠️ warn | 회원(SCD2·IS_CURRENT 필터) |
| FACT_SERVICE_EVENT | SERVICE_SK | DIM_SERVICE.SERVICE_SK | `SK != 0` | 서비스 |
| FACT_SERVICE_EVENT | CAMPAIGN_SK | DIM_CAMPAIGN.CAMPAIGN_SK | `SK != 0` | 캠페인 |
| FACT_TARGET_DEV | ORG_SK | DIM_ORG.ORG_SK | `SK != 0` | 조직(목표) |
| FACT_BUDGET | BUDGET_ITEM_SK | DIM_BUDGET_ITEM.BUDGET_ITEM_SK | `SK != 0` | 예산 세목 |
| FACT_TARGET_BIZ | ORG_SK | DIM_ORG.ORG_SK | `SK != 0`·⚠️이름 크로스워크 | 조직(사업목표)·원천대기 0행 |
| FACT_TARGET_BIZ | SPONSORSHIP_SK | DIM_SPONSORSHIP.SPONSORSHIP_SK | `SK != 0`·⚠️이름 크로스워크 | 후원사업·원천대기 0행 |
| FACT_TARGET_BIZ | CAMPAIGN_SK | DIM_CAMPAIGN.CAMPAIGN_SK | nullable(선택 grain) | 캠페인·원천대기 0행 |

**관계 테스트가 없는 키 (참고 — 조인 계약 미선언)** — 팩트에 존재하나 `relationships` 테스트 미선언(스캐폴드/차원 미완). 개념도에 FK 실선으로 그리지 말 것.
- `FACT_MEMBER_MONTHLY.MONTH_KEY`, `FACT_BUDGET.MONTH_KEY` — 월 grain degenerate(DIM_DATE 미조인).
- `FACT_BUDGET.ORG_SK` — 관계 테스트 미선언.
- `FACT_AD_PERFORMANCE.PERF_DATE_SK / CAMPAIGN_SK / DEVICE_SK` — 관계 테스트 미선언(선언된 FK는 `AD_CREATIVE_SK → DIM_AD_CREATIVE` 뿐).

### B. WIDE 소비뷰 조인 (정본: `models/gold/wide/*.sql`)

WIDE 뷰는 미러가 아니라 **실제 LEFT JOIN 평탄화 뷰**. 회원 차원은 SCD2이므로 **현재버전 1행만** 조인.

| WIDE 뷰 | 팩트 | 회원 조인 방식 | 그 외 차원 조인 |
|---|---|---|---|
| WIDE_MEMBER_MONTHLY | FACT_MEMBER_MONTHLY | `DIM_MEMBER` `WHERE IS_CURRENT=TRUE` + `qualify ROW_NUMBER() PARTITION BY MEMBER_DK` → MEMBER_DK 조인 | CAMPAIGN·SPONSORSHIP·PAYMENT·REASON (SK 조인) |
| WIDE_MEMBER_EVENT | FACT_MEMBER_EVENT | 동일(IS_CURRENT 현재버전 1행) | DATE·CAMPAIGN·SPONSORSHIP·ORG·REASON |
| WIDE_EVENT_PARTICIPATION | FACT_EVENT_PARTICIPATION | 동일(IS_CURRENT 현재버전 1행) | DATE·EVENT·CAMPAIGN·SPONSORSHIP |
| WIDE_SERVICE_EVENT | FACT_SERVICE_EVENT | 동일(IS_CURRENT 현재버전 1행) | DATE·SERVICE·CAMPAIGN |
| WIDE_TARGET_BIZ | FACT_TARGET_BIZ | (회원차원 없음) | ORG·SPONSORSHIP·CAMPAIGN (이름기반 크로스워크). ⚠️원천대기(E-6) 0행·측정치 단위충돌로 NULL |

> **⚠️ FACT_TARGET_BIZ 유의**: ①**측정 단위충돌** — SILVER 원천은 목표 **금액**(`TARGET_AMT`)이나 GOLD measure(`ANNUAL/SUPP_GOAL_CNT` #152~155)는 **건(件)**. 오귀속 방지 위해 측정치 NULL 처리. 해소=GOLD DDL 금액 measure 신설 or 건 원천 확보. ②**조인=이름 크로스워크** — SILVER가 조직/후원사업/캠페인을 이름으로만 제공(코드 부재)하여 DIM `_NAME`/`DEPARTMENT` 이름 매칭. 코드 BK 조인 아님 → 미매칭 시 Unknown(0). 조직명(본부/지부) vs DIM_ORG.DEPARTMENT(부서) grain 불일치 가능.

> **SCD2 fan-out 방지**: 회원 차원은 버전이 여러 개(MEMBER_DK 반복)이므로 WIDE 는 반드시 `IS_CURRENT=TRUE` + `qualify` 로 회원당 현재 1행만 조인. 어기고 MEMBER_DK 직접 조인 시 팩트 행이 버전 수만큼 중복.

### C. 차원 PK / 키 규약

| 차원 | PK (unique) | 조인키 특성 |
|---|---|---|
| DIM_MEMBER | MEMBER_SK = `hash(MEMBER_DK, EFFECTIVE_FROM)` | **SCD2**(회원상태). MEMBER_DK 는 not_null·비-unique(버전 반복). 팩트는 MEMBER_DK 로 조인 후 IS_CURRENT 필터 |
| DIM_MEMBER_IDENTITY | IDENTITY_SK | GA↔회원 신원. Unknown=0 |
| DIM_DATE | DATE_SK = `YYYYMMDD` | 일 grain. Unknown=0. MONTH_KEY 는 속성(역방향 참조용) |
| DIM_CAMPAIGN / SPONSORSHIP / PAYMENT / REASON / SERVICE / EVENT / ORG / DEVICE / GA_EVENT / GA_SOURCE / AD_CREATIVE / BUDGET_ITEM | 각 `_SK` | 해시 대리키. **Unknown 멤버 SK=0** 시드행 보유 |

> **Unknown 센티넬 = 0 (정본)**: 13개 GOLD DIM 전량 `union all select 0 '(미매핑)'` 시드 + `gold_helpers` `COALESCE(...,0)`. `-1` 은 이 프로젝트에서 사용하지 않음(2026-07-16 확정).

---
_Co-authored with CoCo_