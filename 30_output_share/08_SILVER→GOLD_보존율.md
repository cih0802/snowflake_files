...
> **측정일** 2026-08-13 · 모집단 = SILVER 물리 DATA 컬럼(감사 `DW_*` 5종 제외)
...
| 구분 | 건수 |
|---|---:|
| SILVER DATA 컬럼 총계 | **536** |
| GOLD 직접소비 테이블의 컬럼(=보존율 분모) | **381** |
| └ REFERENCED (보존) | **238** |
| └ DROPPED (탈락) | **143** |
...
| `CRM_CODE` | 6 | 3 | 3 | 0 | 0 | 50% | `DIM_CAMPAIGN`, `DIM_MEMBER`, `DIM_REASON`, `FACT_DEV_ACHIEVEMENT`, `FACT_MEMBER_EVENT`, `FACT_MEMBER_FEE` |
...
| `CRM_SEND_MEMBER` | 11 | 10 | 1 | 0 | 0 | 91% | `FACT_SERVICE_EVENT` |
