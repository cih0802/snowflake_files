...
> #### 🟢 [2026-09-07 O140] dbt build 534노드 100% 통과 (PASS=495 WARN=39 ERROR=0) + RBAC 소유권 조정 + 테스트 오타 수정
> * **개요**: `DEC-50`에 따른 dbt 파이프라인 전체 빌드(`dbt build --target dev`) 과정에서 발견된 권한 충돌 및 테스트 스크립트 오타를 해결하고 전체 534개 노드 100% 정상 통과(ERROR=0, SKIP=0) 완료.
> * **주요 작업**:
>   - **RBAC 소유권 정비**: `WIDE_BIGQUERY_BEHAVIOR` 뷰 및 신규 BIGQUERY 3개 테이블(`DIM_BIGQUERY_EVENT`, `DIM_BIGQUERY_SOURCE`, `FACT_BIGQUERY_BEHAVIOR`)에 대한 `GN_DW_ADMIN` 소유권 이관 및 `GN_DW_ENGINEER`에 DML/CREATE VIEW 권한 재부여로 dbt 접근 제어 에러(`003001`) 해결.
>   - **테스트 식별자 정정**: `tests/warn_erp_budget_yearly_grain.sql` 내 잘못된 컬럼 참조 `YEAR_BUDGET_TOT_AMT`를 정본 컬럼명 `YEAR_BDGT_TOT_AMT`로 수정하여 `FACT_BUDGET_YEARLY` 및 하위 11개 노드 스킵 현상 해소.
>   - **파이프라인 전체 빌드 검증**: `dbt build --target dev` 실행 결과 `PASS=495, WARN=39, ERROR=0, SKIP=0, TOTAL=534` 달성.
>   - **임시 작업폴더 정리**: 작업 계획 완료에 따라 `/07_dbt 수정 작업폴더_임시/` 정리.
> * **판정**: 🟢 PASS — dbt 파이프라인 무결성 100% 확보.
...
> #### 🟢 [2026-09-07 O142] DEC-51 AGENCY 원천 전환 지표 개명(GA_CONV_* → AGENCY_CONV_*) 완결 + DDL·dbt·Live DB·SV_AD·산출물 전수 동기화
>
> - **착수 배경**: `FACT_AD_PERFORMANCE` 및 하류 WIDE 뷰, Semantic View에서 대행사(`AGENCY`) 실적 원천의 전환 지표가 과거 초기 설계에 따라 `GA_CONV_MEMBERS`, `GA_CONV_CNT`로 노출되어 BigQuery 웹로그(`BIGQUERY_REFINED_DATA`) 파이프라인과의 혼선 유발. 사용자 지시에 따라 `AGENCY` 명칭 체계로 전면 전환 (`DEC-51`).
> - **주요 작업 내용 및 결과**:
>   1. **DEC-51 신설 및 컬럼개명표 등재**:
>      - `20_issue/30_설계_의사결정.md` §37 `DEC-51` 신설 및 `20_issue/32_컬럼개명표.md` §8-D 등재.
>      - 대행사(`AGENCY`) 원천 기반 전환 지표를 `AGENCY_CONV_MEMBERS`(명) 및 `AGENCY_CONV_CNT`(건/VU)로 전면 일관화 확정.
>   2. **dbt 파이프라인 및 schema yml 전수 갱신**:
>      - `models/gold/fact/FACT_AD_PERFORMANCE.sql`: `p.CONV_MEMBER_CNT as AGENCY_CONV_MEMBERS`, `p.CONV_UNIT_CNT as AGENCY_CONV_CNT` 반영.
>      - `models/gold/wide/WIDE_AD_PERFORMANCE.sql`, `WIDE_AD_DIGITAL.sql`, `WIDE_AD_COMBINED.sql`: `f.AGENCY_CONV_MEMBERS`, `f.AGENCY_CONV_CNT` 반영.
>      - `models/gold/wide/_wide_schema.yml`: 3개 WIDE 뷰의 컬럼 선언 및 `CVR_SRC`, `CPA_SRC` description 전수 동기화.
>   3. **DDL 정본 및 Semantic View 갱신**:
>      - `03_top-down_gold/06_DDL.sql`, `10_WIDE VIEW 코멘트.sql`, `05_필드 인벤토리.md`, `04_SV파생 매핑.md`, `02_지표 분류.md` 내 컬럼명 및 코멘트 전수 갱신.
>      - `05_SV-Agent_ai/05_7_SV_DDL_AD.sql`, `04_SV_설계.md`, `03_SV_metric_배속.md`, `30_마케팅_AGENT_설계.md`: `TOTAL_AGENCY_CONV_MEMBERS` 및 `CVR` 산식 동기화.
>   4. **Snowflake Live DB 배포 및 반영**:
>      - `ALTER TABLE GN_DW.GOLD.FACT_AD_PERFORMANCE RENAME COLUMN GA_CONV_MEMBERS TO AGENCY_CONV_MEMBERS;`
>      - `ALTER TABLE GN_DW.GOLD.FACT_AD_PERFORMANCE RENAME COLUMN GA_CONV_CNT TO AGENCY_CONV_CNT;`
>      - `FACT_AD_PERFORMANCE` 컬럼 COMMENT 4건 갱신 완료.
>      - `WIDE_AD_PERFORMANCE`, `WIDE_AD_DIGITAL`, `WIDE_AD_COMBINED` 뷰 3종 CREATE OR REPLACE 완료.
>      - Semantic View `GN_DW.SERVING.SV_AD` 재배포 완료.
>   5. **산출물/ERD 전수 재발행 및 검증**:
>      - `dump_schema.py` 및 `census_columns.py` 실행 후 `gen_column_mapping.py`, `gen_metric_gold_mapping.py`, `gen_silver_gold_retention.py`, `gen_unresolved_issue_summary.py --write`, `gen_gold_erd.py`, `gen_pipeline_erd.py` 전수 재생성.
>      - `test_generators.py` 21/21 PASS (`DEC-51 AGENCY_CONV renaming` 사유 골든 갱신).
>      - `table_ddl_column_gate.py` 실행 결과 GOLD 37 + SILVER 42 = 79개 모델 100% 집합 일치 통과.
>   6. **임시 폴더 정리**:
>      - 계획 완료에 따라 `/07_bigquery 명칭 변경/` 폴더 정리 완료.
> - **품질 및 게이트 검증 결과**:
>   - 6대 게이트(`doc_census`, `doc_type_gate`, `clause_order_gate`, `index_row_gate`, `doc_heading_gate`, `doc_coord_gate`) 통과.
>   - `line_len.py` 상한 2000자 초과 0줄 확인.
