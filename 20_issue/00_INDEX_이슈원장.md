...
| 🔴 **09 판정 오류(내부)** | **17 미해소** | **[2026-08-07 O49]** 작업 2 착수 실측에서 **처방 이전에 판정이 틀린 것 17건** 적발. ① 2-1 「주간」 중단보고의 **앵커가 월 팩트**(`FMM`) → 주차 measure **6건 오판**. `FME` 월 롤업 = `FMM` **466개월 전부 일치** → 앵커 교체 손실 0 · ② 3-7좌 「도달불가」 **7건 오판** — `DIM_MEMBER_IDENTITY` **1:1 브리지**(1,763,066 전건 유일 · 2홉 조인 68,836 불변 · 성별 실회원 100%) · ③ 「중단(명)」 **4건 처방 오류** — `SUM(STOP_MEMBERS)=SUM(STOP_CNT)=COUNT(*)`, 주차 「명」 합계 **972,942 vs 실제 903,064 = 7.7% 과대** → 비가산 `COUNT(DISTINCT)`. ✅ 1-2 경합 처방은 **성립**(사전집계 후 결합 → 40,054,883 불변). 🆕 **P104**. ⛔ **계정 정지로 09 재생성·골든 갱신 불가** — 골든은 현(틀린) 산출물을 고정 중이므로 **골든 통과 ≠ 정확** | 00 §O49 |
