<!-- SPLIT-CHUNK 50_dbt_파이프라인_미결조치.md | 014/019 | 허브 = 50_dbt_파이프라인_미결조치.md | 원문 2079~2280행 -->
...
- ✅ **A3 — FSE SERVICE_SK·SEND_TITLE** (`models/gold/fact/FACT_MESSAGE_DISPATCH.sql`): `CRM_SEND_MEMBER × CRM_SEND_REQUEST`(SNDNG_KEY, unique) 조인 → `SERVICE_SK = gold_sk([SEND_CHANNEL, SNDNG_TY_CD])`(DIM_SERVICE 동일 산식)·`SEND_TITLE=TIT`. **시뮬 검증: 커버 99.97%(38.46M/38.47M)·DIM_SERVICE 미매칭 SK=0건**. 미매칭 → SERVICE_SK=0(Unknown).
- ✅ **A1 — FMM DEV/STOP 건·명 + HAS_BILLING** (`models/gold/fact/FACT_MEMBER_MONTHLY.sql` + `06_DDL.sql`): 스파인 = 회비(billing) ∪ 개발/중단(FME 월 롤업). `DEV_CNT/DEV_MEMBERS/STOP_CNT/STOP_MEMBERS` 실채움. **신규 컬럼 `HAS_BILLING BOOLEAN`**(출처 플래그)로 보수(billing-only)·정확(전체) 양립. **시뮬 검증: 40,054,883행=distinct grain(fan-out 0)·HAS_BILLING=TRUE 37,792,336(구 스파인과 정확 일치)·EVENT_ONLY 2,262,547·DEV 2.97M/STOP 0.97M 월×회원**.
  - ⚠️ **DDL 변경**: FMM에 `HAS_BILLING` 1컬럼 추가(06_DDL.sql). fact는 append+pre-hook TRUNCATE라 **build 전 06_DDL 재실행(ALTER/재생성) 필요** — 미실행 시 신컬럼 부재로 append 실패.
    - 🟢🟢 **[2026-08-20 O91-B 충족 · 철회 병기] 위 「build 전 06_DDL 재실행 필요」는 이미 충족됐다 — 재실행 불요.**
      ...
      🟢 반영 시점 = `07`/`06_DDL` 실행분(`FACT_BIGQUERY_BEHAVIOR` `LAST_ALTERED 2026-08-18 20:51:48` 등).
...
- 🔜 **잔여(B계열) — ⏸️ Agent 오픈(SV 배포) 후 진행 결정**: SV/Agent를 먼저 열어 실사용 피드백을 받은 뒤 우선순위대로 착수. 데이터·조인은 준비됨, 각 게이트만 남음.
  ...
  - **B2 FMM SPONSORSHIP_SK·PAYMENT_SK**: 원천 컬럼(SPNSR_BSNS_ID·PAYMENT_TYPE 등) 존재 → **O8 다중후원(~~회원 9.9%·최대13~~ 🔴 stale) grain 규칙**(대표후원/최빈) 확정 후.
    🔴 **[2026-08-29 O112 봉합 · `decision_closure_gate` ㉥]** 이 줄의 수치는 **O92 가 stale 로 적발**했다 ⇒ **취소선으로 표시**한다(🔴 지우지 않는다 — 바로 아래 O92 병기가 *"위 「회원 9.9%·최대13」은 stale 이다"* 로 **이 문구를 선행사로 인용**한다).
    · **대체 실측·후원사업 축 종결 근거는 아래 O92 병기(같은 항목 안)**에 있다 ⇒ **월×회원 5.41% / 회원 14.23%** · 규칙 = `DEC-41`.
    · 🔴 **열린 것은 `PAYMENT_SK` 축**이다 — 이 항목 전체를 닫지 마라(아래 O92 병기 마지막 줄과 같은 판정).
...
- ⚠️ WIDE 6종(`WIDE_MEMBER_EVENT`·`WIDE_SERVICE_EVENT`·`WIDE_BIGQUERY_BEHAVIOR`·`WIDE_EVENT_PARTICIPATION`·`WIDE_AD_*`)이 이 컬럼을 **그대로 상속**한다 → 현재 전부 FALSE. 소비 측이 "휴일 아님"으로 오독할 위험.
...
> 🆕 🟢 **[2026-09-17 O170 집합 확정 — 「4 ↔ 6」은 불일치가 아니다. 제목의 6 은 원집합이다.]**
> 근거철 = `20_issue/_o170_evidence.md` **§E1**.
> · **6 = 원집합**(이 절 제목 · 2026-07 발행) · **4 = 현 잔여**(2026-09-08 `O144-C` 재판정 · 아래 O144-C 블록).
> · 인수인계의 「4」가 **현행**이고 두 수는 **서로 다른 집합**이므로 둘 다 참이다.
> 🔴 **제목의 6 을 4 로 고치지 마라** — 원집합은 감사 이력이다(`O91-F` 가 남긴 축). 대신 **병기로 읽는다.**
> 🟢 판정식 = **「N건」 표제를 볼 때 그것이 원집합인지 현 잔여인지 절 본문에서 확인하라.**

모델 작성 여부 결정 필요. **상세는 문서30 §6.** 요약: 소스 준비 4개(즉시 가능)·소스 입고대기 1개(FACT_TARGET_PROJECT, 원천=**CRM** 확정·신규 목표 테이블 대기)·`DIM_MEMBER_IDENTITY`는 **2026-07-15 활성화 완료**(enabled=true·XREF dedup 조인, 1,274명 매칭).

> 🆕 🟡 **[2026-09-08 O144-C 재판정 · 후속과제 후-2] 열림 유지 — 다만 6개 중 2개는 이미 처리됐다.**
> · 🟢 `DIM_MEMBER_IDENTITY` = 본문이 이미 **2026-07-15 활성화 완료**로 적는다.
> · 🟢 `FACT_TARGET_PROJECT` = **스켈레톤 모델이 저작·배포됐다**(아래 §BLOCKING-4 ⏳ 보류 1종 해소 블록) —
>   다만 **0행**이고 원천(E-6) 입고 대기다 ⇒ 「모델 부재」가 아니라 **데이터 부재**로 성격이 바뀌었다.
> · 🟡 **잔여 = 소스 준비 4개의 「모델 작성 여부」** ⇒ 🔴 **설계 결정 사안**(문서30 §6)이므로 이 절은 닫지 않는다.
> 🔴 **이 절을 🟢 로 바꾸지 마라** — 결정이 나기 전에 모델을 만들면 과잉 GOLD 가 된다(이 절의 존재 이유다).
